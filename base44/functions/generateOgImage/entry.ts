  ...
  try {
    ...
    const prompt = [
      ...
      'Center: the headline text "' +
        safeTitle +
        '" rendered large, white, heavy sans-serif (Inter Tight), tight leading, left-aligned within a left margin.',
      safeSub
        ? 'Below the headline, a single line of smaller muted gray (#B2B2B2) text: "' +
          safeSub +
          '".'
        : '',
    ...
    const existing = await base44.asServiceRole.entities.PageMeta.filter(
      { path: String(path) },
      '-updated_date',
      1,
    );
