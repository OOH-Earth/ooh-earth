...
Deno.serve(async (req) => {
  try {
    ...
    const subs = await base44.asServiceRole.entities.Subscription.filter({ user_id: caller.id }, "-created_date", 1);
    const sub = subs?.[0];
    if (!sub?.stripe_customer_id) return Response.json({ error: "no_subscription" }, { status: 404 });
    ...
    if (!res.ok) {
      ...
      return Response.json({ error: data?.error?.message || "Portal unavailable" }, { status: 400 });
    }
