...
async function fetchPolygon() {
  ...
  const body = [
    { jsonrpc: '2.0', id: 1, method: 'eth_getBalance', params: [addr, 'latest'] },
    {
      ...
      id: 2,
      method: 'eth_call',
      params: [{ to: USDC, data: enc(noPrefix) }, 'latest'],
    },
    {
      jsonrpc: '2.0',
      id: 3,
      method: 'eth_call',
      params: [{ to: USDCE, data: enc(noPrefix) }, 'latest'],
    },
  ];
...
Deno.serve(async (req) => {
  try {
    ...
    if (solR.status === 'rejected')
      console.error('cryptoWatch sol:', solR.reason?.message || solR.reason);
    if (ethR.status === 'rejected')
      console.error('cryptoWatch eth:', ethR.reason?.message || ethR.reason);
    ...
    return Response.json(
      {
        ...
        eth: [],
        polygon: null,
        status: { sol: 'error', eth: 'error', polygon: 'error' },
        ts: Date.now(),
      },
      { status: 200 },
    );
