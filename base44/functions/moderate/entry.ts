...
const CAN_VIEW = (u) =>
  !!u && (roleOf(u) === 'admin' || ['admin', 'moderator', 'operative'].includes(accessOf(u)));
const CAN_ACT = (u) =>
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'verify') {
      if (!CAN_ACT(caller))
        return Response.json(
          { error: 'Forbidden — moderator or admin clearance required.' },
          { status: 403, headers },
        );
      ...
      if (!ENTITIES.has(entity))
        return Response.json(
          { error: `Invalid entity. Use: ${[...ENTITIES].join(', ')}` },
          { status: 400, headers },
        );
      if (!id) return Response.json({ error: 'Missing id.' }, { status: 400, headers });
      if (!VERIFY_STATUS.has(status))
        return Response.json(
          { error: `Invalid status. Use: ${[...VERIFY_STATUS].join(', ')}` },
          { status: 400, headers },
        );
      ...
      if (entity === 'Location') {
        try {
          const photos = await base44.asServiceRole.entities.LocationPhoto.filter({
            location_id: id,
            status: 'pending',
          });
          await Promise.all(
            (photos || []).map((p) =>
              base44.asServiceRole.entities.LocationPhoto.update(p.id, { status }),
            ),
          );
        } catch {
          /* gallery cascade is best-effort — location status change still succeeds */
        }
      }
    ...
    return Response.json(
      { error: `Unknown action '${action}'. Use: queue | verify.` },
      { status: 400, headers },
    );
  } catch (error) {
