...
**UPDATE 2026-08-23 — the deferral is now confirmed structural, not just "not done yet":** checked `docs.base44.com` directly rather than assuming. Base44's custom-domain docs (`Setting-up-your-app/Connecting-an-external-domain.md`) state the domain's DNS points directly at Base44's infrastructure (`base44.onrender.com` / a fixed IP) and that Base44 issues/renews the TLS certificate — there is **no documented mechanism for the app owner to interpose a proxy/CDN and add custom response headers**, and no page anywhere in Base44's docs (checked the full `llms.txt` index) covers Content-Security-Policy, HSTS, or any other custom header. This means the policy below cannot be deployed from this repo, from Dave's DNS control, or from any Base44 dashboard setting found in their docs. The two remaining paths are: (1) Base44 ships a native header-configuration feature (external, track via their support/roadmap), or (2) Dave deliberately decides to front the domain with a third-party CDN/proxy he controls — a real production-infrastructure change with its own risk (could interfere with Base44's automatic certificate issuance) that needs his explicit sign-off, not something to implement unilaterally. This appendix stays ready-to-apply the moment either path opens up.

**Why the policy itself was deferred**: the app embeds ~15 independent radio-stream origins, Stripe.js (dynamically-injected script), a Donorbox payment iframe, an OSM iframe embed, a YouTube iframe embed, MapLibre GL (which injects its own worker/style/glyph loads), multiple blockchain RPC/explorer APIs, and a Base44 API+WebSocket host resolved at runtime from `VITE_BASE44_APP_BASE_URL`/an app-param — not statically knowable from source. Shipping a guessed policy risks silently breaking checkout, maps, or radio playback in production. This is exactly the "needs methodical per-integration testing" case KNOWN_ISSUES #19 already flagged — confirmed, not resolved, this cycle.

**Re-verified 2026-08-23 against current `main` (112 commits after this appendix was first drafted) — two corrections:**
- **`img-src` was missing `base44.app`** — Base44's own bundled-asset file host (`base44.app/api/apps/.../files/...`, distinct from `media.base44.com`), used across `MetroKit.jsx`, `CampaignAtlas.jsx`, `About.jsx` and others. Added below.
- **`media-src`'s radio-station list was stale.** `radioStations.js` currently defines **16** stream hosts, not 15, and the set has changed: `radio.stereoscenic.com` and a raw IP (`198.15.94.34:8006`) are now present; `radio.ooh.earth` (the self-hosted stream) is **not** currently live — `radioOps.js`'s `OOH_STREAM_URL` is an empty string (`RADIO_OPS_ENABLED = false`), so that origin should be re-added only if/when self-hosting is actually turned on. List corrected below.
- Everything else (script-src, connect-src, frame-src, the "plain link, not a resource load" exclusions for `zora.co`, the `*.framer.ai`/`.framer.wiki` mini-sites, blockchain explorer links, wallet install links) re-checked via a fresh grep across `src/` + `base44/functions/` and found unchanged.
...
- **img-src**: `'self'`, `data:`, `blob:` (local photo previews via `URL.createObjectURL` in `MultiPhotoUpload.jsx`), `media.base44.com`, `base44.app` (bundled static assets — separate host from `media.base44.com`), `firebasestorage.googleapis.com`, `images.unsplash.com`, `static.wixstatic.com`, `oohearth.app` (legacy WordPress media, KNOWN_ISSUES #4), `i.ytimg.com`, `*.basemaps.cartocdn.com`, `server.arcgisonline.com`.
...
- **media-src**: the 16 radio-stream hosts currently defined in `radioStations.js` (verified directly against that file, not a source grep): `ice5.somafm.com`, `icecast.walmradio.com`, `jazzblues.ice.infomaniak.ch`, `kexp-mp3-128.streamguys1.com`, `live.amperwave.net`, `livex.radiopopolare.it`, `npr-ice.streamguys1.com`, `radio.stereoscenic.com`, `st01.sslstream.dlf.de`, `stream-uk1.radioparadise.com`, `stream.europe1.fr`, `stream.live.vc.bbcmedia.co.uk`, `stream.radioparadise.com`, `tunein.cdnstream1.com`, `wwoz-sc.streamguys.com`, and the raw IP `198.15.94.34` (a station's stream host has no domain name — CSP source lists accept IPs, but pin the port too: `198.15.94.34:8006`). `radio.ooh.earth` (self-hosted stream via `radioOps.js`) is currently disabled (`OOH_STREAM_URL` empty) — add it back only when that feature is turned on.
