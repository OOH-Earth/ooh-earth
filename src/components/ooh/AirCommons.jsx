...
const CITIES = [
  { city: "Bangkok", lat: 13.7563, lng: 100.5018, coords: "13.7563°N · 100.5018°E", status: "Active chapter" },
  { city: "London", lat: 51.489, lng: -0.013, coords: "51.4890°N · 0.0130°E", status: "Active chapter" },
  { city: "Delhi", lat: 28.6139, lng: 77.209, coords: "28.6139°N · 77.2090°E", status: "Monitored" },
  { city: "Jakarta", lat: -6.2088, lng: 106.8456, coords: "6.2088°S · 106.8456°E", status: "Monitored" },
];
      ...
      <div className="px-5 py-16 md:px-8 md:py-24">
        <div className="flex flex-col gap-4 border-b border-slate2/40 pb-6 md:flex-row md:items-end md:justify-between">
          ...
          <p className="max-w-sm font-display text-sm font-normal leading-[1.4] text-darkgray">
            Outdoor advertising accelerates the consumption economy that poisons the air. PM2.5 is the measurable proof — so we log it live alongside every intervention. The brands on the billboards and the particulates in our lungs share a supply chain.
          </p>
        </div>
        ...
        <div className="mt-10 border border-slate2/60">
          {CITIES.map((c, i) => {
            ...
            return (
              <div key={c.city} className={`p-6 md:p-8 ${i !== CITIES.length - 1 ? "border-b border-slate2/60" : ""}`}>
                ...
                <div className="relative mt-5 h-2 w-full bg-slate2/60">
                  <div className="absolute inset-y-0 left-0" style={{ width: `${fillW}%`, backgroundColor: b.color }} />
                  <div className="absolute inset-y-[-4px] w-px bg-silver/60" style={{ left: `${whoLeft}%` }} title="WHO 24-hour guideline" />
                </div>
                ...
                <div className="mt-3 flex items-center justify-between font-mono text-[10px] uppercase tracking-[0.2em]">
                  ...
                  <span className="text-flare">{mult}{has ? "× WHO limit" : ""}</span>
                </div>
              </div>
            );
          })}
        ...
        <div className="mt-6 flex flex-col gap-3 border-t border-slate2/40 pt-6 md:flex-row md:items-center md:justify-between">
          <p className="flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.25em] text-darkgray">
            ...
            {updated && <span className="text-dim">· updated {updated.toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" })}</span>}
          </p>
          <p className="font-display text-sm font-medium leading-[1.4] text-silver/70">The air commons is not for sale.</p>
        </div>
      </div>
    </section>
  );
