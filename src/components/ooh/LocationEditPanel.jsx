...
function SectionHeader({ num, label, complete }) {
  return (
    <div className="mb-2 flex items-center gap-2">
      <span className="flex h-4 w-4 items-center justify-center border border-slate2 font-mono text-[8px] font-bold text-dim">
        {num}
      </span>
      <span className="font-mono text-[9px] uppercase tracking-[0.25em] text-ozone/70">
        {label}
      </span>
      {complete ? (
      ...
      {needsAttention && !open && (
        <div className="flex items-center gap-2 border-t border-flare/30 bg-flare/5 px-4 py-2">
          ...
          <span className="font-mono text-[9px] uppercase tracking-[0.15em] text-flare">
            Needs classification — {!loc.brand_name && 'no brand'}
            {!loc.brand_name && !loc.industry_sector && ', '}
            {!loc.industry_sector && 'no sector'}
          </span>
        </div>
      )}
      ...
      {!open && (
        <div className="flex gap-px border-t border-slate2/40">
          <button
            ...
            disabled={saving}
            className="flex flex-1 items-center justify-center gap-1.5 py-2.5 font-mono text-[9px] font-bold uppercase tracking-[0.2em] text-ozone transition-colors hover:bg-ozone/10 disabled:opacity-40"
          >
          ...
          <button
            ...
            disabled={saving}
            className="flex flex-1 items-center justify-center gap-1.5 py-2.5 font-mono text-[9px] font-bold uppercase tracking-[0.2em] text-flare transition-colors hover:bg-flare/10 disabled:opacity-40"
          >
          ...
          <button
            onClick={startEdit}
            className="flex flex-1 items-center justify-center gap-1.5 py-2.5 font-mono text-[9px] font-bold uppercase tracking-[0.2em] text-darkgray transition-colors hover:bg-slate2/20"
          >
        ...
        <div className="space-y-5 border-t border-slate2/40 px-4 py-5">
          <div>
            <SectionHeader
              ...
              complete={completeness.hasType && completeness.hasSector && completeness.hasCondition}
            />

            {/* Quick classify — determines which map layer this location appears on */}
            <div className="mb-3">
              <span className="mb-1.5 block font-mono text-[9px] uppercase tracking-[0.15em] text-dim">
                Classify as
              </span>
              <div className="flex flex-wrap gap-1.5">
                <button
                  onClick={() => quickClassify('ad')}
                  className={`flex items-center gap-1.5 border px-3 py-1.5 font-mono text-[9px] font-bold uppercase tracking-[0.12em] transition-colors ${formCategory === 'ad' ? 'border-ozone bg-ozone text-void' : 'border-slate2 text-darkgray hover:border-ozone hover:text-ozone'}`}
                >
                  <Megaphone className="h-3 w-3" /> Ad Spot
                </button>
                <button
                  onClick={() => quickClassify('adbust')}
                  className={`flex items-center gap-1.5 border px-3 py-1.5 font-mono text-[9px] font-bold uppercase tracking-[0.12em] transition-colors ${formCategory === 'adbust' ? 'border-flare bg-flare text-void' : 'border-slate2 text-darkgray hover:border-flare hover:text-flare'}`}
                >
                  <Ban className="h-3 w-3" /> Adbust / Subvertising
                </button>
                <button
                  onClick={() => quickClassify('graffiti')}
                  className={`flex items-center gap-1.5 border px-3 py-1.5 font-mono text-[9px] font-bold uppercase tracking-[0.12em] transition-colors ${formCategory === 'graffiti' ? 'border-flare bg-flare text-void' : 'border-slate2 text-darkgray hover:border-flare hover:text-flare'}`}
                >
              ...
              {formCategory === 'graffiti' && (
                <p className="mt-1.5 font-mono text-[9px] text-flare/80">
                  // This location will appear on the Graffiti map layer
                </p>
              )}
              {formCategory === 'adbust' && (
                <p className="mt-1.5 font-mono text-[9px] text-flare/80">
                  // This location will appear on the Adbusting map layer
                </p>
              )}
            </div>
            ...
            {formCategory === 'graffiti' && (
              <div className="mt-3 grid gap-3 border border-flare/30 bg-flare/5 p-3 sm:grid-cols-2 lg:grid-cols-4">
                <label className="flex flex-col gap-1">
                  ...
                  <select
                    value={form.graffiti_medium}
                    onChange={(e) => set('graffiti_medium', e.target.value)}
                ...
                <label className="flex flex-col gap-1">
                  ...
                  <select
                    value={form.graffiti_style}
                    onChange={(e) => set('graffiti_style', e.target.value)}
                    className="border border-slate2 bg-void px-3 py-2 text-sm text-silver outline-none focus:border-flare"
                  >
                    <option value="">— None —</option>
                ...
                <label className="flex flex-col gap-1">
                  ...
                  <input
                    type="number"
                    step="0.1"
                    value={form.graffiti_surface_m2 || ''}
                    onChange={(e) =>
                      set('graffiti_surface_m2', e.target.value ? parseFloat(e.target.value) : '')
                    }
                    className="border border-slate2 bg-void px-3 py-2 text-sm text-silver outline-none focus:border-flare"
                    placeholder="e.g. 4.5"
                  />
                ...
                <label className="flex flex-col gap-1">
                  ...
                  <input
                    ...
                    min="0"
                    max="100"
                    value={form.graffiti_coverage_pct || ''}
                    onChange={(e) =>
                      set('graffiti_coverage_pct', e.target.value ? parseInt(e.target.value) : '')
                    }
                    className="border border-slate2 bg-void px-3 py-2 text-sm text-silver outline-none focus:border-flare"
                    placeholder="0–100"
                  />
          ...
          <div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="flex flex-col gap-1">
                ...
                />
                {form.brand_name && (
                  <BrandBadge name={form.brand_name} className="mt-1 self-start" />
                )}
              </label>
              <label className="flex flex-col gap-1">
                <span className="font-mono text-[9px] uppercase tracking-[0.15em] text-dim">
              ...
              <label className="flex flex-col gap-1">
                ...
                />
                {form.parent_corp && (
                  <BrandBadge name={form.parent_corp} className="mt-1 self-start" />
                )}
              </label>
              <label className="flex flex-col gap-1 sm:col-span-2">
                <span className="font-mono text-[9px] uppercase tracking-[0.15em] text-dim">
                  OOH Operator (structure owner)
                </span>
                <input
                  list="dl-operators"
                ...
                />
                {form.ooh_operator && (
                  <BrandBadge name={form.ooh_operator} className="mt-1 self-start" />
                )}
              </label>
            </div>
          ...
          <div>
            <SectionHeader
              ...
              complete={(form.harm_tags || []).length > 0 || !!form.harm_statement}
            />
              ...
              {HARM_TAG_OPTIONS.map((tag) => {
                ...
                return (
                  <button
                    ...
                    className={`border px-2.5 py-1 font-mono text-[9px] uppercase tracking-[0.1em] transition-colors ${active ? 'border-flare bg-flare/15 text-flare' : 'border-slate2 text-dim hover:border-ozone/50 hover:text-silver'}`}
                  >
                    {tag.replace(/_/g, ' ')}
                  </button>
                );
                ...
                {ACTION_FLAG_OPTIONS.map((flag) => {
                  ...
                  return (
                    <button
                      ...
                      className={`border px-2.5 py-1 font-mono text-[9px] uppercase tracking-[0.1em] transition-colors ${active ? 'border-ozone bg-ozone/15 text-ozone' : 'border-slate2 text-dim hover:border-ozone/50 hover:text-silver'}`}
                    >
                      {flag.replace(/_/g, ' ')}
                    </button>
                  );
          ...
          <div>
            <SectionHeader
              ...
              complete={form.adbust_type !== 'none'}
            />
            <label className="flex flex-col gap-1">
              <span className="font-mono text-[9px] uppercase tracking-[0.15em] text-dim">
                Adbust / Subvertising Type
              </span>
              <select
                value={form.adbust_type}
                onChange={(e) => set('adbust_type', e.target.value)}
                className="border border-slate2 bg-void px-3 py-2 text-sm text-silver outline-none focus:border-ozone"
              >
                {ADBUST_TYPES.map((a) => (
            ...
            {formCategory === 'graffiti' && (
              <p className="mt-2 font-mono text-[9px] text-flare/70">
                // Graffiti details are in Section 1 above
              </p>
            )}
          </div>
