  ...
  if (error === 'not-secure') {
    return (
      <div className="flex aspect-[4/3] flex-col items-center justify-center gap-2 border border-slate2 bg-card text-center">
        ...
        <span className="px-6 font-mono text-[9px] uppercase tracking-[0.15em] text-dim">
          Publish the app to activate the on-site viewfinder
        </span>
      </div>
    );
  }
    ...
    return (
      <div className="flex aspect-[4/3] flex-col items-center justify-center gap-2 border border-slate2 bg-card text-center">
        ...
        <button
          ...
          className="mt-1 inline-flex items-center gap-1.5 border border-slate2 px-3 py-2 font-mono text-[9px] uppercase tracking-[0.2em] text-darkgray transition-colors hover:border-ozone hover:text-ozone"
        >
  ...
  return (
    <div className="relative aspect-[4/3] overflow-hidden border border-slate2 bg-black">
      ...
      <div className="pointer-events-none absolute inset-0">
        ...
        <span className="absolute left-3 top-3 pl-7 font-mono text-[8px] uppercase tracking-[0.2em] text-ozone/80">
          // live
        </span>
      </div>
      <button
      ...
      >
        {uploading ? (
          ...
          <span className="h-6 w-6 rounded-full bg-ozone" />
        )}
      </button>
      {!ready && !uploading && (
