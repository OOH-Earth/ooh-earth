...
const SIGNALS = [
  ...
  {
    match: ['iss', 'satellite', 'flyover', 'pass'],
    tag: 'transit · arbitrage window',
    tone: 'ozone',
  },
...
const toneClass = (tone) =>
  tone === 'ozone'
    ? 'text-ozone border-ozone/50'
    : tone === 'flare'
      ? 'text-flare border-flare/50'
      : 'text-dim border-slate2';
...
export default function SkyIntel() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    base44.functions
      .invoke('cachedIntel', { key: 'skyIntel' })
      .then((res) => {
        if (cancelled) return;
    ...
    <section className="border-t border-slate2/40 px-5 py-14 md:px-8">
      <div className="mx-auto max-w-5xl">
        <div className="flex items-center justify-between">
          <div>
            <span className="font-mono text-[10px] uppercase tracking-[0.3em] text-ozone animate-flicker">
              // sky intel — astro activity
            </span>
            <h2 className="mt-2 font-display text-2xl font-bold tracking-[-0.02em] text-silver md:text-4xl">
              What the cryptids are watching tonight
            </h2>
          </div>
        ...
        <div className="mt-7 grid gap-3 md:grid-cols-[280px_1fr]">
          ...
          <div className="flex flex-col gap-3 border border-slate2/60 bg-card p-5">
            ...
            <div className="mt-1 border border-slate2 px-2 py-1.5">
              ...
              <div
                className={`mt-0.5 font-mono text-[10px] uppercase tracking-[0.15em] ${moonSignal.tone === 'ozone' ? 'text-ozone' : moonSignal.tone === 'flare' ? 'text-flare' : 'text-dim'}`}
              >
                {moonSignal.tag}
              </div>
            </div>
            <p className="mt-1 font-mono text-[9px] leading-[1.5] uppercase tracking-[0.15em] text-dim">
              synodic 29.53d · phase drives action-intensity · full-moon windows marked ozone
            </p>
          </div>
            ...
            <div className="max-h-[360px] overflow-y-auto atlas-track">
              {loading ? (
                ...
                <div className="px-4 py-10 text-center">
                  ...
                  <span className="mt-2 block font-mono text-[10px] uppercase tracking-[0.25em] text-dim">
                    // clear skies — no notable events
                  </span>
                </div>
              ) : (
                events.map((ev, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className="flex gap-3 border-b border-slate2/40 px-4 py-3 last:border-b-0"
                    >
                      <div className="shrink-0 text-right">
                        <div className="font-mono text-[10px] font-bold tabular text-ozone">
                          {fmtDate(ev.date)}
                        </div>
                        <div className="font-mono text-[8px] uppercase tracking-[0.2em] text-dim/60">
                          {String(i + 1).padStart(2, '0')}
                        </div>
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="font-display text-sm font-bold tracking-[-0.01em] text-silver">
                          {ev.title}
                        </div>
                        <p className="mt-0.5 font-body text-xs leading-[1.5] text-darkgray">
                          {ev.body}
                        </p>
                        <span
                          className={`mt-1.5 inline-block border px-1.5 py-0.5 font-mono text-[8px] font-bold uppercase tracking-[0.2em] ${toneClass(sig.tone)}`}
                        >
                          {sig.tag}
                        </span>
                      </div>
