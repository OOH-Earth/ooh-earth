    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="start" className="w-64 border border-slate2 bg-void p-3">
        ...
        <div className="mt-2 border border-slate2 bg-card p-2.5">
          <div className="font-mono text-[7px] uppercase tracking-[0.25em] text-dim">
            Asia/Bangkok
          </div>
          <div className="mt-1 font-mono text-xl font-bold tabular tracking-tight text-silver text-glow-ozone">
            {time}
          </div>
        ...
        <div className="mt-2 grid grid-cols-2 gap-1">
          {tiles.map((t) => (
            <div key={t.k} className="border border-slate2 bg-card p-2">
              <div className="flex items-center justify-between">
                <span className="font-mono text-[7px] uppercase tracking-[0.2em] text-dim">
                  {t.k}
                </span>
                <t.Icon className="h-2.5 w-2.5" style={{ color: t.c }} />
              </div>
              <div className="mt-0.5 font-mono text-base font-bold tabular" style={{ color: t.c }}>
                {t.v}
              </div>
            </div>
          ))}
        ...
        <div className="mt-2 grid grid-cols-2 gap-1">
          {shortcuts.map((s) => (
            ...
            >
              ...
              <span className="font-mono text-[8px] font-bold uppercase tracking-[0.2em] text-silver">
                {s.label}
              </span>
            </Link>
          ))}
