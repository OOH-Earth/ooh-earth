  ...
  return (
    <div className="flex flex-col gap-1 border border-slate2/50 bg-void/40 px-4 py-4">
      <div className="flex items-center gap-2">
        ...
        <span className="font-display text-2xl font-black tabular tracking-[-0.02em] text-flare md:text-3xl">{value}</span>
  ...
  return (
    <section className={`border border-slate2/60 bg-card ${className}`}>
      <div className="flex flex-col gap-3 border-b border-slate2/40 p-6 md:p-8">
        <span className="font-mono text-[10px] uppercase tracking-[0.3em] text-ozone">// Not starting from zero</span>
        <h2 className="font-display text-3xl font-bold leading-[1.05] tracking-[-0.02em] text-silver md:text-5xl">{heading}</h2>
        ...
        <span className="mt-1 inline-flex w-fit items-center gap-2 border border-ozone/50 bg-ozone/5 px-3 py-1.5">
          ...
          <span className="font-mono text-[9px] font-bold uppercase tracking-[0.25em] text-ozone">OOH Earth · {PLATFORM_STATUS}</span>
        </span>
      </div>
      ...
      <ul className="divide-y divide-slate2/40 border-t border-slate2/40">
        {MOVEMENT_ANCHORS.map((a, i) => (
          <li key={i} className="flex items-baseline gap-4 px-6 py-3 md:px-8">
            <span className="w-10 shrink-0 font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-flare">{a.year}</span>
            <span className="flex-1 font-display text-sm leading-[1.5] text-silver/90">{a.text}</span>
            <span className="hidden shrink-0 font-mono text-[9px] uppercase tracking-[0.15em] text-dim sm:inline">{a.source}</span>
          </li>
        ))}
      </ul>

      <p className="border-t border-slate2/40 px-6 py-3 font-mono text-[9px] leading-relaxed tracking-[0.1em] text-dim md:px-8">
        // Movement-wide estimates (est.) since {MOVEMENT.since} — the global subvertising movement, not OOH Earth platform metrics. Order-of-magnitude, conservative. OOH Earth&rsquo;s own numbers are live and audited elsewhere on the platform.
      </p>
    </section>
  );
}
