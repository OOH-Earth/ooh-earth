...
function pmBand(v) {
  if (v == null || Number.isNaN(v))
    return { label: 'NO DATA', text: 'text-dim', bar: 'bg-dim', w: 0 };
  if (v <= WHO_PM25)
    return {
      label: 'GOOD',
      text: 'text-[#39FF14]',
      bar: 'bg-[#39FF14]',
      w: Math.min(v / WHO_PM25, 1),
    };
  if (v <= 35)
    return {
      label: 'MODERATE',
      text: 'text-ozone',
      bar: 'bg-ozone',
      w: Math.min((v - WHO_PM25) / (35 - WHO_PM25), 1),
    };
  if (v <= 55)
    return {
      label: 'UNHEALTHY',
  ...
  return (
    <div className="pointer-events-none absolute inset-0 z-[900] select-none font-mono text-[10px] uppercase tracking-[0.18em] text-silver/70">
        ...
        <div className="mt-2 flex items-center gap-2 border-t border-slate2/40 pt-2">
          ...
          <div>
            ...
            <div className="text-dim">
              {(moon.illum * 100).toFixed(0)}% lit · stage {(moon.phase * 100).toFixed(0)}
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="flex gap-4 border border-slate2/70 bg-void/70 px-3 py-1.5 backdrop-blur-md">
          <span className="text-dim">
            ...
            <span className="text-silver">
              {fmt(tel.lng)},{fmt(tel.lat)}
            </span>
          </span>
          <span className="text-dim">
            BRG <span className="text-ozone">{fmt(tel.bearing, 0)}°</span>
          </span>
          <span className="text-dim">
            PITCH <span className="text-ozone">{fmt(tel.pitch, 0)}°</span>
          </span>
          <span className="text-dim">
            ZOOM <span className="text-ozone">{fmt(tel.zoom, 1)}</span>
          </span>
          <span className="text-dim">
            SPAN <span className="text-flare">{fmt(tel.span, 0)} km</span>
          </span>
        </div>
      </div>
      ...
      {showAir ? (
        <div
          data-tour="hud-pm25"
          className="pointer-events-auto absolute right-3 top-16 hidden w-[230px] md:block"
        >
          <div className="border border-slate2/70 bg-void/70 backdrop-blur-md">
            <div className="flex items-center justify-between border-b border-slate2/60 px-3 py-1.5">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => setShowAir(false)}
                  aria-label="Hide Air Commons"
                ...
                pm.map((p, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-2 py-0.5">
                      ...
                      <span className={`w-9 text-right ${b.text}`}>
                        {p.v != null ? p.v.toFixed(0) : '--'}
                      </span>
                      <span className="h-1 flex-1 bg-slate2/50">
                        <span
                          className={`block h-full ${b.bar}`}
                          style={{ width: `${b.w * 100}%` }}
                        />
                      </span>
        ...
        <button
          onClick={() => setShowAir(true)}
          className="pointer-events-auto absolute right-3 top-16 hidden border border-slate2/70 bg-void/70 px-2.5 py-1.5 text-ozone backdrop-blur-md transition-colors hover:border-ozone md:block"
        >
          AIR
        </button>
      )}
