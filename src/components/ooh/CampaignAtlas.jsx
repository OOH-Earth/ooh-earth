      ...
      <div className="px-5 md:px-8">
        <div className="flex flex-col gap-4 border-b border-slate2/40 pb-6 md:flex-row md:items-end md:justify-between">
          ...
          <p className="max-w-sm font-display text-sm font-normal leading-[1.4] text-darkgray">
            Not ads — interventions. Each plate is a creative resistance logged in the field and
            archived for the public record, mapped to its UN SDG outcome. Traverse the meridian to
            inspect.
          </p>
        </div>
      </div>
      ...
      {!reduced && (
        <div className="flex items-center justify-between px-5 py-4 md:px-8">
          ...
          <div className="flex gap-2">
            <button
              onClick={() => scrollBy(-1)}
              className="border border-slate2 px-3 py-2 font-mono text-[10px] uppercase tracking-[0.2em] text-darkgray transition-colors hover:border-ozone hover:text-ozone"
            >
              Prev
            </button>
            <button
        ...
        {PLATES.map((p) => (
          ...
          >
            <div className={`relative ${reduced ? 'aspect-[4/3]' : 'aspect-[3/4]'}`}>
              ...
              <span className="absolute left-4 top-4 font-mono text-[10px] uppercase tracking-[0.3em] text-silver/80">
                Plate {p.no}
              </span>
              <span className="absolute right-4 top-4 font-mono text-[10px] uppercase tracking-[0.25em] text-ozone">
                {p.sentiment}
              </span>
              ...
              <div className="absolute inset-x-0 bottom-0 p-5">
                <div className="flex items-end justify-between">
                  <div>
                    <h3 className="font-display text-3xl font-bold leading-[1.1] tracking-[-0.02em] text-silver md:text-4xl">
                      {p.title}
                    </h3>
                    <span className="mt-2 block font-mono text-[10px] uppercase tracking-[0.2em] text-darkgray">
                      {p.medium}
                    </span>
                  </div>
                  <a
