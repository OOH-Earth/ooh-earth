  ...
  return (
    <div className="border border-slate2/60 bg-card p-6">
      ...
      <div className="mt-4 flex gap-2">
        <Link
          to="/login"
          className="border border-ozone bg-ozone px-4 py-2 font-mono text-[9px] font-bold uppercase tracking-[0.2em] text-void transition-colors hover:bg-flare hover:border-flare"
        >
          Log in
        </Link>
        <Link
          to="/register"
          className="border border-slate2 px-4 py-2 font-mono text-[9px] font-bold uppercase tracking-[0.2em] text-darkgray transition-colors hover:border-ozone hover:text-ozone"
        >
          Register
        </Link>
      </div>
  ...
  if (loading) {
    return (
      <div className="flex items-center gap-3 border border-slate2/60 bg-card p-6 font-mono text-[10px] uppercase tracking-[0.3em] text-dim">
        <span className="h-1.5 w-1.5 animate-flicker rounded-full bg-ozone" /> Compiling member
        data…
      </div>
    );
  }
  ...
  return (
    <div className="border border-ozone/40 bg-card p-5">
      <div className="flex items-center justify-between">
        ...
        <Link
          to="/operative"
          className="flex items-center gap-1 font-mono text-[9px] uppercase tracking-[0.2em] text-darkgray transition-colors hover:text-ozone"
        >
      ...
      <div className="mt-4 flex items-center gap-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between">
            <span className="font-display text-lg font-black uppercase tracking-tight text-silver">
              {level?.title}
            </span>
            <span className="font-mono text-[9px] tabular text-ozone">
          ...
          <div className="mt-1 flex items-center justify-between font-mono text-[8px] uppercase tracking-[0.15em] text-dim">
            <span>
              {level?.isMax ? 'Max' : `${level?.xpIntoLevel}/${level?.xpForNext} to next`}
            </span>
      ...
      <div className="mt-4 grid grid-cols-3 gap-2">
        <div className="border border-slate2/60 bg-void p-2.5 text-center">
          ...
          <div className="mt-1 font-mono text-sm font-bold tabular text-silver">
            {earnedBadges.length}
          </div>
            ...
            {activeQuests.map((q) => {
              ...
              return (
                <div key={q.id} className="flex items-center gap-2">
                  <span className="w-28 shrink-0 truncate font-mono text-[8px] uppercase tracking-[0.1em] text-darkgray">
                    {q.label}
                  </span>
                  <div className="h-1 flex-1 overflow-hidden bg-slate2/60">
                    <div className="h-full bg-ozone transition-all" style={{ width: `${pct}%` }} />
                  </div>
                  <span className="shrink-0 font-mono text-[8px] tabular text-dim">
                    {q.progress}/{q.target}
                  </span>
                </div>
              );
