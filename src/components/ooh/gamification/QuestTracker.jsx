      ...
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5">
            <span className="font-mono text-[9px] font-bold uppercase tracking-[0.15em] text-silver">{quest.label}</span>
            {claimed && <Check className="h-3 w-3 text-brand-green" />}
          </div>
          <div className="font-mono text-[8px] uppercase tracking-[0.1em] text-dim">{quest.desc}</div>
        </div>
        <span className="shrink-0 font-mono text-[9px] tabular text-ozone">+{quest.reward_xp} XP</span>
      </div>
    ...
    <div className="space-y-6">
      <div>
        <div className="mb-3 flex items-center justify-between">
          ...
          <span className="font-mono text-[9px] tabular text-dim">{dailyDone}/{daily.length} claimed</span>
        </div>
        <div className="grid gap-2.5 sm:grid-cols-2">
          {daily.map(q => (
            <QuestRow key={q.id} quest={q} onClaim={() => onClaim(q.id)} claiming={claiming === q.id} claimed={q.claimed} />
          ))}
        </div>
      </div>
      ...
      <div>
        <div className="mb-3 flex items-center justify-between">
          ...
          <span className="font-mono text-[9px] tabular text-dim">{weeklyDone}/{weekly.length} claimed</span>
        </div>
        <div className="grid gap-2.5 sm:grid-cols-2">
          {weekly.map(q => (
            <QuestRow key={q.id} quest={q} onClaim={() => onClaim(q.id)} claiming={claiming === q.id} claimed={q.claimed} />
      ...
      <div className="flex items-center gap-2 border border-slate2/40 bg-void p-3">
        ...
        <p className="font-mono text-[9px] leading-relaxed text-dim">
          Quests reset daily at midnight and weekly on Monday. Claim completed quests to bank bonus XP — unclaimed quests expire when the period rolls over.
        </p>
      </div>
    </div>
  );
