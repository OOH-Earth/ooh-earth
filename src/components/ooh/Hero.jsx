...
const VIDEO_SRC =
  'https://firebasestorage.googleapis.com/v0/b/standards-site-beta.appspot.com/o/documents%2Fusaglsjaht9%2Fa61ac5238ce%2FSubs6.mp4?alt=media&token=fe1621f1-39b5-4f76-b11e-0700ecedcfba';
...
export default function Hero() {
  ...
  return (
    <section
      id="top"
      className="hero-section relative min-h-[100svh] w-full overflow-hidden bg-void"
    >
      <div
      ...
      <div className="relative z-20 flex h-full flex-col justify-between px-6 pb-6 pt-[calc(7rem_+_env(safe-area-inset-top))] md:px-10 md:pb-10 md:pt-[calc(8rem_+_env(safe-area-inset-top))]">
        <div className="flex items-center justify-end gap-3">
          <span className="flex shrink-0 items-center gap-2 font-mono text-[10px] uppercase tracking-[0.3em] text-silver/50">
            ...
            <span className="glitch" data-text="LIVE FEED">
              LIVE FEED
            </span>
          </span>
        </div>
            ...
            <h1 className="whitespace-nowrap font-brand text-[clamp(1.5rem,7vw,4.5vw)] leading-[0.85] tracking-[-0.03em] text-silver">
              {WORD.split('').map((ch, i) => (
                <span key={i} className="inline-block">
                  {ch === '.' ? <span className="text-ozone">.</span> : ch}
                </span>
              ))}
            </h1>
            <p className="mt-6 max-w-[42ch] font-display text-[15px] font-medium leading-[1.6] text-silver/70 md:text-base">
              An open-source, community-funded app reclaiming the visual commons. Mapping corporate
              advertising offenses and street-art adbusting worldwide — SDG-aligned, from the
              orbital perspective.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              <Link
                to="/map"
                className="group inline-flex items-center gap-2 bg-ozone px-5 py-3 font-mono text-[10px] font-bold uppercase tracking-[0.25em] text-void transition-colors hover:bg-flare"
              >
        ...
        <div className="mt-4 md:hidden">
          <div className="border border-ozone/20 bg-void/60 px-3.5 py-2.5 backdrop-blur-sm">
            <div className="font-mono text-[9px] uppercase leading-[1.5] tracking-[0.25em] text-silver/70">
              <span className="text-ozone/70">// </span>Field dispatch · subvertising reel
            </div>
        ...
        <div className="flex justify-center pt-4">
          <a
            href="#atlas"
            className="flex flex-col items-center gap-2 font-mono text-[10px] uppercase tracking-[0.3em] text-silver/50 transition-colors hover:text-ozone"
          >
            <span>Descend</span>
