...
const GOALS = [
  ...
  { n: "9", title: "Industry, innovation & infrastructure", body: "Open mapping infrastructure for civic documentation at scale." },
  { n: "17", title: "Partnerships for the goals", body: "A union of ad-industry veterans, street artists and on-chain communities." },
];
      ...
      <div className="px-5 py-16 md:px-8 md:py-24">
            ...
            return (
              <div key={d.n} className="bg-card p-6">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-dim">{d.sdg}</span>
                </div>
                <div className="mt-6 font-mono text-[10px] uppercase tracking-[0.2em] text-flare">{d.n}</div>
                <h3 className="mt-2 font-display text-xl font-bold leading-[1.15] tracking-[-0.02em] text-silver">{d.title}</h3>
                <p className="mt-3 font-display text-[13px] font-normal leading-[1.45] text-darkgray">{d.body}</p>
              </div>
            );
          })}
        </div>
        ...
        <div className="mt-12">
          <div className="mb-6 flex items-center gap-3">
            <span className="font-mono text-[10px] uppercase tracking-[0.3em] text-ozone">// UN SDG alignment</span>
            ...
            {GOALS.map((g) => (
              <div key={g.n} className="bg-card p-5">
                <div className="flex items-baseline gap-3">
                  <span className="font-display text-3xl font-black leading-none text-ozone">{g.n}</span>
                  <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-dim">SDG</span>
                </div>
                <h4 className="mt-3 font-display text-sm font-bold leading-[1.2] tracking-[-0.01em] text-silver">{g.title}</h4>
                <p className="mt-2 font-display text-[12px] font-normal leading-[1.4] text-darkgray">{g.body}</p>
              </div>
            ))}
          </div>
        </div>
