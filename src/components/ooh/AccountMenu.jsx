...
import { User as UserIcon, LogOut, LogIn, UserPlus, LayoutDashboard, Zap, Cpu, Settings, ChevronDown, ShieldCheck } from "lucide-react";
  ...
  if (!isAuthenticated || !user) {
    return (
      <div className="relative" ref={ref}>
        ...
        >
          <UserIcon className="h-4 w-4 md:h-3.5 md:w-3.5" /> <span className="hidden sm:inline">Log in</span>
        </button>
        ...
        {open && (
          <div className="absolute right-0 top-[calc(100%+6px)] z-50 w-64 border border-slate2 bg-void shadow-[0_8px_28px_rgba(0,0,0,0.5)]">
            <div className="border-b border-slate2/60 px-4 py-3">
              <div className="font-mono text-[10px] uppercase tracking-[0.2em] text-ozone">Guest access</div>
              <p className="mt-1.5 font-mono text-[10px] leading-relaxed text-dim">You're browsing the open map. Sign in to file offenses, earn rank, and shape the DAO.</p>
            </div>
            <div className="p-2">
              <Link
                to="/login"
    ...
    <div className="relative" ref={ref}>
      ...
      >
        <span className="flex h-7 w-7 items-center justify-center bg-ozone/10 font-mono text-[10px] font-bold text-ozone md:h-6 md:w-6">
          {user.avatar_url ? <img src={user.avatar_url} alt="" className="h-full w-full object-cover" /> : initials(user.full_name, user.email)}
        </span>
        ...
        <div className="absolute right-0 top-[calc(100%+6px)] z-50 w-60 border border-slate2 bg-void shadow-[0_8px_28px_rgba(0,0,0,0.5)]">
          <div className="border-b border-slate2/60 px-3 py-3">
            ...
            <div className="mt-1 flex items-center gap-2">
              <span className={`inline-flex items-center gap-1 border px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-[0.2em] ${ACCESS_BADGE[access] || ACCESS_BADGE.member}`}>
                {(access === "admin" || access === "moderator") && <ShieldCheck className="h-2.5 w-2.5" />}{access}
              </span>
              {isAgency && <span className="border border-ozone/40 px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-[0.2em] text-ozone">agency</span>}
            </div>
          </div>
          <nav className="py-1">
            {items.map((it) => {
