...
export default function QuickCapture({ open, onClose }) {
  ...
  const submit = async () => {
    ...
    const latN = parseFloat(lat),
      lngN = parseFloat(lng);
    if (!image_url) {
      setError('Capture a field photograph first.');
      return;
    }
          ...
          <>
            ...
            <div className="mt-4">
              <MultiPhotoUpload
                ...
                disabled={submitting}
                onRejected={(errors) => setError(errors[0])}
              />
            </div>
              ...
              <div className="grid grid-cols-4 gap-px border border-slate2/60 bg-slate2/40">
                {TYPES.map((t) => (
                  <button
                    ...
                    className={`py-2 font-mono text-[9px] font-bold uppercase tracking-[0.15em] transition-colors ${type === t.value ? 'bg-ozone text-void' : 'bg-card text-darkgray hover:text-silver'}`}
                  >
                    {t.label}
                  </button>
                ))}
            ...
            <input
              ...
              className="mt-4 w-full border border-slate2 bg-void px-4 py-3 font-display text-sm text-silver outline-none transition-colors placeholder:text-dim focus:border-ozone"
            />
            ...
            <div className="mt-3 flex items-center justify-between font-mono text-[10px] uppercase tracking-[0.2em] text-dim">
              ...
              <button onClick={locate} className="text-ozone transition-opacity hover:opacity-70">
                {locating ? '…' : 'Re-locate'}
              </button>
            </div>
            ...
            <button
              ...
              className="mt-5 flex w-full items-center justify-center gap-2 bg-ozone px-6 py-4 font-mono text-[11px] font-bold uppercase tracking-[0.3em] text-void transition-colors hover:bg-flare disabled:opacity-40"
            >
              {submitting ? (
