...
function Sparkline({ data, colorClass }) {
  ...
  return (
    <svg
      ...
      className={`h-7 w-full overflow-visible ${colorClass}`}
    >
      <defs>
...
export default function HeroConsole() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const ver =
          stats.verified != null && Number.isFinite(Number(stats.verified))
            ? Number(stats.verified)
            : verified;
        setD({
          spots,
          verified: ver,
          leads: Number.isFinite(Number(stats.leads))
            ? Number(stats.leads)
            : (leads || []).filter((x) => x.status === 'pending').length,
  ...
  const tickerItems = d.feed.length
    ? d.feed
    : [{ title: '// awaiting field input', created_date: null }];
  const cur = d.feed[fi];
    ...
    <div className="grid grid-cols-2 gap-2.5 md:col-span-6">
      ...
      <div className="col-span-2 flex items-center justify-between border border-border bg-card px-3 py-2 rounded-xl">
        <span className="flex items-center gap-1.5">
          ...
          <span className="font-mono text-[8px] uppercase tracking-[0.3em] text-ozone">
            // field telemetry
          </span>
        </span>
      ...
      <Link
        to="/map"
        className="group relative col-span-2 block overflow-hidden border border-ozone/40 bg-card p-4 rounded-xl transition-all duration-200 hover:-translate-y-0.5 hover:border-ozone hover:shadow-[0_14px_36px_-12px_rgba(0,0,0,0.6)] active:scale-[0.99]"
      >
      ...
      <Link
        to="/map"
        className="group col-span-2 flex items-center justify-between gap-2 border border-border bg-card px-3 py-2.5 rounded-xl transition-all duration-200 hover:-translate-y-0.5 hover:border-ozone/50 active:scale-[0.99]"
      >
        <div className="flex min-w-0 flex-1 items-center gap-2.5">
          <span className="h-6 w-0.5 shrink-0 bg-ozone transition-all duration-200 group-hover:h-7" />
          <div className="min-w-0 flex-1">
            <div className="font-mono text-[7px] uppercase tracking-[0.25em] text-muted-foreground">
              latest log
            </div>
      ...
      <div className="col-span-2 flex items-center justify-between gap-2 border border-border bg-card px-3 py-2 rounded-xl">
        <span className="font-mono text-[9px] leading-[1.3] text-muted-foreground">
          Union-made by veterans &amp; street artists.
        </span>
        <div className="flex gap-1.5">
          <button
            onClick={openCommand}
            className="border border-flare/60 px-3 py-2 font-mono text-[9px] font-bold uppercase tracking-[0.2em] text-flare transition-colors hover:bg-flare hover:text-void active:scale-[0.97]"
          >
            Command
          </button>
          <Link
            to="/campaign"
            className="border border-ozone/60 px-3 py-2 font-mono text-[9px] font-bold uppercase tracking-[0.2em] text-ozone transition-colors hover:bg-ozone hover:text-void active:scale-[0.97]"
          >
            Fund
          </Link>
        </div>
