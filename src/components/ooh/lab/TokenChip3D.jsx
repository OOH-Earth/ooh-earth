...
function makeChipFace(cfg) {
  ...
  ctx.lineWidth = 2;
  ctx.beginPath();
  ctx.arc(cx, cy, fieldR - 18, 0, Math.PI * 2);
  ctx.stroke();
  ...
  archedText(
    ...
    false,
    accent,
    "bold 46px 'Inter Tight', sans-serif",
  );
  archedText(
    ctx,
    cfg.bottomText,
    ...
    true,
    accent,
    "bold 40px 'Inter Tight', sans-serif",
  );
...
function makeChipEdge(ring, spot) {
  ...
  ctx.fillStyle = spot;
  for (let i = 0; i < 4; i++) {
    ctx.fillRect(i * 512 + 156, 0, 200, 64);
  }
  ctx.strokeStyle = 'rgba(0,0,0,0.3)';
  ctx.lineWidth = 2;
  for (let i = 0; i < 4; i++) {
    ctx.strokeRect(i * 512 + 156, 0, 200, 64);
  }
  const tex = new THREE.CanvasTexture(c);
...
const TokenChip3D = forwardRef(
  ...
  function TokenChip3D({ ring, spot, field, glyph, topText, bottomText, accent }, ref) {
    ...
    useEffect(() => {
      ...
      scene.add(fill);
      const rimL = new THREE.DirectionalLight(0xedff00, 0.45);
        ...
        <div className="pointer-events-none absolute inset-0">
          ...
          <div
            ...
            style={{
              ...
              animation: 'chip-scan 5s linear infinite',
            }}
          />
        </div>
