...
const PRINCIPLES = [
  { icon: Layers, label: "Open disclosure", note: "Full material BOM published — no hidden alloys." },
  { icon: Fingerprint, label: "Verifiable", note: "Material hash minted to the on-chain twin metadata." },
  ...
  return (
    <div className="border border-slate2 bg-card p-5">
      <div className="flex items-center gap-2">
        ...
        <div className="font-mono text-[11px] uppercase tracking-[0.2em] text-ozone">Construction · exploded view · real-to-spec</div>
      </div>
      <p className="mt-3 font-mono text-[11px] leading-relaxed text-silver/55">
        Four stacked layers, minted to a 64mm Ø × 4.5mm challenge-chip standard. The face is engraved — never printed — so the artifact outlives the platform. Materials disclosed openly, verifiable on-chain.
      </p>

      <div className="mt-4 overflow-x-auto">
        <svg viewBox="0 0 1000 430" className="w-full min-w-[640px]" role="img" aria-label="Exploded view of Genesis Chip construction layers">
          <defs>
          ...
          <line x1="150" y1="150" x2="870" y2="150" stroke="#3a3a3a" strokeWidth="1" strokeDasharray="3 5" opacity="0.5" />

          {LAYERS.map((L) => (
            <g key={L.title}>
              {/* disc */}
              <g filter="url(#softShadow)">
                <circle cx={L.cx} cy="150" r="92" fill={L.fill} stroke="rgba(0,0,0,0.35)" strokeWidth="1.5" />
                {L.rim && (
                  <>
                    <circle cx={L.cx} cy="150" r="84" fill="none" stroke="#7A5C0E" strokeWidth="2" strokeDasharray="3 3" opacity="0.7" />
                    <circle cx={L.cx} cy="150" r="58" fill="none" stroke="#7A5C0E" strokeWidth="2" opacity="0.5" />
                    <text x={L.cx} y="138" textAnchor="middle" fontSize="30" fontWeight="800" fill="#5A430A" fontFamily="Inter Tight, sans-serif">OOH</text>
                    <text x={L.cx} y="162" textAnchor="middle" fontSize="13" fontWeight="700" fill="#5A430A" letterSpacing="2" fontFamily="Inter Tight, sans-serif">EARTH</text>
                  </>
                )}
                {L.speckle && Array.from({ length: 26 }).map((_, i) => {
                  const a = (i / 26) * Math.PI * 2;
                  const rr = 30 + ((i * 7) % 55);
                  return <circle key={i} cx={L.cx + Math.cos(a) * rr} cy={150 + Math.sin(a) * rr} r={2 + (i % 3)} fill="#6E3410" opacity="0.55" />;
                })}
                {L.facets && Array.from({ length: 14 }).map((_, i) => {
                  const a1 = (i / 14) * Math.PI * 2;
                  const a2 = ((i + 1) / 14) * Math.PI * 2;
                  return <line key={i} x1={L.cx} y1="150" x2={L.cx + Math.cos(a2) * 92} y2={150 + Math.sin(a2) * 92} stroke="#7C7C7C" strokeWidth="0.7" opacity="0.5" />;
                })}
                {L.facets && <circle cx={L.cx} cy="150" r="34" fill="none" stroke="#6E6E6E" strokeWidth="0.8" opacity="0.6" />}
              </g>

              {/* leader line + label */}
              <line x1={L.cx} y1="244" x2={L.cx} y2="288" stroke="#555" strokeWidth="1" />
              <circle cx={L.cx} cy="288" r="2.5" fill="#EDFF00" />
              <text x={L.cx} y="312" textAnchor="middle" fontSize="15" fontWeight="700" fill="#F1F1F1" fontFamily="Inter Tight, sans-serif">{L.title}</text>
              <text x={L.cx} y="334" textAnchor="middle" fontSize="10.5" fill="#B2B2B2" fontFamily="Inter Tight, sans-serif">{L.sub[0]}</text>
              <text x={L.cx} y="350" textAnchor="middle" fontSize="10.5" fill="#B2B2B2" fontFamily="Inter Tight, sans-serif">{L.sub[1]}</text>
            </g>
          ))}
        </svg>
      </div>
      ...
      <div className="mt-5 border-t border-slate2/60 pt-4">
        <div className="font-mono text-[10px] uppercase tracking-[0.2em] text-ozone">Web7 material spec · open · verifiable · circular</div>
        <div className="mt-3 grid grid-cols-2 gap-2 md:grid-cols-3 lg:grid-cols-6">
          {SPEC.map((s) => (
            <div key={s.k} className="border border-slate2 bg-void/40 p-3">
              <div className="font-mono text-[9px] uppercase tracking-widest text-silver/40">{s.k}</div>
