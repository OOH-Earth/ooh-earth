...
function drawGuilloche(ctx, cx, cy, R, matColor) {
  ctx.save();
  ctx.strokeStyle = shade(matColor, 0.88);
  ctx.lineWidth = 0.6;
  ...
  ctx.strokeStyle = shade(matColor, 0.82);
  ctx.globalAlpha = 0.2;
  for (let i = 0; i < 128; i++) {
...
function makeObverseTexture(config, matColor, enamelHex) {
  ...
  ctx.strokeStyle = shade(matColor, 0.4);
  ctx.lineWidth = 6;
  for (let i = 0; i < 64; i++) {
  ...
  // Outer ring
  ctx.strokeStyle = shade(matColor, 0.5);
  ctx.lineWidth = 8;
  ctx.beginPath();
  ctx.arc(cx, cy, R - 76, 0, Math.PI * 2);
  ctx.stroke();
  ...
  if (enamelHex) {
    ...
    ctx.strokeStyle = enamelHex;
    ctx.lineWidth = 4;
    ctx.globalAlpha = 0.85;
    ctx.beginPath();
    ctx.arc(cx, cy, R - 190, 0, Math.PI * 2);
    ctx.stroke();
    ctx.globalAlpha = 1;
  }
  ...
  ctx.strokeStyle = shade(matColor, 0.45);
  ctx.lineWidth = 5;
  ctx.beginPath();
  ctx.arc(cx, cy, R - 195, 0, Math.PI * 2);
  ctx.stroke();
...
function makeObverseBump(config) {
  ...
  ctx.fillRect(0, 0, 1024, 1024); // flat base
...
function makeReverseTexture(num, matColor, enamelHex) {
  ...
  ctx.strokeStyle = shade(matColor, 0.28);
  ctx.lineWidth = 3.5;
  edges.forEach((e) => {
    ctx.beginPath();
...
function makeEdgeTexture(matColor, edgeType) {
  ...
  if (edgeType === 'reeded' || edgeType === 'lettered') {
    ctx.strokeStyle = shade(matColor, 0.38);
    ctx.lineWidth = 2;
    for (let i = 0; i < 256; i++) {
      ctx.beginPath();
      ctx.moveTo(i * 8, 0);
  ...
  if (edgeType === 'rope') {
    ...
    ctx.strokeStyle = shade(matColor, 1.35);
    ctx.lineWidth = 2.5;
    for (let i = 0; i < 128; i++) {
      ctx.beginPath();
      ctx.moveTo(i * 16 + 4, 6);
      ctx.lineTo(i * 16 + 12, 26);
      ctx.stroke();
    }
  }
...
const CoinViewer3D = forwardRef(
  ...
  function CoinViewer3D({ config, materialId, edgeType, enamelId }, ref) {
    ...
    useEffect(() => {
      ...
      scene.add(key);
      const fill = new THREE.DirectionalLight(0x6fd6ff, 0.55);
      fill.position.set(-5, -1, 3);
      scene.add(fill);
      const rim = new THREE.DirectionalLight(0xedff00, 0.5);
      ...
      sG.addColorStop(0.4, 'rgba(0,0,0,0.12)');
    ...
    useEffect(() => {
      ...
      if (faceTop?.material) {
        ...
        faceTop.material.map = obvTex;
        faceTop.material.bumpMap = obvBump;
      ...
      if (faceBot?.material) {
        ...
        faceBot.material.map = revTex;
        faceBot.material.bumpMap = revBump;
        ...
        <div className="pointer-events-none absolute inset-0">
          ...
          <div
            ...
            style={{
              ...
              animation: 'coin-scan 5s linear infinite',
            }}
          />
        </div>
