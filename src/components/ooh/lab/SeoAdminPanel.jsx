...
import { Loader2, ImageIcon, Sparkles, Search, Eye, EyeOff, ExternalLink, CheckCircle2 } from "lucide-react";
...
function allRoutes() {
  const statics = Object.keys(META).map((p) => ({ path: p, label: META[p].title.split(" — ")[0], dynamic: false }));
...
export default function SeoAdminPanel() {
  ...
  const setDraft = (path, patch) => setDrafts((p) => ({ ...p, [path]: { ...draft(path), ...patch } }));
  ...
  const ogFor = (path) => records?.[path]?.og_image || (META[path] && OG_IMAGES[ogKey(path)]) || OG_IMAGES.default;

  // Map route path to the OG_IMAGES category key used in routeMeta
  function ogKey(path) {
    if (path.startsWith("/lab")) return "lab";
    if (path.startsWith("/store") || path.startsWith("/zora") || path === "/lab/nft") return "store";
    if (path.startsWith("/map") || path.startsWith("/location") || path.startsWith("/bus") || path.startsWith("/categories") || path.startsWith("/category") || path.startsWith("/regions") || path === "/report" || path === "/ar") return "map";
    if (path.startsWith("/campaign") || path.startsWith("/capital") || path.startsWith("/investor") || path.startsWith("/portal") || path === "/console" || path === "/portfolio") return "campaign";
    if (path.startsWith("/adbusting") || path.startsWith("/ecology") || path.startsWith("/rivers") || path.startsWith("/warzones") || path === "/inhome") return "adbusting";
          ...
          return (
            <div key={route.path} className="px-4 py-3.5">
              <div className="flex items-center gap-2">
                ...
                <a href={route.path} target="_blank" rel="noreferrer" className="ml-auto text-silver/30 transition-colors hover:text-ozone"><ExternalLink className="h-3 w-3" /></a>
                <span className="font-mono text-[9px] tracking-[0.1em] text-silver/35">{route.path}</span>
              </div>
              ...
              <div className="mt-2.5 flex flex-col gap-3 md:flex-row">
                ...
                <div className="shrink-0">
                  <div className="relative h-[78px] w-[148px] overflow-hidden border border-slate2/50 bg-void">
                    {og ? <img src={og} alt="" className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center"><ImageIcon className="h-5 w-5 text-slate2" /></div>}
                    {rec?.og_generated && <span className="absolute right-1 top-1 border border-ozone/50 bg-void/80 px-1 py-0.5 font-mono text-[6px] uppercase tracking-[0.1em] text-ozone">auto</span>}
                  </div>
                  <button
                  ...
                  >
                    {genPath === route.path ? <Loader2 className="h-3 w-3 animate-spin" /> : <Sparkles className="h-3 w-3" />}
                ...
                <div className="flex-1 space-y-2">
                  <div>
                    <label className="font-mono text-[8px] uppercase tracking-[0.2em] text-silver/45">Title</label>
                    <input
                      value={d.title}
                  ...
                  <div>
                    <label className="font-mono text-[8px] uppercase tracking-[0.2em] text-silver/45">Description</label>
                    <textarea
                      value={d.description}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    >
                      {savingPath === route.path ? <Loader2 className="h-3 w-3 animate-spin" /> : <CheckCircle2 className="h-3 w-3" />}
                      Save
                    </button>
                  </div>
                </div>
