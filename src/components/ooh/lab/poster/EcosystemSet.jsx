    ...
    <>
      <section className="border border-slate2 bg-card p-6">
        <div className="font-mono text-[11px] uppercase tracking-[0.2em] text-ozone">
          OOH Earth ecosystem integration
        </div>
            ...
            <MiniPhone label="Map">
              <div className="relative h-24 border border-slate2/50 bg-void grid-bg">
                {[
                  ...
                  <div
                    key={i}
                    className="absolute h-1.5 w-1.5 -translate-x-1/2 -translate-y-1/2 bg-flare"
                    style={{ left: `${x}%`, top: `${y}%` }}
                  />
                ))}
              ...
              <div className="mt-2 flex justify-center">
                <div
                  className="h-8 w-8 rounded-full border-2 border-ozone/60"
                  style={{ boxShadow: '0 0 12px rgba(237,255,0,.2)' }}
                />
              </div>
            </MiniPhone>
            <MiniPhone label="DAO">
              <div className="border border-slate2/50 bg-void p-2">
                ...
                <div className="mt-2 bg-ozone py-1.5 text-center font-mono text-[8px] font-bold uppercase tracking-wide text-void">
                  Vote
                </div>
              </div>
              <div className="mt-2 flex justify-between font-mono text-[7px] text-silver/40">
                {['Vote', 'Prop', 'Gov', 'Hist'].map((s) => (
                  <span key={s}>{s}</span>
                ))}
              </div>
            </MiniPhone>
            <MiniPhone label="Wallet">
              <div className="border border-slate2/50 bg-void p-2 text-center">
                ...
                <div className="mt-2 grid grid-cols-3 gap-1">
                  {[0, 1, 2, 3, 4, 5].map((i) => (
                    <div key={i} className="aspect-square border border-slate2/50 bg-card" />
                  ))}
                </div>
              </div>
              <div className="mt-2 flex justify-between font-mono text-[7px] text-silver/40">
                {['Wal', 'Act', 'Col', 'Set'].map((s) => (
                  <span key={s}>{s}</span>
                ))}
              </div>
            </MiniPhone>
