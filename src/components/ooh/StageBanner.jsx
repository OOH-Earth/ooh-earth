...
export default function StageBanner() {
  useEffect(() => {
    ...
    if (titleEl) { obs = new MutationObserver(stamp); obs.observe(titleEl, { childList: true }); }
  ...
  return (
    <>
      ...
      <div aria-hidden className="pointer-events-none fixed inset-0 z-[9998] border-2 border-[#FF0040]/45" />
      ...
      <div
        ...
        style={{ background: "repeating-linear-gradient(45deg, rgba(255,0,64,0.28) 0 8px, rgba(10,10,10,0.94) 8px 16px)" }}
      >
        <AlertTriangle className="h-3 w-3 text-[#FF0040]" />
        <span className="font-mono text-[9px] font-bold uppercase tracking-[0.3em] text-[#FF0040]">Stage · not production</span>
      </div>
    </>
  );
}
