...
function Stat({ icon: Icon, value, label, accent }) {
  return (
    <div className="flex flex-col items-center gap-2 px-4 py-6 text-center">
      ...
      <div className="font-display text-4xl font-black leading-none tracking-[-0.02em] text-silver md:text-5xl tabular">
        {value}
      </div>
      ...
      <div className="px-5 py-16 md:px-8 md:py-24">
        <div className="flex flex-col gap-4 border-b border-slate2/40 pb-6 md:flex-row md:items-end md:justify-between">
          ...
          <p className="max-w-sm font-display text-sm font-normal leading-[1.4] text-darkgray">
            Every report filed is a person who stopped walking past. This is the live count of
            members, active cities, and documented interventions — the network grows with every
            submission.
          </p>
        </div>
        ...
        <p className="mt-2 max-w-2xl font-mono text-[10px] leading-relaxed tracking-[0.1em] text-dim">
          OOH Earth is a day-one platform — early access, seeking founding backers. The counts above
          are ours and honest. The wider resistance is not new: since ~{MOVEMENT.since} the global
          subvertising movement has grown to an estimated{' '}
          <span className="text-flare">~{fmtK(MOVEMENT.subvertisers)}+ subverters</span> across{' '}
          <span className="text-flare">{MOVEMENT.collectives}+ collectives</span> in{' '}
          <span className="text-flare">{MOVEMENT.countries}+ countries</span> (est.). We&rsquo;re
          here to put it on one map.
        </p>
      </div>
    </section>
  );
