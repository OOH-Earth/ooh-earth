...
const CONSOLE_TAB = { to: "/dashboard", icon: LayoutDashboard, label: "Console", notify: true };
...
export default function MobileBottomTabs() {
  ...
  const allTabs = [...TABS, ...CAMERAS, CONSOLE_TAB];
  const activeIndex = allTabs.findIndex((t) =>
    t.to === "/" ? current === "/" : current.startsWith(t.to)
  );
  const active = activeIndex >= 0 ? activeIndex : 0;

  const renderCamera = ({ to, icon: Icon, label, accent }, idx) => {
    const i = TABS.length + idx;
  ...
  const renderTab = ({ to, icon: Icon, label, primary, notify }, i) => {
    const isActive = i === active;
  ...
  return (
    ...
    >
      <div className="ooh-mobile-nav flex items-center gap-1 rounded-full border border-border bg-card px-2 py-2 shadow-[0_8px_32px_rgba(0,0,0,0.5)] backdrop-blur-xl">
        {/* Home → Map → Report → Channel */}
        {TABS.map((t, i) => renderTab(t, i))}
        {/* Divider */}
        <span className="h-7 w-px bg-border" />
        {/* Camera shortcuts */}
        {CAMERAS.map((c, idx) => renderCamera(c, idx))}
        {/* Divider */}
        <span className="h-7 w-px bg-border" />
        {/* Console */}
        {renderTab(CONSOLE_TAB, allTabs.length - 1)}
      </div>
    </nav>
  );
}