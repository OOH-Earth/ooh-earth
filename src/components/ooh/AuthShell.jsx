...
export const INPUT =
  'w-full border border-slate2 bg-void py-3 pl-10 pr-3 font-mono text-sm text-silver outline-none transition-colors placeholder:text-darkgray focus:border-ozone';
    ...
    <div className="min-h-screen bg-void grid-bg text-silver">
      <header className="flex items-center justify-between border-b border-slate2 px-6 py-4">
        <div className="flex items-center gap-2.5">
          <Link
            to="/"
            className="flex items-center gap-2.5 font-brand text-xl tracking-tight text-silver transition-colors hover:text-ozone"
          >
        ...
        <Link
          to="/map"
          className="flex items-center gap-1.5 font-mono text-[10px] uppercase tracking-[0.2em] text-dim transition-colors hover:text-ozone"
        >
      ...
      <div className="mx-auto grid max-w-6xl grid-cols-1 lg:grid-cols-2">
        ...
        <div className="flex flex-col justify-center gap-9 px-6 py-12 lg:border-r lg:border-slate2 lg:px-12 lg:py-20">
          <div>
            <div className="font-mono text-[11px] uppercase tracking-[0.3em] text-ozone">
              Enter the network
            </div>
            <h1 className="mt-4 text-3xl font-bold uppercase leading-[1.05] tracking-tight md:text-4xl">
              Reclaim the
              <br />
              <span className="text-ozone">visual commons.</span>
            </h1>
            <p className="mt-4 max-w-md font-mono text-xs leading-loose text-silver/60">
              Map corporate advertising in public space, put every offense on the record, and
              coordinate the creative response — one city at a time. Community-funded, copyleft, and
              not for sale.
            </p>
          </div>
              ...
              {TIERS.map((t, i) => (
                <div key={t.label} className="flex items-center gap-1">
                  <div className="border border-slate2 px-2 py-1.5 text-center">
                    <div className="font-mono text-[10px] uppercase tracking-wide text-silver/70">
                      {t.label}
                    </div>
          ...
          <div className="flex flex-wrap gap-2">
            {TRUST.map((t) => (
              <span
                key={t}
                className="border border-slate2/60 px-2 py-1 font-mono text-[9px] uppercase tracking-[0.15em] text-dim"
              >
                {t}
              </span>
            ))}
