...
const STACK_IMG =
  'https://base44.app/api/apps/6a62213cff3ccbca88c04ff5/files/mp/public/6a62213cff3ccbca88c04ff5/0c368a455_1777667192-01-4t5x.webp';
...
function Ring({ pct, label, color }) {
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      <span className="font-mono text-[8px] uppercase tracking-[0.15em] text-primary-foreground/60">
        {label}
      </span>
    </div>
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const d = res.data || {};
        if (active)
          setS({ reports: d.reports || 0, verified: d.verified || 0, raised: d.raised || 0 });
      } catch {}
    })();
    return () => {
      active = false;
      ...
      <div className="mx-auto max-w-7xl px-4 py-20 md:px-8 md:py-24">
        <div className="mb-10 max-w-2xl">
          ...
          <p className="mt-3 font-display text-sm leading-relaxed text-muted-foreground">
            OOH Street Maps connects mappers with local activists, surfaces patterns in corporate
            advertising, and alerts communities to new threats — from planning applications to
        ...
        <div className="grid gap-4 [&>article]:min-w-0 md:grid-cols-2 md:gap-5">
          ...
          <article className="group flex flex-col gap-5 rounded-xl border border-border bg-card p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-ozone/50 hover:shadow-[0_18px_44px_-18px_rgba(0,0,0,0.65)] active:scale-[0.99]">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h3 className="font-display text-lg font-bold tracking-[-0.01em] text-foreground">
                  9 Offense Categories
                </h3>
                <p className="mt-1 font-display text-[13px] leading-relaxed text-muted-foreground">
                  Every ad tells a story the advertiser doesn't want you to read — from
                  psychological manipulation to cultural erasure.
                </p>
              </div>
            ...
            <div className="rounded-lg border border-border bg-foreground/5 p-4">
              <div className="flex items-center gap-2">
                ...
                <span className="font-display text-sm font-bold text-foreground">
                  28 Day Streak Active
                </span>
              </div>
              <p className="mt-1 font-display text-[12px] text-muted-foreground">
                Build honest streaks that reflect real commitment.
              </p>
              <div className="mt-3 flex items-center justify-between">
                {WEEK.map((d, i) => (
                  <div
                    key={i}
                    className="relative flex h-8 w-8 items-center justify-center rounded-full border border-border bg-card font-mono text-[9px] font-bold text-muted-foreground"
                  >
                    {d}
                    {DONE[i] && (
            ...
            <div className="mt-auto flex flex-wrap gap-2">
              {PILLS.map((p) => (
                <span
                  key={p.t}
                  className="inline-flex items-center gap-1.5 rounded-full border border-border bg-background px-3 py-1.5 font-mono text-[9px] font-bold uppercase tracking-[0.1em] text-muted-foreground"
                >
          ...
          <article className="group metro-invert flex flex-col gap-5 rounded-xl border border-border bg-primary p-6 text-primary-foreground shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-ozone/40 hover:shadow-[0_18px_44px_-18px_rgba(237,255,0,0.22)] active:scale-[0.99]">
            <div>
              ...
              <p className="mt-1 font-display text-[13px] leading-relaxed text-primary-foreground/60">
                A simple view that shows only the field actions active right now across documented
                cities.
              </p>
            </div>
            ...
            <div className="metro-sub rounded-lg border border-primary-foreground/10 bg-primary-foreground/5 p-4">
              <div className="flex items-center justify-between border-b border-primary-foreground/10 pb-2">
                ...
                <span className="font-mono text-[10px] tabular text-primary-foreground/80">
                  {billboardsPct}% Completion
                </span>
              </div>
              <div className="mt-2 space-y-1.5">
                {CITY_ROWS.map((r, i) => (
                  <div
                    key={i}
                    className="metro-row flex items-center gap-3 rounded-md bg-primary-foreground/5 px-2.5 py-1.5"
                  >
                    <span
                      className={`h-1.5 w-1.5 shrink-0 rounded-full ${r.on ? 'bg-[#39FF14]' : 'bg-primary-foreground/30'}`}
                    />
                    <span className="font-mono text-[9px] tabular text-primary-foreground/50">
                      {r.t}
                    </span>
                    <span className="min-w-0 flex-1 truncate font-display text-[12px] font-medium">
                      {r.s}
                    </span>
            ...
            <Link
              to="/map"
              className="mt-auto inline-flex w-fit items-center gap-1.5 rounded-sm font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-primary-foreground/90 transition-colors hover:text-primary-foreground/70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-foreground/60 focus-visible:ring-offset-2 focus-visible:ring-offset-primary"
            >
          ...
          <article className="group relative flex min-h-[260px] flex-col justify-between overflow-hidden rounded-xl border border-border p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-ozone/50 hover:shadow-[0_18px_44px_-18px_rgba(0,0,0,0.65)] active:scale-[0.99]">
            <Image
              src={STACK_IMG}
              alt="Documented street-art response over a corporate billboard"
              loading="lazy"
            ...
            <div className="relative">
              <h3 className="font-display text-lg font-bold tracking-[-0.01em] text-white">
                Community Action Stacks
              </h3>
              <p className="mt-1 max-w-xs font-display text-[13px] leading-relaxed text-white/70">
                Group field actions into simple blocks so your day feels organized instead of
                scattered.
              </p>
            </div>
            <Link
          ...
          <article className="group metro-invert flex flex-col gap-5 rounded-xl border border-border bg-primary p-6 text-primary-foreground shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-ozone/40 hover:shadow-[0_18px_44px_-18px_rgba(237,255,0,0.22)] active:scale-[0.99]">
            ...
            <div className="metro-sub rounded-lg border border-primary-foreground/10 bg-primary-foreground/5 p-5">
              <div className="mb-4 font-mono text-[9px] uppercase tracking-[0.2em] text-primary-foreground/70">
            ...
            <div className="mt-auto flex items-center gap-2 font-mono text-[9px] uppercase tracking-[0.2em] text-primary-foreground/50">
              <MapPin className="h-3 w-3 text-primary-foreground/70" /> Live telemetry · updates
              hourly
            </div>
          </article>
        </div>
