      ...
      <div className="absolute right-0 top-0 flex h-full w-full max-w-[420px] flex-col border-l border-slate2/70 bg-void shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate2/60 px-5 py-4">
          <div className="flex items-center gap-2">
            ...
            <span className="font-display text-sm font-black uppercase tracking-[0.3em] text-silver">AI Unit Finder</span>
          </div>
          <button onClick={onClose} className="text-dim transition-colors hover:text-ozone" aria-label="Close">
            <X className="h-5 w-5" />
          </button>
        </div>

        <div className="border-b border-slate2/60 px-5 py-4">
          <p className="mb-2 font-mono text-[10px] uppercase tracking-[0.2em] text-dim">// enter a city or area to scan for OOH units</p>
          ...
          {error && <p className="mt-2 font-mono text-[10px] uppercase tracking-[0.15em] text-flare">{error}</p>}
        </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          {loading ? (
            <div className="flex flex-col items-center gap-3 py-16">
              ...
              <span className="font-mono text-[10px] uppercase tracking-[0.3em] text-dim">// scanning field…</span>
            </div>
          ) : candidates.length ? (
            <div className="px-5 py-3">
              ...
              {candidates.map((c) => {
                ...
                return (
                  <div key={c.id} className={`mb-2 border border-slate2/70 bg-card px-3 py-2.5 ${!c.located ? "opacity-50" : ""}`}>
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <div className="font-display text-sm font-bold text-silver">{c.name}</div>
                        <div className="mt-0.5 flex items-center gap-2">
                          <span className="font-mono text-[9px] uppercase tracking-[0.2em] text-ozone">{TYPE_LABEL[c.type] || c.type || "Other"}</span>
                          <span className="font-mono text-[9px] uppercase tracking-[0.2em] text-dim">{c.located ? `${c.lat.toFixed(3)}, ${c.lng.toFixed(3)}` : "no coords"}</span>
                        </div>
                      </div>
                      <button
                        onClick={() => addOne(c)}
                        ...
                        className={`flex shrink-0 items-center gap-1 border px-2 py-1 font-mono text-[9px] font-bold uppercase tracking-[0.15em] transition-colors ${
                          added ? "border-[#39FF14] text-[#39FF14]" : "border-slate2 text-silver hover:border-ozone hover:text-ozone disabled:opacity-30"
                        }`}
                      >
                    ...
                    <div className="mt-1 flex items-start gap-1 font-mono text-[10px] text-darkgray">
                      ...
                      <span>{c.address}{c.notes ? ` · ${c.notes}` : ""}</span>
                    </div>
                  </div>
                );
              })}
        ...
        <div className="border-t border-slate2/60 px-5 py-3">
          <p className="font-mono text-[9px] uppercase tracking-[0.2em] text-dim">Added units log as PENDING · realtime map sync</p>
        </div>
      </div>
    </div>
  );
