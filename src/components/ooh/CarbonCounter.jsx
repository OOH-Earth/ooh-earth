...
function Stat({ icon: Icon, value, label, sub, accent }) {
  return (
    <div className="border border-slate2/60 p-5 md:p-7">
      ...
      <div className="mt-4 font-display text-4xl font-black leading-none tracking-[-0.02em] text-silver md:text-5xl tabular">
        {value}
      </div>
      <div className="mt-2 font-mono text-[10px] uppercase tracking-[0.2em] text-darkgray">
        {label}
      </div>
      {sub && (
        <div className="mt-1 font-mono text-[9px] uppercase tracking-[0.15em] text-dim">{sub}</div>
      )}
    </div>
  );
}
      ...
      <div className="px-5 py-16 md:px-8 md:py-24">
        <div className="flex flex-col gap-4 border-b border-slate2/40 pb-6 md:flex-row md:items-end md:justify-between">
          ...
          <p className="max-w-sm font-display text-sm font-normal leading-[1.4] text-darkgray">
            Digital billboards run 24/7 — each one burns thousands of kWh a year and exhales tonnes
            of CO₂. We tally the documented screens in the atlas against the grid average. This is
            the invoice the advertisers never send.
          </p>
        </div>
        ...
        {!data ? (
          ...
          <div className="mt-10 grid grid-cols-2 gap-px bg-slate2/40 lg:grid-cols-4">
            <Stat
              icon={Zap}
              value={data.count}
              label="Digital screens logged"
              sub="in field atlas"
              accent="text-ozone"
            />
            <Stat
              icon={Zap}
              value={`${fmt(data.kwh / 1000)}k`}
              label="kWh / year"
              sub="est. @ 5,200 kWh/screen"
              accent="text-ozone"
            />
            <Stat
              icon={Cloud}
              value={`${fmt(data.co2kg / 1000)}t`}
              label="CO₂ / year"
              sub="est. @ 0.42 kg/kWh"
              accent="text-flare"
            />
            <Stat
              icon={TreePine}
              value={fmt(data.trees)}
              label="Trees to offset"
              sub="@ 21 kg CO₂/yr per tree"
              accent="text-flare"
            />
          </div>
        )}
        ...
        <p className="mt-6 font-mono text-[10px] uppercase tracking-[0.25em] text-dim">
          // Estimates derived from documented digital screens only · grid-carbon factor 0.42 kg
          CO₂/kWh (IEA global avg)
        </p>
      </div>
    </section>
  );
