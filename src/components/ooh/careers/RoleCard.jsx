  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          <span
            className={`mb-2 inline-flex items-center gap-1.5 border px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-[0.2em] ${st.cls}`}
          >
            <span className={`h-1 w-1 ${st.dot}`} /> {st.label}
          </span>
          <h3 className="font-display text-lg font-semibold tracking-[-0.02em] text-silver transition-colors group-hover:text-ozone">
            {role.title}
          </h3>
        ...
        <span
          className={`shrink-0 border px-2 py-0.5 font-mono text-[9px] uppercase tracking-[0.2em] ${TYPE_STYLES[role.type] || TYPE_STYLES['Part-time']}`}
        >
          {role.type}
        </span>
      </div>
