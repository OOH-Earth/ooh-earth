  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      out.push({
        divider: true,
        tone: 'flare',
        label: `MOVEMENT · EST · SINCE ${MOVEMENT.since}`,
        title: MOVEMENT_NOTE,
      });
      out.push({
        icon: Users,
        label: 'SUBVERTISERS',
        value: `~${fmtK(MOVEMENT.subvertisers)}+`,
        est: true,
      });
      out.push({
        icon: Zap,
        label: 'INTERVENTIONS',
        value: `~${fmtK(MOVEMENT.interventions)}+`,
        est: true,
      });
      ...
      try {
        ...
        if (s.points != null)
          out.push({
            icon: Zap,
            label: 'RESISTANCE',
            value: `${Number(s.points).toLocaleString()} PTS`,
            accent: 'text-ozone',
          });
        if (s.reports != null) out.push({ icon: MapPin, label: 'LOCATIONS', value: s.reports });
        if (s.digital_busts != null)
          out.push({
            icon: Monitor,
            label: 'DIGITAL',
            value: s.digital_busts,
            accent: 'text-ozone',
          });
        if (s.verified != null)
          out.push({
        ...
        if (s.cities != null)
          out.push({ icon: Building2, label: 'CITIES', value: s.cities, accent: 'text-flare' });
      } catch (e) {
      ...
      try {
        ...
        const coin = (id, sym, dp) => {
          ...
          return {
            icon: DollarSign,
            label: sym,
            value: `$${Number(c.usd).toLocaleString(undefined, { maximumFractionDigits: dp })}`,
            delta: `${Math.abs(chg).toFixed(1)}%`,
            up: chg >= 0,
          };
        };
        [coin('bitcoin', 'BTC', 0), coin('ethereum', 'ETH', 0), coin('solana', 'SOL', 2)].forEach(
          (c) => {
            if (c) out.push(c);
          },
        );
    ...
    <div className="pointer-events-none absolute inset-x-0 bottom-0 z-[950] border-t border-border bg-card/95 backdrop-blur-md shadow-[0_-6px_20px_rgba(0,0,0,0.10)]">
      <div className="relative flex h-9 items-center overflow-hidden">
        <div className="relative z-30 flex h-full shrink-0 items-center gap-2 border-r border-border bg-background px-3">
          ...
          <span className="font-mono text-[9px] font-bold uppercase tracking-[0.25em] text-[#FF0033]">
            Live
          </span>
        ...
        {items.length ? (
          ...
          <span className="px-4 font-mono text-[9px] uppercase tracking-[0.25em] text-dim">
      ...
      {items.map((it, i) =>
        it.divider ? (
          <span
            key={i}
            title={it.title}
            className="flex shrink-0 items-center gap-1.5 border-l border-r border-slate2/40 bg-background/60 px-3"
          >
            <span
              className={`h-1.5 w-1.5 rounded-full ${it.tone === 'flare' ? 'bg-flare' : 'bg-ozone'} ${it.pulse ? 'animate-blink' : ''}`}
            />
            <span
              className={`font-mono text-[9px] font-bold uppercase tracking-[0.25em] ${it.tone === 'flare' ? 'text-flare' : 'text-ozone'}`}
            >
              {it.label}
            </span>
          </span>
          ...
          <span key={i} className="flex shrink-0 items-center gap-1.5 px-4">
            ...
            <span className="font-mono text-[9px] font-bold uppercase tracking-[0.2em] text-darkgray">
              {it.label}
            </span>
            <span
              className={`font-mono text-[10px] tabular ${it.accent || (it.est ? 'text-flare' : 'text-silver')}`}
            >
              {it.value}
            </span>
            {it.est && (
