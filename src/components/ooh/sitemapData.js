...
export const SITEMAP_GROUPS = [
  {
    ...
    routes: [
      { path: "/", name: "Home", vis: "public", status: "live", ux: "Cinematic hero, live MetroKit telemetry grid, 3D globe, and the full narrative stack — mandate, atlas, offenders, members, manifesto, on-chain. The front door; dispatches visitors to map, report, or fund.", audience: "All visitors", auth: "none", flows: ["/map", "/report", "/campaign", "/portfolio"] },
  ...
  },
  {
    group: "Field Ops",
    accent: "text-ozone",
    desc: "Browse openly; contributing data requires a member session.",
    routes: [
      { path: "/report", name: "Field Report", vis: "public", status: "live", ux: "File a new advertising-offense report: location, type, photo, access key, notes. Saves to Location entity as pending.", audience: "Members", auth: "required", flows: ["/dashboard", "/map"] },
      { path: "/ar", name: "AR Lens", vis: "public", status: "testing", ux: "AR overlay tool — visualize adbusts in situ via camera. Field-readiness gated.", audience: "Members", auth: "optional", flows: ["/report"] },
      { path: "/scan", name: "TrueCost", vis: "public", status: "testing", ux: "UPC barcode scan → product impact / true-cost analysis. Camera requires HTTPS publish; blocked in preview iframe.", audience: "Members", auth: "optional", flows: ["/report"] },
      { path: "/trash", name: "Trash ID", vis: "public", status: "testing", ux: "Waste traceability — scan/ID waste to map corporate polluters. Camera requires HTTPS publish.", audience: "Members", auth: "optional", flows: ["/report"] },
      { path: "/inhome", name: "In-Home", vis: "public", status: "live", ux: "In-home / digital-surface busting tools — screen grids and digital scenes.", audience: "Members", auth: "optional", flows: ["/report"] },
      { path: "/zora", name: "Zora Mint", vis: "public", status: "testing", ux: "Location-to-NFT minting pipeline (non-custodial, client-side metadata builder). Mints route creator fees to the creator's wallet.", audience: "Members", auth: "required", flows: ["/portfolio", "/location/:id"] },
      { path: "/field-id", name: "Field ID", vis: "public", status: "live", ux: "Member field ID card generator (printable).", audience: "Members", auth: "optional", flows: ["/dashboard"] },
      { path: "/card", name: "Union Card", vis: "public", status: "live", ux: "Digital union membership / credential card.", audience: "Members", auth: "optional", flows: ["/dashboard"] },
      { path: "/access-keys", name: "Access Keys", vis: "public", status: "live", ux: "Open-access key registry — the standard tools that unlock transit-shelter and ad-space housings (H60, T-Handle, Circle, JCD Superlock, 4-Way Utility and more), each with a map of the bus-stop locations it likely fits. Keys are unconfirmed until a field check.", audience: "Members", auth: "none", flows: ["/bus-stops", "/access-keys/:slug"] },
      { path: "fn › objection", name: "Objection Generator", vis: "public", status: "building", ux: "Generates a formal, citable planning-consent objection for a specific billboard — offense grounds + precedent baked in. Feature exists in the field flow; a standalone route is not built yet.", audience: "Members", auth: "optional", flows: ["/report"], planned: true },
      { path: "lib › precedent", name: "Precedent Library", vis: "internal", status: "exploring", ux: "Citable legal precedent (A/69/286, Les Déboulonneurs 2013, First Things First) surfaced behind objections. Scoping only.", audience: "Members", auth: "none", flows: [], planned: true },
    ],
  },
  {
    group: "Operate · Your Account",
    accent: "text-ozone",
    desc: "Progression and personal console for a signed-in member.",
    routes: [
      { path: "/operative", name: "Member Profile", vis: "public", status: "live", ux: "Member identity, tier progression (Scout → Field Reporter → City Ambassador), and personal field record.", audience: "Members", auth: "optional", flows: ["/dashboard", "/field-id"] },
      { path: "/dashboard", name: "Member Console", vis: "public", status: "live", ux: "Your console — your field captures, a verification queue if you're a verifier, an access-aware badge, and account controls. Realtime-synced via Location subscription.", audience: "Members / Admin", auth: "protected", flows: ["/report", "/map", "/fde"] },
      { path: "/guides", name: "Guides", vis: "public", status: "live", ux: "Status matrix + journey steps — onboarding guides for new members.", audience: "Members", auth: "none", flows: ["/report", "/dashboard"] },
      { path: "feat › streaks", name: "Streaks & Nudges", vis: "internal", status: "exploring", ux: "Retention loop — capture streaks and next-best-action nudges toward the next tier. Not built.", audience: "Members", auth: "optional", flows: ["/operative"], planned: true },
    ],
  },
  {
    group: "Fund & Store",
  ...
  },
  {
    ...
    routes: [
      { path: "/login", name: "Login", vis: "public", status: "live", ux: "Email + password and Google OAuth. Hard-redirects home on success.", audience: "Members", auth: "none", flows: ["/"] },
      { path: "/register", name: "Register", vis: "public", status: "live", ux: "Email + password + confirm, Google, then OTP verification → token → hard redirect home.", audience: "Members", auth: "none", flows: ["/"] },
      { path: "/forgot-password", name: "Forgot Password", vis: "public", status: "live", ux: "Email → reset link. Always shows generic success (API hides whether email exists).", audience: "Members", auth: "none", flows: ["/reset-password"] },
      { path: "/reset-password", name: "Reset Password", vis: "public", status: "live", ux: "Reads ?token= + new password + confirm → reset → redirect to login.", audience: "Members", auth: "none", flows: ["/login"] },
    ],
  },
  {
  ...
  },
  {
    ...
    routes: [
      ...
      { path: "/fde", name: "FDE Portal", vis: "internal", status: "live", ux: "0101001 forward-deployed engineer portal: active deployments, triage preview, reporting tools, member roster.", audience: "Engineers / Admin", auth: "protected", flows: ["/dashboard", "/report", "/portfolio"] },
      ...
      { path: "/radio-ops", name: "Radio Ops", vis: "internal", status: "live", ux: "Field-radio operations surface — ambient comms / soundscape ops for the crew.", audience: "Members / Admin", auth: "protected", flows: ["/dashboard"] },
  ...
  },
  {
    ...
    routes: [
      ...
      { path: "fn › moderate", name: "Moderation Service", vis: "internal", status: "live", ux: "Server-gated function that is the only elevated path for the queue: members read incoming, moderators + admins approve/reject. Entity RLS stays admin-only.", audience: "Moderators / Admin", auth: "protected", flows: ["/dashboard"] },
  ...
  },
  {
    ...
    routes: [
      { path: "/journey", name: "Journey Map", vis: "public", status: "live", ux: "Full UX journey map: every user perspective (Passer-by → Scout → Field Reporter → City Ambassador → Supporter → Movement Partner), wireframed low-fi, with a build status on every feature and a master roadmap matrix.", audience: "Team / Partners", auth: "none", flows: ["/sitemap", "/map"] },
...
export const JOURNEYS = [
  ...
  { label: "Field Reporter", color: "text-flare", steps: ["Profile", "Field Tools", "Zora Mint", "Ambassador"] },
