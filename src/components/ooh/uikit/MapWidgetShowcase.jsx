...
const GRID_LAYOUT = [
  ...
  {
    pos: 'top-right',
    mobile: 'Graffiti · Capture · Report',
    desktop: 'Style · Find · GeoJSON · Keys │ Graffiti · Capture · Report',
    z: 1000,
  },
  {
    pos: 'top-center',
    mobile: 'Alert ticker (below buttons)',
    desktop: 'Alert ticker (left-3 → right-3, below buttons)',
    z: 900,
  },
  {
    pos: 'left-rail',
    mobile: 'Field tally (closable)',
    desktop: 'Field tally (top-28, left rail)',
    z: 1000,
  },
  { pos: 'bottom', mobile: 'SpecsBar (flat only)', desktop: 'SpecsBar (flat only)', z: 900 },
  {
    ...
    desktop: 'Zoom controls (terminal-styled)',
    z: 1000,
  },
];
  ...
  return (
    <div className="space-y-5">
      <p className="max-w-2xl font-body text-sm leading-[1.6] text-darkgray">
        Floating overlay system for the field map — every widget is positioned on a strict grid with
        z-indexed layering. Mobile collapses secondary tools to keep the viewport clear; desktop
        shows the full rail. Terminal-styled, closable, and persistent via localStorage.
      </p>

      {/* Grid layout reference */}
      <div>
        <div className="mb-2 font-mono text-[9px] uppercase tracking-[0.25em] text-dim">
          // Widget grid · position map
        </div>
          ...
          {GRID_LAYOUT.map((w) => (
            <div key={w.pos} className="border border-slate2/40 bg-card p-3">
              ...
              <div className="mt-2 space-y-0.5">
                <div className="font-mono text-[8px] text-dim/60">
                  M: <span className="text-silver/80">{w.mobile}</span>
                </div>
                <div className="font-mono text-[8px] text-dim/60">
                  D: <span className="text-silver/80">{w.desktop}</span>
                </div>
              </div>
            </div>
          ))}
      ...
      <div>
        <div className="mb-2 font-mono text-[9px] uppercase tracking-[0.25em] text-dim">
          // Layout mockup · mobile vs desktop
        </div>
        <div className="grid gap-2 sm:grid-cols-2">
          {/* Mobile mockup */}
          <div className="border border-slate2/60 bg-void p-2">
            <div className="mb-1 font-mono text-[7px] uppercase tracking-[0.2em] text-flare">
              Mobile · 373px
            </div>
            <div className="relative aspect-[9/16] border border-slate2/40 bg-[#0a0a0a] grid-bg">
              <div className="absolute left-1.5 top-1.5 border border-slate2 bg-void/80 px-1 py-0.5 font-mono text-[6px] text-ozone">
                FLAT
              </div>
              <div className="absolute right-1.5 top-1.5 flex gap-0.5">
                <span className="border border-flare bg-flare px-1 py-0.5 font-mono text-[6px] font-bold text-void">
                  G
                </span>
                <span className="border border-ozone bg-ozone px-1 py-0.5 font-mono text-[6px] font-bold text-void">
                  C
                </span>
                <span className="border border-ozone bg-ozone px-1 py-0.5 font-mono text-[6px] font-bold text-void">
                  R
              ...
              <div className="absolute bottom-5 left-1.5 border border-slate2/70 bg-void/85 px-1 py-0.5 font-mono text-[6px] text-ozone">
                ▸ Tally 847
              </div>
              <div className="absolute inset-x-0 bottom-0 border-t border-slate2 bg-[#1a1a1a] px-1 py-0.5 font-mono text-[6px] text-dim">
                SpecsBar
              </div>
            </div>
          </div>
          {/* Desktop mockup */}
          <div className="border border-slate2/60 bg-void p-2">
            <div className="mb-1 font-mono text-[7px] uppercase tracking-[0.2em] text-ozone">
              Desktop · 1024px+
            </div>
            <div className="relative aspect-[16/9] border border-slate2/40 bg-[#0a0a0a] grid-bg">
              <div className="absolute left-2 top-2 border border-slate2 bg-void/80 px-1.5 py-0.5 font-mono text-[7px] text-ozone">
                FLAT | GLOBE
              </div>
              <div className="absolute right-2 top-2 flex items-center gap-0.5">
                <span className="border border-slate2 bg-void/80 px-1 py-0.5 font-mono text-[6px] text-darkgray">
                  Style
                </span>
                <span className="border border-slate2 bg-void/80 px-1 py-0.5 font-mono text-[6px] text-darkgray">
                  Find
              ...
              <div className="absolute left-2 top-14 right-2 border border-slate2/60 bg-void/90 px-1 py-0.5 font-mono text-[5px] text-silver/80">
                ▸ Alert ticker
              </div>
              ...
              <div className="absolute bottom-2 right-2 flex flex-col border border-ozone/15 bg-void/85">
                <span className="border-b border-slate2/60 px-1 py-0.5 font-mono text-[6px] text-darkgray">
                  +
                </span>
                <span className="px-1 py-0.5 font-mono text-[6px] text-darkgray">−</span>
              </div>
              <div className="absolute inset-x-0 bottom-0 border-t border-slate2 bg-[#1a1a1a] px-2 py-0.5 font-mono text-[6px] text-dim">
                SpecsBar
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-px bg-slate2/40">
            {STATS.map((x) => (
              <div key={x.k} className="bg-void px-2.5 py-1.5">
                <div className="font-mono text-[7px] uppercase tracking-[0.2em] text-dim">
                  {x.k}
                </div>
      ...
      <div>
        <div className="mb-2 font-mono text-[9px] uppercase tracking-[0.25em] text-dim">
          // FieldTallyWidget · collapsed state
        </div>
      ...
      <div>
        <div className="mb-2 font-mono text-[9px] uppercase tracking-[0.25em] text-dim">
          // Action button group · desktop layout
        </div>
      ...
      <div>
        ...
        <div className="flex items-center border border-slate2/60">
          {[
            ...
            <div
              key={m.label}
              className={`flex items-center gap-1.5 px-2.5 py-1.5 font-mono text-[9px] font-bold uppercase tracking-[0.15em] ${m.active ? 'bg-ozone text-void' : 'text-darkgray'}`}
            >
      ...
      <div>
        <div className="mb-2 font-mono text-[9px] uppercase tracking-[0.25em] text-dim">
          // CollapsiblePanels · independent search + results
        </div>
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-[8px] uppercase tracking-[0.2em] text-dim/60">
              ← expand tabs (stacked on map edge)
            </span>
          </div>
        </div>
      </div>
      ...
      <div>
        <div className="mb-2 font-mono text-[9px] uppercase tracking-[0.25em] text-dim">
          // MapZoomControls · terminal-styled (bottom-right)
        </div>
