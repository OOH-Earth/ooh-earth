...
function useStream(base, delta) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      setV((prev) =>
        Math.max(0, prev + Math.round(base * (delta / 100) * (0.4 + Math.random() * 0.9))),
      );
    }, 1500);
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="relative grid grid-cols-1 gap-3 md:grid-cols-[1.4fr_1fr]">
        <div
          className="relative grid place-items-center overflow-hidden border border-slate2/60 bg-void grid-bg"
          style={{ minHeight: 280 }}
        >
          <motion.div
            className="pointer-events-none absolute h-56 w-56 rounded-full"
            style={{
              background: 'conic-gradient(from 0deg, rgba(237,255,0,0.10), transparent 60%)',
            }}
            animate={{ rotate: 360 }}
          ...
          <Pin
            ...
            brands={BRANDS}
          />
          <span className="absolute left-3 top-3 font-mono text-[8px] uppercase tracking-[0.3em] text-dim/70">
            // stage · active pin · rollover reveals brand orbit
          </span>
          <button
        ...
        <div className="flex flex-col gap-3">
          <div className="border border-slate2/60 bg-card p-4">
            ...
            <div className="mt-1 flex items-baseline gap-2">
              <span className="font-display text-3xl font-bold tabular-nums text-silver">
            ...
            <div className="mt-4">
              <span className="flex items-center gap-1.5 font-mono text-[9px] uppercase tracking-[0.3em] text-dim">
                <Tag className="h-3 w-3" /> tagged brands
              </span>
              <div className="mt-2 flex flex-wrap gap-1.5">
                {BRANDS.map((b) => (
                  <span
                    key={b.id}
                    className="flex items-center gap-1 border border-slate2/60 px-1.5 py-1 font-mono text-[8px] text-silver/80"
                  >
                    <span
        ...
        {PIN_TYPES.map((p) => {
          ...
          return (
            ...
            >
              ...
              <span
                className={`font-mono text-[8px] uppercase tracking-[0.15em] ${isActive ? 'text-ozone' : 'text-dim'}`}
              >
                {p.label}
              </span>
            </button>
      ...
      <div className="grid grid-cols-3 gap-2">
        {STATES.map((s) => (
          <div
            key={s.key}
            className="flex flex-col items-center gap-2 border border-slate2/60 bg-void p-3 grid-bg"
          >
            <Pin
              type={active.type}
              size={44}
              selected={s.selected}
              hovered={s.hovered}
              brands={s.hovered || s.selected ? BRANDS : []}
            />
      ...
      <div>
        <div className="mb-2 flex items-center gap-2 border-b border-slate2/40 pb-2">
          <span className="font-mono text-[9px] uppercase tracking-[0.3em] text-ozone">
            // location detail · "Run Duck Cover"
          </span>
        </div>
        <SpotDetailPanel />
      </div>
      ...
      <p className="flex items-center gap-2 font-mono text-[9px] uppercase tracking-[0.2em] text-dim/70">
        <MousePointerClick className="h-3 w-3 text-ozone" /> click a variant to load it into the
        stage · hover reveals tagged brands + superscript reach
      </p>
    </div>
  );
}
