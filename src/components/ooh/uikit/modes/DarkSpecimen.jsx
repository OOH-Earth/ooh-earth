  ...
  return (
    <div className="dark relative min-h-[320px] overflow-hidden border border-border bg-background p-5 text-foreground">
      ...
      <div
        ...
        style={{ boxShadow: 'inset 0 0 80px -10px rgba(0,0,0,0.8)' }}
      />
      {CORNERS.map((c, i) => (
      ...
      <div className="relative">
        ...
        <div className="mt-3 flex items-end gap-3 border border-border bg-card p-3">
          <div>
            ...
            <div className="font-mono text-2xl font-bold tabular text-ozone text-glow-ozone">
              042
            </div>
          </div>
          <svg viewBox="0 0 100 30" className="h-7 flex-1" preserveAspectRatio="none">
            <path
              ...
              d="M0,28 L14,20 L28,24 L42,12 L56,16 L70,8 L84,14 L100,4 L100,30 L0,30 Z"
              fill="rgba(237,255,0,0.18)"
            />
            <path
              className="spark-line"
              d="M0,28 L14,20 L28,24 L42,12 L56,16 L70,8 L84,14 L100,4"
              fill="none"
              stroke="rgb(var(--c-ozone))"
              strokeWidth="1.6"
            />
          </svg>
          <span className="flex items-center gap-1 font-mono text-[9px] tabular text-ozone">
            <Activity className="h-3 w-3" /> +18%
          </span>
        </div>

        <div className="mt-3 flex items-center justify-between">
          <div className="flex gap-2">
            <button className="bg-ozone px-3 py-2 font-mono text-[9px] font-bold uppercase tracking-[0.2em] text-void transition-colors hover:bg-flare">
              Deploy
            </button>
            <button className="border border-border px-3 py-2 font-mono text-[9px] font-bold uppercase tracking-[0.2em] text-foreground transition-colors hover:border-ozone hover:text-ozone">
              Report
            </button>
          </div>
        ...
        <div className="mt-3 flex items-center gap-3 border-t border-border pt-2">
          {[
            ['ozone', 'bg-ozone'],
            ['flare', 'bg-flare'],
            ['void', 'bg-void border border-border'],
          ].map(([n, c]) => (
