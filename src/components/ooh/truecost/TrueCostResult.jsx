import { Loader2, AlertTriangle, Leaf, Droplet, Recycle, Users, TrendingUp, BadgeCheck } from "lucide-react";
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center gap-3 border border-slate2/60 bg-card p-12">
        ...
        <span className="font-mono text-[10px] uppercase tracking-[0.3em] text-dim">Auditing true cost…</span>
      </div>
    );
  }
  if (error) {
    ...
    <div className="border border-slate2/60 bg-card">
      <div className="border-b border-slate2/60 p-4">
        <div className="flex items-center gap-2">
          <span className="font-mono text-[9px] uppercase tracking-[0.25em] text-ozone">// TrueCost audit</span>
          {data.identified ? (
        ...
        <h3 className="mt-1 font-display text-xl font-black text-silver">{data.product_name || "Unknown product"}</h3>
        {data.brand_owner && <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-darkgray">{data.brand_owner}</p>}
      ...
      <div className="grid grid-cols-2 gap-2 p-4 pt-0 md:grid-cols-4">
        <Stat icon={Leaf} label="Carbon" value={`${Number(data.carbon_kg || 0).toFixed(1)} kg`} color="#FF5C00" />
        <Stat icon={Droplet} label="Water" value={`${Number(data.water_liters || 0).toLocaleString()} L`} color="#1F51FF" />
        <Stat icon={Users} label="Labor risk" value={(data.labor_risk || "—").toUpperCase()} color={rc} />
        <Stat icon={Recycle} label="Packaging" value={data.packaging_recyclable ? "Recyclable" : "Landfill"} color={data.packaging_recyclable ? "#39FF14" : "#FF5C00"} />
      </div>
        ...
        <div className="border-t border-slate2/60 p-4">
          ...
          <ul className="space-y-1">
            {data.greenwashing_flags.map((f, i) => (
              <li key={i} className="font-display text-sm text-silver">— {f}</li>
            ))}
          </ul>
        </div>
      )}
      ...
      {data.brand_accountability && (
        <div className="border-t border-slate2/60 p-4">
          <div className="mb-1 font-mono text-[9px] uppercase tracking-[0.2em] text-dim">// Brand accountability</div>
          <p className="font-display text-sm leading-relaxed text-darkgray">{data.brand_accountability}</p>
        </div>
      )}
