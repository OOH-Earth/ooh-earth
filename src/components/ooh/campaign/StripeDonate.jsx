    ...
    <div className="overflow-hidden border border-ozone/40 bg-card p-4 md:p-6">
      ...
      {inIframe && (
        <p className="mt-3 border border-flare/40 bg-flare/5 p-3 font-mono text-[9px] uppercase leading-relaxed tracking-[0.2em] text-flare">
          // Checkout works only from the published app. Open oohearth.app in its own tab to donate
          by card.
        </p>
      )}
      {error && (
        <p className="mt-3 font-mono text-[10px] uppercase tracking-[0.2em] text-flare">{error}</p>
      )}
      <button
        onClick={donate}
