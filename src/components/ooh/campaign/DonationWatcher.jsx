  ...
  return (
    <div className="border border-slate2/60 bg-card p-5">
      ...
      <p className="mt-1 font-mono text-[10px] uppercase tracking-[0.2em] text-dim">
        // recent on-chain activity — incoming to the OOH treasury
      </p>
      ...
      <div className="mt-4 space-y-1.5">
        {loading ? (
          ...
          <div className="py-6 text-center font-mono text-[10px] uppercase tracking-[0.2em] text-dim">
            // feed unavailable — send the first transmission
          </div>
        ) : items.length ? (
          items.map((it) => (
            <a
              key={it.chain + it.hash}
              href={it.url}
              target="_blank"
              rel="noreferrer"
              className="group flex items-center gap-3 border border-slate2/40 bg-void p-2.5 transition-colors hover:border-ozone/50"
            >
              <span
                className={`font-mono text-[9px] font-bold uppercase tracking-[0.15em] ${it.chain === 'SOL' ? 'text-ozone' : 'text-flare'}`}
              >
                {it.chain}
              </span>
              ...
              <span className="font-mono text-[9px] uppercase tracking-[0.15em] text-dim">
                {it.time ? formatDistanceToNow(new Date(it.time), { addSuffix: true }) : 'recent'}
              </span>
          ...
          <div className="py-6 text-center font-mono text-[10px] uppercase tracking-[0.2em] text-dim">
            // no activity yet — be the first to fund
          </div>
        )}
      </div>
      {ts && (
        <div className="mt-3 font-mono text-[9px] uppercase tracking-[0.2em] text-dim/60">
          last sync {formatDistanceToNow(new Date(ts), { addSuffix: true })}
        </div>
      )}
    </div>
  );
