    ...
    <div className="overflow-hidden border border-slate2/60 bg-card p-4 md:p-6">
      ...
      <div className="mt-4 space-y-2">
        {available.length ? available.map((c) => (
          <div key={c.key} className="group relative overflow-hidden border border-slate2/50 bg-void p-3 transition-colors hover:border-ozone/40">
            <div className="flex items-center gap-2">
              <span className="w-12 shrink-0 font-mono text-[10px] font-bold uppercase tracking-[0.15em] text-ozone">{c.ticker}</span>
            ...
            <div className="mt-1.5 flex items-center gap-2 overflow-hidden">
              <span className="truncate font-mono text-[8px] uppercase tracking-[0.2em] text-silver/50">{c.network}</span>
              {c.sharedWith && <span className="shrink-0 font-mono text-[8px] uppercase tracking-[0.15em] text-flare/60">// shared with {c.sharedWith.toUpperCase()}</span>}
            </div>
