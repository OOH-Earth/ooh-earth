...
export async function unlockInvestor(input) {
  ...
  try {
    ...
    if (data?.ok && data?.token) {
      try { sessionStorage.setItem(KEY, data.token); } catch { /* storage blocked */ }
      return true;
    }
    return false;
  } catch { return false; }
...
export function getInvestorToken() { try { return sessionStorage.getItem(KEY) || ""; } catch { return ""; } }
export function hasInvestorToken() { return !!getInvestorToken(); }
export function clearInvestorSession() { try { sessionStorage.removeItem(KEY); } catch { /* noop */ } }
...
export async function verifyInvestorSession() {
  ...
  try {
    const data = payload(await base44.functions.invoke("investorAccess", { action: "verify", token }));
