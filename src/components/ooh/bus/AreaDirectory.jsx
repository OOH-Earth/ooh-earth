...
const AREA_INDEX = (() => {
  const m = {};
  BUS_STOP_AREAS.forEach(({ area, stops }) => stops.forEach((s) => { m[s.id] = area; }));
  return m;
})();
...
export default function AreaDirectory({ stops }) {
  const groups = useMemo(() => {
    ...
    return Array.from(map, ([area, list]) => ({ area, list })).sort((a, b) => b.list.length - a.list.length);
  }, [stops]);

  if (!stops.length) {
    return <div className="py-12 text-center font-mono text-[10px] uppercase tracking-[0.25em] text-dim">// no stops match</div>;
  }
      ...
      {groups.map(({ area, list }) => (
        <section key={area}>
          <div className="mb-2 flex items-center gap-2 border-b border-slate2/60 pb-1.5">
            <span className="font-mono text-[9px] uppercase tracking-[0.3em] text-ozone">// {area}</span>
            <span className="font-mono text-[8px] uppercase tracking-[0.2em] text-dim/50">· {list.length}</span>
            ...
            {list.map((b) => (
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  ...
                  {b.shape === "shelter" && <span className="font-mono text-[8px] uppercase tracking-[0.2em] text-ozone/70">shelter</span>}
                </div>
              </Link>
            ))}
          </div>
