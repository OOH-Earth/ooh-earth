  ...
  return (
    <div className="border border-slate2/60 bg-void">
      ...
      <div className="grid gap-6 border-t border-slate2/60 p-5 md:grid-cols-3">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="font-mono text-[9px] uppercase tracking-[0.25em] text-dim">
              // Field-check progress
            </div>
            <div className="mt-1 font-display text-lg font-bold text-flare">
              {confirmed}/{total}
            </div>
            <div className="font-mono text-[9px] uppercase tracking-[0.2em] text-dim/70">
              keys confirmed in field
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex h-3 w-full overflow-hidden">
            <div
              style={{
                width: `${Math.round((shelters / total) * 100)}%`,
                backgroundColor: '#EDFF00',
              }}
            />
            <div
              style={{
                width: `${Math.round((poles / total) * 100)}%`,
                backgroundColor: 'rgba(241,241,241,0.18)',
              }}
            />
          </div>
          <div className="mt-2 flex justify-between font-mono text-[10px] tabular">
      ...
      <div className="border-t border-slate2/60 p-5">
        <div className="mb-3 font-mono text-[9px] uppercase tracking-[0.25em] text-dim">
          // Distribution by area
        </div>
        <div className="grid gap-1.5 sm:grid-cols-2">
          {topAreas.map((a) => (
            <div key={a.area} className="flex items-center gap-2">
              <span className="w-28 shrink-0 truncate font-mono text-[9px] uppercase tracking-[0.15em] text-darkgray">
                {a.area}
              </span>
              <div className="h-2 flex-1 bg-slate2/30">
                <div
                  className="h-full bg-ozone/70"
                  style={{ width: `${(a.n / maxArea) * 100}%` }}
                />
              </div>
