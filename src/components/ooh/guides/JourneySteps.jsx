  ...
  return (
    <ol className="space-y-2">
      {STEPS.map((s) => (
        <li
          key={s.n}
          className="border border-slate2/60 bg-card p-4 transition-colors hover:border-ozone/40"
        >
          ...
          <h3 className="mt-1.5 font-display text-lg font-bold tracking-[-0.02em] text-silver">
            {s.title}
          </h3>
          ...
          <ul className="mt-3 flex flex-wrap gap-1.5">
            {s.actions.map((a) => (
              <li
                key={a}
                className="border border-slate2 px-2 py-1 font-mono text-[10px] uppercase tracking-[0.15em] text-silver/85"
              >
                {a}
              </li>
            ))}
