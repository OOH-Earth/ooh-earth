const STATUS = [
  {
    area: 'Atlas / Maps',
    state: 'LIVE',
    note: 'Seeded static directory · live oohearth.app ingest blocked by bot protection',
  },
  ...
  {
    area: 'Wallet connect (SOL/EVM)',
    state: 'BETA',
    note: 'Read-only address · no on-chain tx signing yet',
  },
  {
    area: 'AR Lens overlay',
    state: 'BETA',
    note: 'UI ready · device AR session gated behind publish',
  },
  {
    area: 'TrueCost scan',
    state: 'GATED',
    note: 'Works only on published HTTPS · blocked in preview iframe',
  },
  { area: 'Trash ID', state: 'GATED', note: 'Same HTTPS gate as TrueCost' },
  {
    area: 'Stripe checkout',
    state: 'GAP',
    note: 'Live mode claimed · no donation product/price configured yet',
  },
  {
    area: 'Crypto treasury',
    state: 'GAP',
    note: 'BTC/ETH/SOL shown · USDC & Polygon slots empty · watcher unverified until first tx',
  },
  {
    area: 'Email notifications',
    state: 'GAP',
    note: 'Registered app users only · external addresses unsupported',
  },
];
    ...
    <div>
      <div className="mb-4 grid grid-cols-4 gap-2">
        <div className="border border-slate2/60 bg-card p-3">
          ...
          <div className="font-display text-2xl font-bold tabular-nums text-ozone">
            {String(live).padStart(2, '0')}
          </div>
        </div>
        <div className="border border-slate2/60 bg-card p-3">
          <span className="font-mono text-[9px] uppercase tracking-[0.25em] text-dim">Beta</span>
          <div className="font-display text-2xl font-bold tabular-nums text-flare">
            {String(STATUS.filter((s) => s.state === 'BETA').length).padStart(2, '0')}
          </div>
        </div>
        <div className="border border-slate2/60 bg-card p-3">
          <span className="font-mono text-[9px] uppercase tracking-[0.25em] text-dim">Gated</span>
          <div className="font-display text-2xl font-bold tabular-nums text-darkgray">
            {String(STATUS.filter((s) => s.state === 'GATED').length).padStart(2, '0')}
          </div>
        </div>
      ...
      <div className="border border-slate2/60">
        {STATUS.map((s, i) => (
          ...
          >
            <span
              className={`w-fit border px-2 py-1 font-mono text-[9px] font-bold uppercase tracking-[0.2em] ${BADGE[s.state]}`}
            >
              {s.state}
            </span>
            <div className="min-w-0">
              <div className="font-display text-sm font-semibold tracking-[-0.01em] text-silver">
                {s.area}
              </div>
              <div className="font-body text-[12px] leading-[1.45] text-darkgray">{s.note}</div>
            </div>
            <span className="hidden font-mono text-[9px] tabular text-dim/80 md:block">
