  ...
  return (
    <div className="flex h-full flex-col">
      <div className="border-b border-slate2/60 px-4 py-2">
        <span className="font-mono text-[10px] uppercase tracking-[0.25em] text-dim">// {busts.length} digital busts</span>
      </div>
        ...
        {busts.length ? (
          busts.map((b) => {
            ...
            return (
              ...
              >
                <span className="flex h-8 w-8 shrink-0 items-center justify-center border border-slate2/60" style={{ backgroundColor: "#0a0a0a" }}>
                  <Icon className="h-4 w-4" style={{ color: meta.accent }} strokeWidth={1.5} />
                </span>
                <span className="min-w-0 flex-1">
                  <span className="flex items-center gap-1.5">
                    <span className="truncate font-display text-xs font-bold text-silver">{b.target_brand}</span>
          ...
          <div className="p-6 text-center font-mono text-[10px] uppercase tracking-[0.25em] text-dim">// no busts</div>
        )}
      </div>
    </div>
  );
