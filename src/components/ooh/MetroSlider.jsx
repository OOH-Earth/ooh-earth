  ...
  return (
    <div className="group relative z-40 border-b border-slate2/40 bg-void/80 backdrop-blur-sm">
      <div className="flex items-center justify-between px-5 py-2 md:px-8">
        <span className="font-mono text-[9px] uppercase tracking-[0.3em] text-dim">// Latest field log</span>
        <Link
          to="/map"
        ...
        >
          {loop.map((l, i) => {
            ...
            return (
              ...
              >
                <Image src={l.image_url} alt={l.title || "Field location"} className="h-full w-full" fittingType="fill" />
                ...
                <div className="absolute inset-x-0 bottom-0 p-2">
                  <div className="truncate font-display text-xs font-bold text-silver">{l.title}</div>
                  {l.address && (
                    <div className="truncate font-mono text-[8px] uppercase tracking-[0.15em] text-darkgray">{l.address}</div>
                  )}
                </div>
              </Link>
            );
