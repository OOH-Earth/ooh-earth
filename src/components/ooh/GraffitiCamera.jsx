...
export default function GraffitiCamera({ open, onClose }) {
  ...
  const reset = () => {
    setDone(null); setImageUrl(""); setAddress(""); setLat(""); setLng("");
    setType("painted"); setMedium("spray_paint"); setStyle("");
    setSurfaceM2(""); setCoveragePct(""); setError("");
  };
  ...
  const submit = async () => {
    ...
    if (!image_url) { setError("Capture a graffiti photograph first."); return; }
  ...
  return (
    <div className="fixed inset-0 z-[2000] flex items-end justify-center bg-black/70 backdrop-blur-sm sm:items-center" onClick={onClose}>
      <div className="relative max-h-[92dvh] w-full max-w-md overflow-y-auto border border-flare/40 bg-void p-5 sm:p-6" onClick={(e) => e.stopPropagation()}>
        ...
        {done ? (
          <>
            ...
            <p className="mt-2 font-display text-sm leading-[1.4] text-darkgray">{done.queued ? `Offline — saved on this device. It transmits automatically when you reconnect. Position ${done.lat?.toFixed(4)}, ${done.lng?.toFixed(4)}.` : `Graffiti documented at ${done.lat?.toFixed(4)}, ${done.lng?.toFixed(4)}. It renders on the map and graffiti portal pending verification.`}</p>
          ...
          <>
            <div className="flex items-center gap-2">
              ...
              <span className="font-mono text-[10px] uppercase tracking-[0.3em] text-flare">Graffiti field camera</span>
            </div>
            <h3 className="mt-2 font-display text-xl font-bold tracking-[-0.02em] text-silver">Photograph the piece</h3>
            <p className="mt-1 font-mono text-[10px] leading-relaxed text-dim">Capture graffiti and street art in the wild — logs to the graffiti portal and field map.</p>
            ...
            {/* Surface type */}
            <div className="mt-4">
              <span className="mb-2 block font-mono text-[10px] uppercase tracking-[0.3em] text-dim">Surface type</span>
              <div className="grid grid-cols-5 gap-px border border-slate2/60 bg-slate2/40">
                {GRAFFITI_TYPES.map((t) => (
                  <button key={t.value} type="button" onClick={() => setType(t.value)} className={`py-2 font-mono text-[9px] font-bold uppercase tracking-[0.1em] transition-colors ${type === t.value ? "bg-flare text-void" : "bg-card text-darkgray hover:text-silver"}`}>{t.label}</button>
                ))}
              </div>
            </div>
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              <label className="flex flex-col gap-1">
                ...
                <select value={medium} onChange={(e) => setMedium(e.target.value)} className="border border-slate2 bg-void px-3 py-2 text-sm text-silver outline-none focus:border-flare">
              ...
              <label className="flex flex-col gap-1">
                ...
                <select value={style} onChange={(e) => setStyle(e.target.value)} className="border border-slate2 bg-void px-3 py-2 text-sm text-silver outline-none focus:border-flare">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              <label className="flex flex-col gap-1">
                ...
                <input type="number" step="0.1" value={surfaceM2} onChange={(e) => setSurfaceM2(e.target.value)} className="border border-slate2 bg-void px-3 py-2 text-sm text-silver outline-none focus:border-flare" placeholder="e.g. 4.5" />
              </label>
              <label className="flex flex-col gap-1">
                <span className="font-mono text-[9px] uppercase tracking-[0.2em] text-dim">Coverage %</span>
                <input type="number" min="0" max="100" value={coveragePct} onChange={(e) => setCoveragePct(e.target.value)} className="border border-slate2 bg-void px-3 py-2 text-sm text-silver outline-none focus:border-flare" placeholder="0–100" />
              </label>
            </div>

            <input value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Street, district (optional)" className="mt-4 w-full border border-slate2 bg-void px-4 py-3 font-display text-sm text-silver outline-none transition-colors placeholder:text-dim focus:border-flare" />
            ...
            <div className="mt-3 flex items-center justify-between font-mono text-[10px] uppercase tracking-[0.2em] text-dim">
              ...
              <button onClick={locate} className="text-flare transition-opacity hover:opacity-70">{locating ? "…" : "Re-locate"}</button>
            </div>
            ...
            <button onClick={submit} disabled={submitting || uploading} className="mt-5 flex w-full items-center justify-center gap-2 border border-flare bg-flare px-6 py-4 font-mono text-[11px] font-bold uppercase tracking-[0.3em] text-void transition-colors hover:bg-ozone hover:border-ozone disabled:opacity-40">
