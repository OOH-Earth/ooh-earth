      ...
      <div className="px-5 py-16 md:px-8 md:py-24">
        <div className="flex flex-col gap-4 border-b border-slate2/40 pb-6 md:flex-row md:items-end md:justify-between">
          ...
          <p className="max-w-sm font-display text-sm font-normal leading-[1.4] text-darkgray">
            The same corporations buying every billboard are among the world's worst emitters.
            Here's their outdoor ad spend set against the damage it greenwashes — the cost never
            appears in their creative briefs.
          </p>
        </div>
              ...
              <div key={r.brand} className="p-5 md:p-6">
                <div className="flex items-baseline justify-between gap-4">
                  <div className="flex items-baseline gap-3">
                    <span className="font-display text-sm font-black text-dim tabular">
                      {String(i + 1).padStart(2, '0')}
                    </span>
                    <span className="font-display text-xl font-bold tracking-[-0.02em] text-silver md:text-2xl">
                      {r.brand}
                    </span>
                  </div>
                  <div className="text-right">
                    <span className="font-display text-xl font-black tabular text-ozone md:text-2xl">
                      ${(r.ad_spend_musd || 0).toLocaleString('en-US')}M
                    </span>
                    <span className="ml-2 font-mono text-[9px] uppercase tracking-[0.15em] text-dim">
                      OOH spend/yr
                    </span>
                  </div>
                </div>
                <div className="mt-3 h-1.5 w-full bg-slate2/60">
                  <div
                    className="h-full bg-ozone/70"
                    style={{ width: `${((r.ad_spend_musd || 0) / maxSpend) * 100}%` }}
                  />
                </div>
                <div className="mt-3 flex flex-wrap items-center gap-x-5 gap-y-1 font-mono text-[10px] uppercase tracking-[0.15em]">
                  ...
                  {r.emissions_mt > 0 && (
                    <span className="flex items-center gap-1.5 text-darkgray">
                      <DollarSign className="h-3 w-3" /> ~{r.emissions_mt.toLocaleString('en-US')}{' '}
                      Mt CO₂e/yr
                    </span>
                  )}
                </div>
