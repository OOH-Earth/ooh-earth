...
export default function AdvertiserInfo({ loc }) {
  ...
  const hasFields = loc.brand_name || loc.campaign_name || loc.ad_agency || loc.parent_corp || loc.ooh_operator || loc.industry_sector;
  const hasHarm = (loc.harm_tags && loc.harm_tags.length > 0) || loc.harm_statement || (loc.action_flags && loc.action_flags.length > 0);
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex items-center gap-2">
        {isSubverted ? <Ban className="h-3.5 w-3.5 text-flare" /> : <Megaphone className="h-3.5 w-3.5 text-ozone" />}
        <span className={`font-mono text-[9px] font-bold uppercase tracking-[0.25em] ${isSubverted ? "text-flare" : "text-ozone"}`}>
