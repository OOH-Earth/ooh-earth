    ...
    <div className="border border-slate2/60 bg-card">
      ...
      <div className="grid grid-cols-2 gap-px bg-slate2/40 sm:grid-cols-4">
        <Stat
          label="USDC"
          sub="native · Polygon"
          value={loading ? '…' : data ? num(data.usdc) : '--'}
          accent
        />
        <Stat label="USDC.e" sub="bridged" value={loading ? '…' : data ? num(data.usdce) : '--'} />
        <Stat
          label="POL"
          sub={`@ ${data && data.maticUsd ? '$' + num(data.maticUsd, 4) : '—'}`}
          value={loading ? '…' : data ? num(data.matic) : '--'}
        />
        <Stat
          label="Total"
          sub="USD value"
          value={loading ? '…' : data ? usd(data.totalUsd) : '--'}
          accent
        />
      </div>
      ...
      <div className="flex items-center justify-between px-5 py-2.5">
        <a
          href={`https://polygonscan.com/address/${POL_ADDR}`}
          target="_blank"
          rel="noreferrer"
      ...
      <p className="border-t border-slate2/40 px-5 py-2 font-mono text-[8px] uppercase leading-relaxed tracking-[0.1em] text-dim">
        // USDC (native) is the canonical Polygon stablecoin; USDC.e is the bridged legacy variant.
        Both are tracked live from the treasury wallet.
      </p>
    </div>
  );
}
...
function Stat({ label, sub, value, accent = false }) {
  return (
    <div className="bg-card p-4">
      ...
      <div
        className={`mt-1 font-display text-xl font-bold tabular-nums ${accent ? 'text-ozone' : 'text-silver'}`}
      >
        {value}
      </div>
      <div className="mt-0.5 font-mono text-[8px] uppercase tracking-[0.15em] text-dim/70">
        {sub}
      </div>
    </div>
  );
