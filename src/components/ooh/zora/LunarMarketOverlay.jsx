...
export default function LunarMarketOverlay() {
  ...
  const days = Array.from({ length: 30 }, (_, i) => {
    ...
    return { dt, p, label: phaseLabel(p), isToday: i === 0, isFull: p > 0.47 && p < 0.53, isNew: p < 0.03 || p > 0.97 };
  });
  ...
  return (
    <div className="border border-slate2/60 bg-card p-5 md:p-7">
      <div className="mb-3 flex items-center justify-between">
        <span className="font-mono text-[10px] uppercase tracking-[0.3em] text-ozone">// lunar × market overlay</span>
        <span className="font-mono text-[9px] uppercase tracking-[0.2em] text-dim">synodic · 29.53d</span>
      </div>
      ...
      <div className="flex items-center gap-4 border-b border-slate2/40 pb-3">
        ...
        <div className="ml-auto text-right">
          <div className="font-mono text-[9px] uppercase tracking-[0.2em] text-dim">{PRIMARY_COIN.symbol} now</div>
          <div className="font-mono text-lg font-bold text-ozone">{d ? fmtPrice(d.price) : "…"}</div>
        </div>
      </div>
      ...
      <div className="mt-4 flex gap-0.5">
        {days.map((day, i) => (
          <div key={i} className="group relative flex-1" title={`${day.dt.toLocaleDateString("en-US", { month: "short", day: "numeric" })} · ${day.label}`}>
            <div
              ...
              style={{
                background:
                  day.isNew ? "#000"
                  : day.isFull ? "#EDFF00"
                  : day.p < 0.5 ? `rgba(237,255,0,${0.1 + day.p * 0.6})`
      ...
      <p className="mt-3 font-mono text-[9px] uppercase tracking-[0.2em] text-dim">
        full-moon windows marked ozone · action-intensity scales with phase brightness · price readout live
      </p>
    </div>
  );
}
