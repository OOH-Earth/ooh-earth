  ...
  if (!d) {
    return (
      <div className="flex h-48 items-center justify-center border border-slate2/60 bg-card">
        <span className="font-mono text-[10px] uppercase tracking-[0.3em] text-dim animate-flicker">// acquiring market signal…</span>
      </div>
    );
  }
      ...
      <div className="flex flex-wrap items-end justify-between gap-3 border-b border-slate2/40 pb-4">
        <div>
          <div className="flex items-center gap-2">
            <span className="bg-ozone px-2 py-0.5 font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-void">{PRIMARY_COIN.symbol}</span>
            <span className="font-mono text-[9px] uppercase tracking-[0.25em] text-dim">{PRIMARY_COIN.note}</span>
          </div>
          <div className="mt-3 font-display text-4xl font-bold tracking-[-0.02em] text-silver">{fmtPrice(d.price)}</div>
        </div>
        <div className="flex items-center gap-4">
          <Stat label="24h" value={d.change == null ? "--" : `${Math.abs(d.change).toFixed(2)}%`} tone={up ? "up" : "down"} icon={up ? <TrendingUp className="h-3 w-3" /> : <TrendingDown className="h-3 w-3" />} />
        ...
        {d.history.length > 1 ? (
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={d.history} margin={{ top: 4, right: 4, bottom: 4, left: 4 }}>
              ...
              <Tooltip
                contentStyle={{ background: "#0a0a0a", border: "1px solid rgba(241,241,241,0.1)", fontSize: 11 }}
                labelFormatter={() => ""}
                formatter={(v) => [fmtPrice(v), "price"]}
              />
              <Line type="monotone" dataKey="p" stroke={up ? "#EDFF00" : "#FF5C00"} strokeWidth={1.5} dot={false} isAnimationActive={false} />
            </LineChart>
          </ResponsiveContainer>
        ) : (
...
function Stat({ label, value, tone = "", icon = null }) {
  return (
    <div className="text-right">
      ...
      <div className={`mt-0.5 flex items-center justify-end gap-1 font-mono text-sm font-bold ${tone === "up" ? "text-ozone" : tone === "down" ? "text-flare" : "text-silver"}`}>
        {icon}{value}
      </div>
    </div>
  );
