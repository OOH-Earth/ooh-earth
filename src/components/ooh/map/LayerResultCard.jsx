...
export default function LayerResultCard({ item, layer }) {
  ...
  const title =
    layer === 'war' ? item.title : layer === 'rivers' ? item.name : item.region || 'Unknown';
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          ...
          {tag && (
            <span
              className="ml-auto border px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-[0.15em]"
              style={{ color: tagColor, borderColor: `${tagColor}60` }}
            >
              {tag}
            </span>
          )}
        </div>
        <div className="mt-0.5 truncate font-display text-sm font-semibold leading-tight text-silver">
          {title}
        </div>
        {subtitle && (
        ...
        {layer === 'rivers' && (
          <div className="mt-1 flex gap-3 font-mono text-[9px]">
            ...
            <span className="text-dim">
              pH <span className="text-silver">{item.ph}</span>
            </span>
            <span className="text-dim">
              NTU <span className="text-silver">{item.turbidity}</span>
            </span>
          </div>
        )}
        {note && (
