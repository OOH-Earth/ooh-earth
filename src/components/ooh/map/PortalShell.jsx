...
export default function PortalShell({
  ...
  const resultsContent = (
    <div className="space-y-px">
      {loading ? (
        <div className="p-6 text-center font-mono text-[10px] uppercase tracking-[0.25em] text-dim">// Loading…</div>
      ) : results.length ? (
        results.map((item, i) => renderCard(item, i, { selectedId, setSelectedId, hoverId, setHoverId }))
      ) : (
  ...
  return (
    <div className="fixed inset-0 flex flex-col overflow-hidden bg-void pt-[calc(7rem_+_env(safe-area-inset-top))] md:pt-[calc(8rem_+_env(safe-area-inset-top))] pb-[calc(76px_+_env(safe-area-inset-bottom))] lg:pb-0">
      ...
      <div className="flex items-center gap-3 border-b border-slate2/40 px-4 py-2 md:px-8">
        <span className="font-mono text-[10px] font-bold uppercase tracking-[0.25em]" style={{ color: accent }}>
          // {title}
        </span>
        {loading && <Loader2 className="h-3 w-3 animate-spin text-dim" />}
        <span className="font-mono text-[9px] uppercase tracking-[0.2em] text-dim">{results.length} results</span>
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button
            onClick={() => { setQuery(""); onFilterChange?.("all"); }}
        ...
        <div className="atlas-track flex items-center gap-1.5 overflow-x-auto border-b border-slate2/40 px-4 py-1.5 md:px-8">
          {filterTags.map((t) => {
            ...
            return (
              ...
              >
                {t.label}
                <span className={`text-[9px] ${active ? "text-void/70" : "text-dim"}`}>{t.count}</span>
              </button>
            );
          })}
        </div>
      ...
      <div className="flex min-h-0 flex-1 flex-col lg:flex-row">
        ...
        <div className={`min-h-0 flex-col border-r border-slate2/60 ${cardsClass}`}>
          <div className="flex items-center justify-between border-b border-slate2/60 px-4 py-2">
            <span className="font-mono text-[10px] uppercase tracking-[0.25em] text-dim">// {results.length} results</span>
          </div>
          {onRefresh ? (
