...
function requestGeocode(q) {
  const now = Date.now();
  if (q === _cache.q && now - _cache.ts < 8000) { emit(); return; }
...
export default function MapSearch({ query, setQuery, onFlyTo, onReset = null, placeholder = "Search street, city, or place", className = "" }) {
  ...
  useEffect(() => { requestGeocode(query); }, [query]);
  ...
  const onKeyDown = (e) => {
    ...
    if (e.key === "ArrowDown") { e.preventDefault(); setActiveIdx((i) => Math.min(i + 1, suggestions.length - 1)); setOpen(true); }
  ...
  return (
    <div ref={boxRef} data-tour="search" className={`relative ${className}`}>
      <div className="flex items-center gap-2 border border-slate2 bg-card px-3 py-2.5 transition-colors focus-within:border-ozone/60">
        ...
        {!loading && query && (
          <button onClick={clear} aria-label="Clear search" className="shrink-0 text-dim transition-colors hover:text-ozone">
