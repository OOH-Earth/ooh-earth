...
function PseudoQR({ seed }) {
  ...
  const Finder = ({ r, c }) => (
    <div
      className="absolute border-2 border-silver"
      style={{
        top: `${(r / size) * 100}%`,
        left: `${(c / size) * 100}%`,
        width: `${(7 / size) * 100}%`,
...
export default function FieldIdCard({ op }) {
  ...
  return (
    <div
      className="relative w-[340px] select-none overflow-hidden border border-slate2 bg-void"
      style={{ height: '214px' }}
    >
      <div className="hi-vis-stripes flex h-[26px] items-center justify-between px-3">
        <span className="bg-void px-1.5 font-mono text-[9px] font-bold uppercase tracking-[0.25em] text-ozone">
          OOH·EARTH
        </span>
        <span className="bg-void px-1.5 font-mono text-[9px] font-bold uppercase tracking-[0.25em] text-ozone">
          Field Reporter
      ...
      <div className="flex gap-3 px-3 pt-2">
        <div className="flex h-16 w-14 shrink-0 items-center justify-center border border-slate2 bg-card font-brand text-2xl text-ozone">
          {initials}
        </div>
        <div className="min-w-0 flex-1">
          <div className="font-mono text-[8px] uppercase tracking-[0.25em] text-dim">Member</div>
          <div className="truncate font-display text-xl font-bold leading-tight tracking-[-0.02em] text-silver">
            {op.handle || 'ghost'}
          </div>
          <div className="mt-0.5 font-body text-[10px] leading-tight text-darkgray">{op.name}</div>
          <div className="mt-1.5 flex items-center gap-1.5">
            <span
              className={`border px-1.5 py-0.5 font-mono text-[8px] font-bold uppercase tracking-[0.2em] ${tier.cls}`}
            >
              {tier.label}
            </span>
            {op.verified && (
      ...
      <div className="absolute bottom-[8px] left-3 right-3 flex items-end justify-between">
        <div className="font-mono text-[8px] leading-tight text-dim">
          <div>
            REGION · <span className="text-silver/80">{op.region || '—'}</span>
          </div>
          <div>
            ID · <span className="text-silver/80">{memberId(op.handle)}</span>
          </div>
          <div>
            PTS · <span className="text-ozone tabular">{op.points ?? 0}</span>
          </div>
        </div>
        <div className="w-14">
          <PseudoQR seed={op.handle + op.tier} />
        </div>
      </div>
    </div>
  );
