...
export default function NfcFieldCard({ handle = 'operative', memberId = 'OOH-0000-0' }) {
  return (
    <div
      className="relative w-[340px] select-none overflow-hidden border border-slate2 bg-void shadow-[0_24px_60px_rgba(0,0,0,0.6)]"
      style={{ height: '214px', transform: 'perspective(900px) rotateY(-14deg) rotateX(4deg)' }}
    >
      ...
      <div className="px-3 pt-3">
        ...
        <div className="font-display text-xl font-bold leading-tight tracking-[-0.02em] text-silver">
          {handle}
        </div>
        <div className="mt-1 inline-block border border-ozone px-1.5 py-0.5 font-mono text-[8px] font-bold uppercase tracking-[0.2em] text-ozone">
          Field · Verified
        </div>
      </div>
      ...
      <div className="absolute bottom-[10px] right-3 flex items-end gap-1">
        {[0, 1, 2].map((i) => (
          <span
            key={i}
            className="rounded-full border border-ozone/40"
            style={{ width: `${10 + i * 8}px`, height: `${10 + i * 8}px`, opacity: 1 - i * 0.28 }}
          />
        ))}
      </div>
