...
const GLYPHS =
  '\u30A2\u30AB\u30B5\u30BF\u30CA\u30CF\u30DE\u30E4\u30E9\u30EF\u30F2\u30A6\u30A8\u30AA\u30AD\u30B1\u30B30123456789\uFF66\uFF67\uFF68\uFF69\uFF6A\uFF6B\uFF6C:.=*+-<>\u00A6\uFF5C';
...
export default function MatrixLoader({ label = 'LOADING', fullscreen = true }) {
  ...
  if (!matrixLoaderEnabled()) {
    return (
      <div
        className={`${fullscreen ? 'fixed' : 'absolute'} inset-0 z-[9999] flex flex-col items-center justify-center gap-3 bg-void`}
      >
        <div className="h-8 w-8 animate-spin rounded-full border-4 border-slate2 border-t-ozone" />
        <span className="font-mono text-[10px] uppercase tracking-[0.28em] text-ozone/70">
          // {label}
          {dots}
        </span>
      </div>
  ...
  return (
    <div
      className={`${fullscreen ? 'fixed' : 'absolute'} inset-0 z-[9999] overflow-hidden bg-[#020402]`}
    >
      ...
      <div
        ...
        style={{
          background:
            'radial-gradient(120% 120% at 50% 45%, transparent 38%, rgba(0,0,0,0.88) 100%)',
        }}
      />
      {/* readout */}
      <div className="absolute inset-0 flex items-center justify-center">
        <div className="relative px-9 py-7">
          ...
          <div
            className="flex items-center gap-2 font-mono text-sm font-bold uppercase tracking-[0.3em] text-ozone"
            style={{ textShadow: '0 0 14px rgba(237,255,0,0.55)' }}
          >
            <span>
              // {label}
              {dots}
