...
import { X, ExternalLink, Globe, Building2, MapPin, Layers, Calendar, DollarSign, Users, AlertTriangle, FileText, TrendingUp } from "lucide-react";
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4">
          ...
          <div className="mt-4 grid grid-cols-2 gap-px border border-slate2/60 bg-slate2/40">
            ...
            <StatCell icon={Layers} label="Panels" value={corp.panels ? corp.panels.toLocaleString() : "—"} />
            <StatCell icon={Building2} label="Parent" value={corp.parent || "—"} />
            <StatCell icon={Calendar} label="Founded" value={corp.founded_year || "—"} />
            <StatCell icon={DollarSign} label="Revenue (M USD)" value={corp.revenue_usd_m ? `$${corp.revenue_usd_m.toLocaleString()}` : "—"} />
            <StatCell icon={Users} label="Employees" value={corp.employees ? corp.employees.toLocaleString() : "—"} />
            ...
            <div className="mt-4">
              ...
              <div className="mt-2 flex flex-wrap gap-1.5">
                {corp.controversy_tags.map((t) => (
                  <span key={t} className="border border-flare/40 px-2 py-1 font-mono text-[9px] uppercase tracking-[0.12em] text-flare">{t.replace(/_/g, " ")}</span>
                ))}
              </div>
            </div>
          )}
            ...
            <div className="mt-4">
              ...
              <div className="mt-2 space-y-1">
                {corp.data_sources.map((s, i) => (
                  <div key={i} className="font-mono text-[9px] text-dim/70">· {s}</div>
                ))}
              </div>
            </div>
          )}
          ...
          <div className="mt-5 flex flex-col gap-2">
            ...
            {corp.sustainability_url && (
              <a href={corp.sustainability_url} target="_blank" rel="noreferrer"
