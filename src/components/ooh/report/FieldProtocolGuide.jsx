...
import { Camera, Sparkles, ShieldAlert, Megaphone, ChevronDown, Check, BookOpen } from "lucide-react";
...
const STEPS = [
  ...
  },
  {
    ...
    image: "https://media.base44.com/images/public/6a62213cff3ccbca88c04ff5/dd151496c_generated_image.png",
  },
  {
    n: "04",
    label: "Respond",
    ...
    image: "https://media.base44.com/images/public/6a62213cff3ccbca88c04ff5/4c5833a10_generated_image.png",
  },
];
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          <div className="text-left">
            <div className="font-mono text-[9px] uppercase tracking-[0.3em] text-ozone">Field Protocol 01</div>
            <div className="font-mono text-[8px] uppercase tracking-[0.2em] text-dim">Safety & Field Manual</div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <ChevronDown className={`h-4 w-4 text-darkgray transition-transform duration-200 ${open ? "rotate-180" : ""}`} />
        </div>
      </button>
              ...
              return (
                ...
                >
                  ...
                  <div className="absolute left-0 top-0 z-10 flex items-center gap-1 bg-void/80 px-2 py-1 backdrop-blur-sm">
                    <span className={`font-mono text-[10px] font-bold tracking-[0.2em] ${COLOR[step.color].text}`}>STEP {step.n}</span>
                  </div>
                  ...
                  <div className="relative aspect-[16/10] overflow-hidden border-b border-slate2/60 bg-void">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 flex items-center gap-2 bg-gradient-to-t from-void via-void/80 to-transparent px-3 pb-2 pt-8">
                      ...
                      <span className="font-display text-sm font-bold uppercase tracking-[0.1em] text-silver">{step.label}</span>
                    </div>
                  </div>

                  {/* Instruction text */}
                  <div className="flex flex-1 flex-col p-4">
                    <h3 className="font-display text-base font-bold leading-tight tracking-tight text-silver">{step.title}</h3>
                    <p className="mt-2 font-display text-[12px] leading-[1.5] text-darkgray">{step.desc}</p>
                    ...
                    <div className="mt-3 space-y-1 border-t border-slate2/40 pt-2.5">
                      {step.tips.map((t, ti) => (
                        <div key={ti} className="flex items-start gap-2">
                          ...
                          <span className="font-mono text-[8px] uppercase tracking-[0.1em] text-dim">{t}</span>
                        </div>
                      ))}
                    </div>
                  </div>
