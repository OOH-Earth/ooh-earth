...
export default function MediaCorpGlobe({
  corps,
  selected,
  onSelect,
  onBoundsChange,
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      map.addLayer({
        ...
        layout: {
          'icon-image': [
            'case',
            ['==', ['get', 'selected'], true],
            ['concat', 'mc-pin-', ['get', 'scope'], '-sel'],
            ['concat', 'mc-pin-', ['get', 'scope']],
          ],
      ...
      map.on('click', 'mc-pins', (e) => {
        ...
        popupRef.current
          .setLngLat(/** @type {GeoJSON.Point} */ (f.geometry).coordinates.slice())
          .setHTML(corpPopupHTML(p))
          .addTo(map);
        onSelectRef.current?.(p.id);
      });
      map.on('mouseenter', 'mc-pins', () => {
        map.getCanvas().style.cursor = 'pointer';
      });
      map.on('mouseleave', 'mc-pins', () => {
      ...
      map.on('mouseenter', 'mc-clusters', () => {
        map.getCanvas().style.cursor = 'pointer';
      });
      map.on('mouseleave', 'mc-clusters', () => {
        map.getCanvas().style.cursor = '';
      });

      readyRef.current = true;
      setReady(true);
      /** @type {import("maplibre-gl").GeoJSONSource} */ (map.getSource('mc-markers')).setData(
        /** @type {GeoJSON.GeoJSON} */ (dataRef.current),
      );
      /** @type {import("maplibre-gl").GeoJSONSource} */ (map.getSource('mc-coverage')).setData(
        /** @type {GeoJSON.GeoJSON} */ (dataRef.current),
      );
      reportBounds();
    });
  ...
  useEffect(() => {
    ...
    popupRef.current
      ...
      .setHTML(corpPopupHTML(selected))
