...
export default function ReportStep3Classify({ data, onChange }) {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Harm statement */}
      <div>
        <label className="mb-1 block font-mono text-[10px] uppercase tracking-[0.3em] text-dim">What is this ad doing to this space?</label>
        <p className="mb-2 font-mono text-[10px] text-dim/60">To this community? To you? Speak with truth — not for clicks, but for change.</p>
        <textarea value={data.harm_statement} onChange={(e) => onChange({ harm_statement: e.target.value })}
          rows={4} placeholder="Tell us what's wrong with this ad and why it doesn't belong here…"
          className={`${inp} resize-none`} />
      </div>

      {/* Harm tags */}
      <div>
        <label className="mb-2 block font-mono text-[10px] uppercase tracking-[0.3em] text-dim">Categorise the violation <span className="text-dim/50">(select all that apply)</span></label>
        <div className="grid gap-1.5 sm:grid-cols-2">
          {HARM_TAGS.map((h) => {
            const active = (data.harm_tags || []).includes(h.value);
            return (
              <button key={h.value} type="button" onClick={() => toggleHarm(h.value)}
                ...
                <span className={`mt-0.5 h-3.5 w-3.5 shrink-0 border ${active ? "border-flare bg-flare" : "border-slate2"}`} />
                <div>
                  <div className={`font-mono text-[10px] font-bold uppercase tracking-[0.12em] ${active ? "text-flare" : "text-darkgray"}`}>{h.label}</div>
                  <div className="font-mono text-[8px] uppercase tracking-[0.15em] text-dim/60">{h.sdg}</div>
                </div>
              </button>
            );
          })}
      ...
      <div>
        <label className="mb-2 block font-mono text-[10px] uppercase tracking-[0.3em] text-dim">Infrastructure condition</label>
        <div className="grid grid-cols-2 gap-px border border-slate2/60 bg-slate2/40 sm:grid-cols-3">
          {CONDITIONS.map((c) => (
            <button key={c.value} type="button" onClick={() => onChange({ condition: c.value })}
      ...
      {/* Field notes */}
      <div>
        <label className="mb-2 block font-mono text-[10px] uppercase tracking-[0.3em] text-dim">Field notes <span className="text-dim/50">(optional)</span></label>
        <textarea value={data.notes} onChange={(e) => onChange({ notes: e.target.value })}
          rows={3} placeholder="Context, counter-narrative, observations…" className={`${inp} resize-none`} />
      </div>
    </div>
  );
}