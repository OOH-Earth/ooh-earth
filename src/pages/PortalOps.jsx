...
const PROTOCOLS = [
  ...
  ["Polygon", "Live treasury watcher + donation panel", "impl", "Treasury", "Most complete integration. See Risk R-01."],
  ["Solana", "Treasury wallet + tx watcher + Phantom", "impl", "Treasury", "Separate from the pump.fun coin."],
  ["Base", "Only wired mint pipeline", "impl", "Mint pipeline", "No treasury watcher on Base."],
  ["Zora protocol", "Metadata prep + market display on Base", "impl", "Mint pipeline", "Mint tx happens off-app at zora.co/create."],
  ["pump.fun (Solana)", "Hosts the placeholder community coin", "plan", "Coin Registry", "Explicitly flagged as a placeholder."],
];
const WALLETS = [
...
const EXT = [
  ["Etherscan", "ETH tx links"], ["Polygonscan", "Polygon treasury address link"],
  ["Basescan", "Base-chain minted-coin links"], ["Solscan", "SOL tx/token links"],
  ["Blockchair", "ETH tx history for cryptoWatch"], ["CoinGecko", "BTC/ETH/SOL/MATIC price feeds"],
  ["DexScreener", "Live price/mcap/volume for Zora + pump.fun"],
];
const ENTITIES = [
  ["AccessLog", "ADMIN-ONLY (ALL CRUD)"], ["BlogPost", "ADMIN-ONLY (PUBLIC VIA BLOG FN)"],
  ["DigitalBust", "READ: VERIFIED / OWNER / ADMIN"], ["FundingLead", "CREATE OPEN · R/U/D ADMIN"],
  ["IntelCache", "READ OPEN · WRITE ADMIN"], ["LeadClaim", "R/CREATE OPEN · U/D ADMIN"],
  ["Location", "READ: VERIFIED / OWNER / ADMIN"], ["Mint", "ADMIN-ONLY"],
...
const DOCS = [
  ...
  ["BACKLOG.md", "BACKLOG.md"], ["CHANGELOG.md", "CHANGELOG.md"], ["CONTRIBUTING.md", "CONTRIBUTING.md"],
  ["Radio self-host runbook", "RADIO-SELF-HOST-RUNBOOK.md"], ["LICENSE (AGPL-3.0)", "LICENSE"], ["LICENSE-CONTENT (CC BY-SA 4.0)", "LICENSE-CONTENT"],
];
const NODES = {
  "Frontend routes": [["/campaign", "route"], ["/dashboard", "route"], ["/portal/investor", "route"], ["/portal/ops", "route"], ["/zora", "route"], ["MintLocationPanel", "component"]],
  "Backend functions": [["blog", "func"], ["cachedIntel", "func"], ["createDonationCheckout", "func"], ["createProductCheckout", "func"], ["cryptoWatch", "func"], ["fetchMapLocations", "func"], ["fieldStats", "func"], ["importKmlLocations", "func"], ["investorAccess", "func"], ["moderate", "func"], ["n8nPing", "func"], ["personaCtl", "func"], ["stripeWebhook", "func"]],
  "Entities": [["AccessLog", "entity"], ["BlogPost", "entity"], ["DigitalBust", "entity"], ["FundingLead", "entity"], ["IntelCache", "entity"], ["LeadClaim", "entity"], ["Location", "entity"], ["Mint", "entity"], ["Operative", "entity"], ["QuestCompletion", "entity"], ["StoreItem", "entity"], ["User", "entity"]],
  "Chains & external": [["Base", "ext"], ["Bitcoin", "ext"], ["Ethereum", "ext"], ["n8n", "ext"], ["Polygon", "ext"], ["pump.fun", "ext"], ["Solana", "ext"], ["Stripe", "ext"], ["Zora protocol", "ext"]],
};
const EDGES = [
  ["/campaign", "fetchMapLocations"], ["/campaign", "fieldStats"], ["/dashboard", "fieldStats"],
  ["/portal/investor", "investorAccess"], ["/portal/ops", "personaCtl"], ["/zora", "cryptoWatch"],
  ["MintLocationPanel", "createDonationCheckout"], ["blog", "BlogPost"], ["cachedIntel", "IntelCache"],
  ["createDonationCheckout", "Stripe"], ["createDonationCheckout", "FundingLead"], ["createProductCheckout", "Stripe"],
  ["createProductCheckout", "StoreItem"], ["cryptoWatch", "Ethereum"], ["cryptoWatch", "Polygon"], ["cryptoWatch", "Solana"],
  ["fetchMapLocations", "Location"], ["fieldStats", "Operative"], ["fieldStats", "Location"], ["importKmlLocations", "Location"],
  ["investorAccess", "User"], ["moderate", "Location"], ["moderate", "DigitalBust"], ["n8nPing", "n8n"],
  ["personaCtl", "AccessLog"], ["personaCtl", "User"], ["stripeWebhook", "StoreItem"], ["stripeWebhook", "FundingLead"],
  ["stripeWebhook", "Stripe"], ["Mint", "Base"], ["Mint", "Zora protocol"], ["DigitalBust", "QuestCompletion"],
  ["LeadClaim", "FundingLead"], ["Bitcoin", "cryptoWatch"], ["pump.fun", "Solana"],
];
...
function Badge({ children, tone = "mute" }) {
  const map = {
    ok: "border-ozone/50 text-ozone", warn: "border-flare/50 text-flare",
    high: "border-[#FF0040]/60 text-[#FF0040]", mute: "border-slate2/60 text-dim",
...
function Stat({ k, v, sub, accent }) {
  return (
    <div className="border border-slate2/60 bg-card p-4">
      ...
      <div className={`mt-3 font-display text-2xl font-bold tabular leading-none ${accent || "text-silver"}`}>{v}</div>
      {sub && <div className="mt-2 font-mono text-[9px] uppercase tracking-[0.12em] text-dim">{sub}</div>}
    </div>
  );
}
function Gap({ n, children }) {
  return (
    <div className="relative border border-dashed border-slate2/60 p-4 pl-6">
      ...
      <div className="font-mono text-[10px] font-bold uppercase tracking-[0.14em] text-flare">{n}</div>
...
function Block({ title, desc, children }) {
  return (
    <div className="mb-5 border border-slate2/60 bg-card/40 p-6">
      <h2 className="font-mono text-[13px] font-bold uppercase tracking-[0.14em] text-silver">{title}</h2>
...
function Copyable({ v }) {
  ...
  return (
    <button
      onClick={() => { navigator.clipboard?.writeText(v).then(() => { setDone(true); setTimeout(() => setDone(false), 1100); }); }}
    ...
    >
      ...
      {done ? <Check className="h-3 w-3 shrink-0" /> : <Copy className="h-3 w-3 shrink-0 opacity-40 group-hover:opacity-100" />}
    </button>
  );
}
...
function ExecView({ stats }) {
  ...
  return (
    <>
      ...
      <Block title="Environments" desc="From sitemapData.js — imported, not copied, in the real app. Standing rule: prove on BACKUP before promoting to main.">
        <table className="w-full border-collapse">
          <tbody>
            <tr><Td name>STAGE / BACKUP</Td><Td>Internal review · appId <span className="font-mono">6a6748e0…591871</span></Td><Td right><Badge>Internal</Badge></Td></tr>
            <tr><Td name>LIVE / main</Td><Td>Production · appId <span className="font-mono">6a62213c…04ff5</span></Td><Td right><Badge tone="ok">Production</Badge></Td></tr>
          </tbody>
        </table>
      </Block>
...
function MapView() {
  ...
  const dotColor = { route: "border-l-ozone", func: "border-l-flare", component: "border-l-ozone", entity: "border-l-slate2", ext: "border-l-[#39FF14]" };
  return (
    <Block title="Dependency Graph" desc={`${TOTAL_NODES} nodes · ${EDGES.length} verified edges — every edge traces to a real dependency. Click any node to trace its relationships.`}>
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        {Object.entries(NODES).map(([col, arr]) => (
          <div key={col}>
            <h3 className="mb-3 font-mono text-[9px] uppercase tracking-[0.13em] text-dim">{col}</h3>
            {arr.map(([n, t]) => {
              const isSel = sel === n, isRel = adj.has(n);
              const dim = sel && !isSel && !isRel;
              return (
                <button key={n} onClick={() => setSel(isSel ? null : n)}
                  ...
                  <div className="mt-0.5 font-mono text-[8px] uppercase tracking-[0.11em] text-dim">{t}</div>
                </button>
              );
            })}
          </div>
      ...
      <div className="mt-4 min-h-[18px] font-mono text-[11.5px] leading-relaxed text-dim">
        {sel ? <span><span className="text-ozone">{sel}</span> → {[...adj].join(" · ") || "no verified edges recorded"}</span> : "Select a node to trace its edges."}
      </div>
    </Block>
  );
}
...
function ProtocolView() {
  return (
    <Block title="Protocol Stack" desc="Imported from fundConfig.js / zoraConfig.js in the real app.">
      <table className="w-full border-collapse">
        <thead><tr><Th>Protocol</Th><Th>Purpose</Th><Th>Status</Th><Th>Owner</Th><Th>Notes</Th></tr></thead>
        <tbody>{PROTOCOLS.map(([p, u, s, o, n]) => <tr key={p}><Td name>{p}</Td><Td>{u}</Td><Td>{s === "impl" ? <Badge tone="ok">Implemented</Badge> : <Badge>Planned</Badge>}</Td><Td>{o}</Td><Td>{n}</Td></tr>)}</tbody>
      </table>
    </Block>
  );
}
  ...
  return (
    <>
      <Block title="Wallet Addresses" desc="Real treasury wallets from fundConfig.js — already public in the client bundle. Click any address to copy.">
        {WALLETS.map(([k, v]) => <div key={k} className="flex flex-wrap items-center justify-between gap-4 border-b border-slate2/30 py-4 last:border-0"><span className="font-mono text-[11px] uppercase tracking-[0.13em] text-dim">{k}</span><Copyable v={v} /></div>)}
      </Block>
      ...
      <Block title="Recent SOL / ETH Activity" desc="Live via cryptoWatch — most recent treasury signatures/transactions.">
        {recent.length ? recent.map((t) => (
          <a key={t.hash} href={t.url} target="_blank" rel="noreferrer" className="group flex items-center justify-between gap-3 border-b border-slate2/30 py-3 last:border-0">
            <span className="flex items-center gap-3"><Badge tone={t.chain === "SOL" ? "ok" : "mute"}>{t.chain}</Badge><span className="font-mono text-[11.5px] text-silver group-hover:text-ozone">{short(t.hash)}</span></span>
            <ArrowUpRight className="h-3.5 w-3.5 text-dim group-hover:text-ozone" />
          </a>
        )) : <p className="font-mono text-[11px] text-dim">— no recent activity returned, or watcher unavailable.</p>}
      </Block>
    </>
  );
}
...
function CoinsView() {
  return (
    <>
      <Block title="Coin Registry" desc="Every token discovered in the codebase. Click any contract to copy.">
        <table className="w-full border-collapse">
          <thead><tr><Th>Symbol</Th><Th>Chain</Th><Th>Contract</Th><Th>Status</Th><Th>Notes</Th></tr></thead>
  ...
  return (
    <>
      ...
      <Block title="Proposed Functions — planned" desc="New functions to close the open gaps. None exist yet — listed so the roadmap stays visible and honest.">
        <table className="w-full border-collapse">
          <thead><tr><Th>Function</Th><Th>Purpose</Th><Th>Closes</Th></tr></thead>
          <tbody>{PROPOSED.map(([f, p, c]) => <tr key={f}><Td name mono>{f}</Td><Td>{p}</Td><Td><Badge tone="warn">{c}</Badge></Td></tr>)}</tbody>
        </table>
      </Block>
      <Block title="External Dependencies" desc="Third-party APIs actually called from this codebase.">
        <table className="w-full border-collapse">
          <thead><tr><Th>Service</Th><Th>Used for</Th><Th right>Health / latency</Th></tr></thead>
          <tbody>{EXT.map(([s, u]) => <tr key={s}><Td name>{s}</Td><Td>{u}</Td><Td right><Badge>Planned</Badge></Td></tr>)}</tbody>
        </table>
      </Block>
    </>
  );
...
function EntityGrid() {
  return <div className="grid grid-cols-1 gap-2.5 md:grid-cols-2">{ENTITIES.map(([n, r]) => <div key={n} className="border border-slate2/60 p-4"><div className="font-display text-[13px] font-bold text-silver">{n}</div><div className="mt-1 font-mono text-[10px] uppercase tracking-[0.06em] text-dim">{r}</div></div>)}</div>;
}
function InfraView() {
  return (
    <>
...
function SecurityView() {
  ...
  return (
    <>
      ...
      <Block title="Least-Privilege Review" desc="Entity-level row security posture."><EntityGrid /></Block>
      <div className="mb-3 mt-8 font-mono text-[12px] font-bold uppercase tracking-[0.14em] text-dim">Not yet implemented</div>
      <div className="grid gap-2.5 md:grid-cols-2">{notImpl.map((x) => <Gap key={x} n={x}>Not implemented — no process configured.</Gap>)}</div>
    </>
  );
}
...
function DocsView() {
  return (
    <>
      <Block title="Repo Documentation" desc="Every verified root doc — linked to GitHub source (oohearth/ooh-earth) in the real app.">
        <div className="grid grid-cols-1 gap-2.5 md:grid-cols-2">{DOCS.map(([n, f]) => <div key={f} className="flex items-center justify-between gap-3 border border-slate2/60 bg-card p-4"><div><div className="font-display text-[13.5px] font-bold text-silver">{n}</div><div className="mt-1 font-mono text-[10px] uppercase tracking-[0.06em] text-dim">{f}</div></div></div>)}</div>
      </Block>
      <Gap n="Architecture decision records">No formal ADR system — architectural context lives in CLAUDE.md, BACKLOG.md, and commit history instead.</Gap>
    </>
  );
}
...
function MonitorView({ lat }) {
  return (
    <>
      <Block title="Self-measured Client Latency" desc="Round-trip of two live calls (fieldStats, cryptoWatch) measured in this browser on page load — explicitly not infrastructure monitoring.">
        <div className="grid grid-cols-1 gap-2.5 md:grid-cols-2">
          <Stat k="fieldStats round-trip" v={lat?.fieldStats != null ? `${lat.fieldStats} ms` : "— ms"} sub="measured client-side" accent={lat?.fieldStats != null ? "text-ozone" : "text-dim"} />
          <Stat k="cryptoWatch round-trip" v={lat?.cryptoWatch != null ? `${lat.cryptoWatch} ms` : "— ms"} sub="measured client-side" accent={lat?.cryptoWatch != null ? "text-ozone" : "text-dim"} />
        </div>
      </Block>
      <div className="grid gap-2.5 md:grid-cols-2">
...
function DeployView() {
  const items = ["BACKUP build is green (exit 0)", "Smoke-tested the change on BACKUP", "Target appId confirmed explicitly", "dry_run passed on complex edits", "CHANGELOG.md updated"];
  ...
  return (
    <>
      ...
      <div className="mt-5">
        <Block title="GitHub Mirror" desc="Non-destructive bidirectional mirror. S3 stays the source of truth (git_remote_source = s3); code mirrors to GitHub as commits automatically; GitHub-side edits sync back. No manual commit step.">
          <table className="w-full border-collapse"><tbody>
            <tr><Td name>main</Td><Td>→ <span className="font-mono">oohearth/ooh-earth</span> · PUBLIC · AGPL-3.0 + CC BY-SA 4.0</Td></tr>
            <tr><Td name>BACKUP</Td><Td>→ <span className="font-mono">oohearth/ooh-earth-backup</span></Td></tr>
          </tbody></table>
      ...
      <Block title="Promotion Gate — BACKUP → main" desc="The standing rule made mechanical. Every box must be checked before promotion unlocks.">
        {items.map((t, i) => (
          <label key={i} className="flex cursor-pointer items-start gap-3 border-b border-slate2/30 py-3 last:border-0">
            <input type="checkbox" checked={checked[i]} onChange={() => setChecked((c) => c.map((v, j) => j === i ? !v : v))} className="mt-0.5 h-4 w-4 accent-[#EDFF00]" />
            <span className="text-[13.5px] text-silver">{t}</span>
          </label>
        ))}
        <div className="mt-4 flex flex-wrap items-center gap-3">
          <button disabled={!all} onClick={() => setOut(true)} className={`border px-4 py-2 font-mono text-[11px] font-bold uppercase tracking-[0.09em] transition-colors ${all ? "border-ozone bg-ozone text-void hover:bg-flare hover:border-flare" : "border-slate2 text-dim"}`}>Promote to main</button>
          <span className={`font-mono text-[11px] ${all ? "text-ozone" : "text-dim"}`}>{all ? "All checks passed — promotion unlocked." : "Checklist incomplete — promotion locked."}</span>
        </div>
        {out && <pre className="mt-3 overflow-x-auto border border-slate2/50 bg-black p-3 font-mono text-[11px] leading-relaxed text-dim"><span className="text-flare">// WIRE:</span> base44.functions.invoke("promoteBackup", {'{'} target:"main", appId:"6a62213c…04ff5" {'}'}){"\n"}<span className="text-ozone">→ guarded action (proposed). In this preview, no write is performed.</span></pre>}
      </Block>
    </>
  );
}
...
function ConsoleView({ queue, onVerify, busy }) {
  ...
  const ACTIONS = [
    { key: "n8n", title: "n8n bridge test", desc: "Ping the Base44 → n8n bridge (proven end-to-end). test action only.", btn: "Run n8nPing", fn: "n8nPing", args: {} },
    { key: "cache", title: "Refresh intel cache", desc: "Re-pull the daily-cached LLM intel (skyIntel).", btn: "Run cachedIntel", fn: "cachedIntel", args: {} },
    { key: "stats", title: "Refresh field stats", desc: "Re-fetch the PII-free aggregate that powers the orbital HUD.", btn: "Run fieldStats", fn: "fieldStats", args: {} },
    { key: "health", title: "Health probe", desc: "Aggregate reachability probe. Proposed — opsHealth does not exist yet.", btn: "opsHealth (proposed)", proposed: true },
  ];
  const pending = queue?.length || 0;
  return (
    <>
      <Block title="Ops Console" desc="Live actions via base44.functions.invoke(...). Buttons fire the real function and print its response; opsHealth is proposed and only prints the call it would make.">
        <div className="grid grid-cols-1 gap-2.5 md:grid-cols-2">
          {ACTIONS.map((a) => {
            const o = outs[a.key];
            return (
              <div key={a.key} className="border border-slate2/60 bg-card p-5">
                ...
                <button
                  onClick={() => a.proposed ? setOuts((s) => ({ ...s, [a.key]: { state: "stub" } })) : run(a.key, a.fn, a.args)}
                ...
                {o && (
                  <pre className="mt-3 overflow-x-auto border border-slate2/50 bg-black p-2.5 font-mono text-[10.5px] leading-relaxed text-dim">
                    {o.state === "stub" && <><span className="text-flare">// WIRE:</span> base44.functions.invoke("opsHealth")<br /><span className="text-flare">→ proposed function — not called.</span></>}
                    {o.state === "run" && <span className="text-ozone">→ dispatching…</span>}
                    {o.state === "ok" && <><span className="text-[#39FF14]">→ ok · {o.dt} ms</span><br />{o.preview}</>}
      ...
      <Block title="Moderation Queue" desc="Live pending captures (Location, status=pending). Approve/reject writes through immediately via Location.update.">
        {queue == null ? (
          ...
          <>
            {queue.map((r) => (
              <div key={r.id} className="flex flex-wrap items-center justify-between gap-3 border-b border-slate2/30 py-3 last:border-0">
                <div className="min-w-0 text-[13px] text-silver">
                  <span className="truncate">{r.title || "Untitled capture"}</span>
                  <span className="ml-2 font-mono text-[10.5px] text-[#39FF14]">{r.address || (r.lat != null ? `${r.lat?.toFixed?.(4)}, ${r.lng?.toFixed?.(4)}` : "")}</span>
                </div>
                <div className="flex gap-2">
                  <button disabled={busy?.[r.id]} onClick={() => onVerify(r.id, "verified")} className="border border-ozone/50 px-2.5 py-1.5 font-mono text-[10px] font-bold uppercase tracking-[0.08em] text-ozone hover:bg-ozone hover:text-void disabled:opacity-40">Approve</button>
                  <button disabled={busy?.[r.id]} onClick={() => onVerify(r.id, "rejected")} className="border border-[#FF0040]/50 px-2.5 py-1.5 font-mono text-[10px] font-bold uppercase tracking-[0.08em] text-[#FF0040] hover:bg-[#FF0040] hover:text-void disabled:opacity-40">Reject</button>
                </div>
              </div>
            ))}
...
function RosterView() {
  const PEOPLE = [
    ["Dee Sidhom", "Founder", "ADMIN", "role: admin", "Everything · Persona Control · audit log", "ACTIVE", "ok"],
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const dt = Math.round(performance.now() - t0);
        if (alive) { setStats(payload(r)); setLat((l) => ({ ...l, fieldStats: dt })); }
      } catch { /* keep placeholder */ }
      try {
        const t0 = performance.now();
        const r = await base44.functions.invoke("cryptoWatch");
        const dt = Math.round(performance.now() - t0);
        if (alive) { setCrypto(payload(r)); setLat((l) => ({ ...l, cryptoWatch: dt })); }
      } catch { /* keep placeholder */ }
      try {
      ...
      <div className="relative min-h-screen bg-void page-top">
        ...
        <main className="px-5 pb-24 md:px-8">
          <div className="mx-auto mt-20 max-w-lg border border-dashed border-flare/60 bg-flare/[0.03] p-10 text-center">
            ...
            <h1 className="mt-4 font-mono text-[13px] font-bold uppercase tracking-[0.16em] text-flare">Agency access required</h1>
            <p className="mx-auto mt-3 max-w-[42ch] text-[13.5px] leading-relaxed text-dim">The Architecture Operations Portal is restricted to agency members. Ask an admin to switch on your agency status.</p>
            <Link to="/dashboard" className="mt-6 inline-block border border-slate2 px-4 py-2 font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-dim transition-colors hover:border-ozone hover:text-ozone">← Back to dashboard</Link>
          </div>
        </main>
      </div>
    );
      ...
      <main className="aop-grid px-5 pb-28 md:px-8">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2 border border-ozone/20 bg-ozone/[0.03] px-3 py-2">
            <span className="font-mono text-[9px] uppercase tracking-[0.2em] text-dim">Preview clearance</span>
            {[3, 2, 1, 0].map((v) => (
              <button key={v} onClick={() => setClr(v)} className={`border px-2 py-0.5 font-mono text-[9px] uppercase tracking-[0.2em] transition-colors ${clr === v ? "border-ozone bg-ozone text-void" : "border-slate2/60 text-darkgray hover:border-ozone hover:text-ozone"}`}>{CLR_NAMES[v]}</button>
            ))}
            {clr < 3 && <span className="font-mono text-[9px] uppercase tracking-[0.2em] text-flare/80">UI preview only · permissions unchanged</span>}
          </div>
          )}
            ...
            {SECTIONS.map((s) => {
              ...
              return (
                <button key={s.id} onClick={() => setActive(s.id)}
                  ...
                  {s.label}
                  {s.isNew && <span className="absolute -right-1.5 -top-1.5 bg-flare px-1 py-px font-mono text-[7px] font-bold tracking-[0.06em] text-void">NEW</span>}
          ...
          <div className="mt-7">
            {locked ? (
              <div className="border border-dashed border-flare/60 bg-flare/[0.03] p-11 text-center">
                ...
                <h3 className="mt-3 font-mono text-[13px] font-bold uppercase tracking-[0.14em] text-flare">Clearance required</h3>
                <p className="mx-auto mt-2.5 max-w-[46ch] text-[13.5px] leading-relaxed text-dim">This panel is gated to <span className="font-bold text-ozone">{CLR_NAMES[sec.min]}</span> and above. Your clearance is <span className="font-bold text-ozone">{CLR_NAMES[effClr]}</span>.{isAdmin ? " Raise it above to view." : ""}</p>
              </div>
            ) : renderView()}
          </div>
