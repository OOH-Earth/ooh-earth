...
function Toggle({ on, onClick, label, desc }) {
  return (
    <button
      onClick={onClick}
      className="flex w-full items-start justify-between gap-4 border-b border-slate2/30 py-4 text-left last:border-0"
    >
      ...
      <span
        className={`mt-1 flex h-5 w-9 shrink-0 items-center rounded-full border px-0.5 transition-colors ${on ? 'justify-end border-ozone bg-ozone/30' : 'justify-start border-slate2 bg-card'}`}
      >
...
export default function Account() {
  ...
  const hydrate = useCallback((u) => {
    ...
    let stored = null;
    try {
      stored = JSON.parse(localStorage.getItem('ooh_prefs') || 'null');
    } catch {
      /* ignore */
    }
  ...
  const savePrefs = async (next) => {
    setPrefs(next);
    try {
      localStorage.setItem('ooh_prefs', JSON.stringify(next));
    } catch {
      /* ignore */
    }
    setSavingPref(true);
    ...
    try {
      ...
      setSavedPref(true);
      setTimeout(() => setSavedPref(false), 2200);
    } catch (e) {
      setErr(e?.message || 'Preferences saved locally; sync failed.');
    } finally {
      setSavingPref(false);
    }
  };
  ...
  const sendReset = async () => {
    ...
    try {
      ...
      setErr(e?.message || 'Could not send reset email (may not apply to SSO logins).');
    }
  };
  ...
  const exportData = async () => {
    ...
    try {
      let captures = [];
      try {
        captures = await base44.entities.Location.filter(
          { created_by_id: me.id },
          '-created_date',
          500,
        );
      ...
      a.href = url;
      a.download = `ooh-earth-account-${me.id}.json`;
      a.click();
    ...
    <div className="relative min-h-screen bg-void page-top">
      ...
      <main className="px-5 pb-28 md:px-8">
        <div className="mx-auto max-w-3xl">
          ...
          <div className="flex flex-wrap items-end justify-between gap-4 border-b border-slate2/50 pb-6 pt-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <h1 className="mt-1 font-display text-3xl font-bold leading-none tracking-[-0.02em] text-silver md:text-4xl">
                ...
                <p className="mt-2 flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.2em] text-dim">
                  {me.email}
                  <span
                    className={`inline-flex items-center gap-1 border px-1.5 py-0.5 ${ACCESS_BADGE[access] || ACCESS_BADGE.member}`}
                  >
                  ...
                  {isAgency && (
                    <span className="border border-ozone/40 px-1.5 py-0.5 text-ozone">agency</span>
                  )}
                </p>
              </div>
            </div>
            ...
            {quickLinks.map((l) => {
              ...
              return (
                <Link
                  key={l.to}
                  to={l.to}
                  className="flex items-center gap-2 border border-slate2/60 bg-card px-3 py-2 font-mono text-[10px] uppercase tracking-[0.12em] text-dim transition-colors hover:border-ozone/50 hover:text-silver"
                >
          ...
          {err && (
            <div className="mt-5 border border-flare/50 bg-flare/[0.05] px-4 py-3 font-mono text-[11px] text-flare">
              {err}
            </div>
          )}
          ...
          {tab === 'profile' && (
            <div className="mt-6 border border-slate2/60 bg-card/40 p-6">
              ...
              <p className="mt-1.5 mb-5 text-[12.5px] leading-relaxed text-dim">
                This is you across the movement. Editable and saved to your record.
              </p>
              <div className="grid gap-5 md:grid-cols-2">
                ...
                <Field label="Handle" hint="Used for shout-outs and leaderboards.">
                  <input
                    className={inputCls}
                    value={form.handle}
                    onChange={(e) =>
              ...
              <div className="mt-5">
                <Field
                  label="Avatar URL"
                  hint="Link to an image. Leave blank to use your initials."
                >
                  <input
                    className={inputCls}
              ...
              <div className="mt-6 flex items-center gap-3">
                <button
                  onClick={saveProfile}
                  disabled={savingP}
                  className="flex items-center gap-2 border border-ozone bg-ozone px-4 py-2.5 font-mono text-[11px] font-bold uppercase tracking-[0.12em] text-void transition-colors hover:bg-flare hover:border-flare disabled:opacity-50"
                >
                  {savingP ? (
            ...
            <div className="mt-6 border border-slate2/60 bg-card/40 p-6">
              <div className="flex items-center justify-between">
                ...
                {savingPref ? (
                  ...
                  <span className="flex items-center gap-1 font-mono text-[10px] uppercase tracking-[0.18em] text-ozone">
                    <Check className="h-3 w-3" /> saved
                  </span>
                ) : null}
              </div>
              <p className="mt-1.5 mb-3 text-[12.5px] leading-relaxed text-dim">
                Tuned per device instantly and synced to your record.
              </p>
              ...
              <p className="mt-4 font-mono text-[10px] uppercase tracking-[0.15em] text-darkgray">
                Theme lives in the top bar toggle · applies instantly.
              </p>
            </div>
          )}
            ...
            <div className="mt-6 space-y-5">
              <div className="border border-slate2/60 bg-card/40 p-6">
                ...
                <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-b border-slate2/30 py-3">
                  <span className="flex items-center gap-2 text-[13px] text-silver">
                    <Mail className="h-3.5 w-3.5 text-dim" /> {me.email}
                  </span>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 py-4">
                  <div>
                    ...
                    <p className="mt-0.5 text-[12px] leading-relaxed text-darkgray">
                      Send a reset link to your email. Google / SSO logins are managed by your
                      provider.
                    </p>
                  </div>
                  <button
                    onClick={sendReset}
                    disabled={pwSent}
                    className="flex items-center gap-2 border border-slate2 px-4 py-2 font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-silver transition-colors hover:border-ozone hover:text-ozone disabled:opacity-60"
                  >
              ...
              <div className="border border-slate2/60 bg-card/40 p-6">
                ...
                <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                  <p className="text-[12.5px] leading-relaxed text-dim">
                    Sign out of this device. Session tokens are managed by Base44 auth.
                  </p>
                  <button
                    onClick={() => base44.auth.logout('/')}
                    className="flex items-center gap-2 border border-slate2 px-4 py-2 font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-darkgray transition-colors hover:border-flare hover:text-flare"
                  >
                    <LogOut className="h-3.5 w-3.5" /> Sign out
                  </button>
                </div>
              ...
              <div className="border border-slate2/60 bg-card/40 p-6">
                ...
                <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-0.5 text-[12px] leading-relaxed text-darkgray">
                      Download your profile, preferences, and every capture you've filed as JSON.
                    </p>
                  </div>
                  <button
                    onClick={exportData}
              ...
              <div className="border border-flare/40">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-4">
                  <div>
                    ...
                    <p className="mt-0.5 text-[12px] leading-relaxed text-darkgray">
                      Permanently sign out and remove your session. This cannot be undone.
                    </p>
                  </div>
                    ...
                    <AlertDialogContent className="border-flare/50 bg-void">
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription className="font-mono text-[11px] leading-relaxed text-darkgray">
                          This signs you out permanently. To fully erase stored data, contact Base44
                          support after confirming. This cannot be undone.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
          ...
          <div className="mt-12 border-t border-slate2/40 pt-6 font-mono text-[10px] leading-relaxed tracking-[0.05em] text-dim">
            OOH EARTH · ACCOUNT · profile writes via updateMe (safe fields only) · role, access
            &amp; agency are set by admins, never here.
          </div>
        </div>
      </main>
    </div>
