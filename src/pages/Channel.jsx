...
function FeaturedCard({ program, onPlay }) {
  return (
    <article className="group cursor-pointer" onClick={onPlay}>
      <div
        className="relative overflow-hidden border border-slate2"
        style={{ aspectRatio: '16 / 9' }}
      >
        <img
          src={program.thumb}
          alt=""
          className="h-full w-full object-cover opacity-75 transition-transform duration-500 group-hover:scale-[1.03]"
        />
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
        ...
        <span className="shrink-0 font-mono text-[9px] uppercase tracking-[0.25em] text-dim/60">
          {program.channel} · {program.runtime}
        </span>
      </div>
    </article>
  );
...
function ArchiveCard({ program, index, onPlay }) {
  return (
    <article className="group cursor-pointer" onClick={onPlay}>
      <div
        className="relative overflow-hidden border border-slate2/60 transition-colors group-hover:border-ozone/50"
        style={{ aspectRatio: '16 / 9' }}
      >
        <img
          src={program.thumb}
          alt=""
          className="h-full w-full object-cover opacity-65 transition-all duration-500 group-hover:opacity-90 group-hover:scale-[1.04]"
        />
        ...
        <span className="absolute bottom-2 right-2 font-mono text-[8px] uppercase tracking-[0.2em] text-silver/70">
          {program.runtime}
        </span>
      </div>
      <div className="pt-2">
        <span className="font-mono text-[8px] uppercase tracking-[0.3em] text-ozone/80">
          {program.topic}
        </span>
        ...
        <span className="mt-0.5 block font-mono text-[9px] uppercase tracking-[0.15em] text-dim/60">
          {program.channel}
        </span>
      </div>
    </article>
    ...
    <div className="min-h-screen bg-void">
      ...
      <main className="page-top mx-auto max-w-6xl px-5 pb-24">
        ...
        <p className="mt-3 max-w-2xl text-[13px] leading-relaxed text-darkgray">
          A resistance broadcast — curated open-access field footage on subvertising, adbusting,
          brandalism, billboard liberation &amp; culture jamming. Click any story to watch; nothing
          autoplays.
        </p>
        ...
        {active && (
          <div className="mt-8">
            ...
            <div className="mt-3 border border-slate2 bg-void p-4">
              ...
              <h2 className="mt-1 font-display text-xl font-bold tracking-[-0.02em] text-silver">
                {active.title}
              </h2>
              <p className="mt-1.5 text-[13px] leading-relaxed text-darkgray">{active.deck}</p>
              <span className="mt-2 block font-mono text-[9px] uppercase tracking-[0.2em] text-dim/60">
                {active.channel} · {active.runtime}
              </span>
            </div>
          </div>
        )}
