      ...
      <main className="mx-auto max-w-5xl px-5 pb-24 page-top md:px-8">
        <header className="border-b border-slate2/40 pb-8">
          ...
          <p className="mt-4 max-w-2xl font-body text-sm leading-[1.6] text-darkgray">
            Generate a printable field ID badge. Enter your details (or load from the roster),
            review the credential, and print or save as PDF — carry it in the field as proof of your
        ...
        <div className="mt-6 grid gap-6 md:grid-cols-[1fr_auto]">
          ...
          <section className="no-print border border-slate2/60 bg-card p-5">
            <div className="mb-4 font-mono text-[9px] font-bold uppercase tracking-[0.3em] text-silver">
              Generate credential
            </div>
              ...
              {ops.length > 0 && (
                <Field label="Or load from roster">
                  <Select value="" onValueChange={(v) => v && loadFromRoster(v)}>
                    ...
                    <SelectContent>
                      {ops.map((o) => (
                        <SelectItem key={o.id} value={o.handle}>
                          {o.handle} · {o.tier}
                        </SelectItem>
                      ))}
                    </SelectContent>
        ...
        <footer className="mt-10 border-t border-slate2/40 pt-6">
          <p className="font-mono text-[9px] uppercase tracking-[0.3em] text-dim">
            // Field ID Kit v1 · non-state credential · carry with honor
          </p>
        </footer>
      </main>
    </div>
