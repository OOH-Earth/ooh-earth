...
function Stat({ Icon, label, value, accent = '' }) {
  return (
    <div className="border border-slate2/60 bg-card p-4">
      ...
      <div className="mt-0.5 font-mono text-[9px] uppercase tracking-[0.25em] text-dim">
        {label}
      </div>
    </div>
  );
...
function DeploymentRow({ r }) {
  return (
    <Link
      to={`/location/${r.id}`}
      className="group flex items-center gap-3 border border-slate2/50 bg-card p-3 transition-colors hover:border-ozone/60"
    >
      <LocationThumb
        m={{ image: r.image_url, type: r.type, title: r.title }}
        className="h-12 w-12 border border-slate2/50"
      />
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <span className="truncate font-display text-sm font-bold text-silver">{r.title}</span>
          <span
            className={`shrink-0 border px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-[0.2em] ${STATUS_BADGE[r.status] || ''}`}
          >
            {r.status}
          </span>
        </div>
...
export default function FdePortal() {
  ...
  const cities = new Set(
    deployments.map((d) => d.address?.split(',')?.pop()?.trim() || d.title).filter(Boolean),
  ).size;
      ...
      <main className="px-5 pb-24 pt-28 md:px-8">
        <div className="mx-auto max-w-5xl">
          ...
          <div className="flex flex-wrap items-end justify-between gap-4 border-b border-slate2/50 pb-6">
            <div>
              <span className="font-mono text-[10px] uppercase tracking-[0.3em] text-ozone">
                // 0101001 // forward-deployed engineer portal
              </span>
              ...
              <p className="mt-2 flex flex-wrap items-center gap-2 font-mono text-[10px] uppercase tracking-[0.2em] text-dim">
                <span className="flex items-center gap-1 border border-ozone/50 px-2 py-0.5 text-ozone">
                  <Radio className="h-3 w-3 animate-flicker" /> {node}
                </span>
                {user?.email}
              </p>
            </div>
          ...
          {pending.length > 0 && (
            <section className="mt-10">
              <div className="flex items-center justify-between">
                <h2 className="font-display text-lg font-bold uppercase tracking-[-0.01em] text-flare">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2.5 md:grid-cols-3">
              {TOOLS.map((t) => (
                <Link
                  key={t.to}
                  to={t.to}
                  className="group flex flex-col border border-slate2/60 bg-card p-4 transition-colors hover:border-ozone/60 hover:bg-card/80"
                >
                  <t.Icon className="h-4 w-4 text-dim transition-colors group-hover:text-ozone" />
                  <div className="mt-4 font-display text-sm font-bold tracking-[-0.01em] text-silver">
                    {t.label}
                  </div>
            ...
            <section className="mt-10">
              ...
              <div className="mt-4 grid gap-2 md:grid-cols-2">
                {operatives.map((o, i) => (
                  <div
                    key={o.id}
                    className="flex items-center gap-3 border border-slate2/50 bg-card p-3"
                  >
                    <span className="flex h-8 w-8 items-center justify-center border border-slate2/60 font-mono text-[9px] font-bold tabular text-ozone">
                      {nodeId(o.id || o.handle).slice(0, 2)}
                    </span>
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                      ...
                      <p className="truncate font-mono text-[9px] uppercase tracking-[0.15em] text-dim">
                        {o.region || 'undisclosed'} · {o.tier}
                      </p>
                    </div>
                    <span className="font-mono text-[10px] tabular text-ozone">
                      {o.points?.toLocaleString() || 0}
                    </span>
                  </div>
                ))}
              </div>
