...
import {
  ...
  keyDetail,
  ALL_COUNTRIES,
} from '@/components/ooh/access/accessKeyDetails';
  ...
  return (
    <div className="min-h-screen bg-void text-silver">
      ...
      <main className="page-top mx-auto max-w-5xl px-5 pb-24">
        <Breadcrumbs
          items={[{ label: 'Bus Stops', to: '/bus-stops' }, { label: 'Access Keys' }]}
          className="mb-4"
        />
        <Link
          to="/bus-stops"
        ...
        <p className="mt-3 max-w-2xl text-[13px] leading-relaxed text-darkgray">
          Keys unlock. Advertising units are illegal — we've got the keys to claim back our public
          space. The standard tools that open transit-shelter and ad-space housings worldwide, each
          with a map of the bus-stop locations it likely fits.
          ...
          {list.map((k) => (
            ...
            >
              <AccessKeyIcon
                slug={k.slug}
                iconSvg={k.iconSvg}
                chipClassName="h-12 w-12 shrink-0 border border-slate2"
                className="h-9 w-9"
              />
              <span className="min-w-0 flex-1">
                <span className="flex items-center justify-between gap-2">
                  <span className="font-display text-[15px] font-semibold tracking-[-0.01em] text-silver transition-colors group-hover:text-ozone">
                    {k.label}
                  </span>
                ...
                <span className="mt-1 block text-[12px] leading-relaxed text-darkgray">
                  {k.blurb}
                </span>
                {(k.countries || []).length > 0 && (
                  <span className="mt-2 flex flex-wrap gap-1">
                    {k.countries.map((c) => (
                      <span
                        key={c}
                        className="border border-slate2/60 px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-[0.15em] text-dim/70"
                      >
                        {c}
                      </span>
                    ))}
        ...
        {list.length === 0 && (
          <p className="mt-8 font-mono text-[11px] text-dim">
            // no keys documented for {country} yet — log a field check to add the first.
          </p>
        )}
        <p className="mt-6 font-mono text-[10px] leading-relaxed text-dim">
          // more key types exist worldwide and can be added to the collection as field data comes
          in.
        </p>
      </main>
    </div>
