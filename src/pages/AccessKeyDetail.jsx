    ...
    return (
      <div className="min-h-screen bg-void text-silver">
        ...
        <div className="mx-auto max-w-3xl px-5 py-24 text-center">
          ...
          <Link
            to="/access-keys"
            className="mt-6 inline-flex items-center gap-2 border border-slate2 px-4 py-2 font-mono text-[10px] font-bold uppercase tracking-[0.25em] text-silver transition-colors hover:border-ozone hover:text-ozone"
          >
  ...
  return (
    <div className="min-h-screen bg-void text-silver">
      ...
      <main className="page-top mx-auto max-w-6xl px-5 pb-24">
        <Breadcrumbs
          items={[{ label: 'Access Keys', to: '/access-keys' }, { label: k.label }]}
          className="mb-4"
        />
        <Link
          to="/access-keys"
          className="mb-6 hidden items-center gap-2 font-mono text-[10px] uppercase tracking-[0.25em] text-dim transition-colors hover:text-ozone lg:inline-flex"
        >
          <ArrowLeft className="h-3.5 w-3.5" /> Access keys
        </Link>
        ...
        <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
          ...
          <div className="flex flex-col gap-4">
            <div
              className="border border-slate2/60"
              ...
              {stops.length ? (
                ...
                <div className="flex h-full flex-col items-center justify-center gap-2 px-6 text-center">
                  ...
                  <p className="font-mono text-[11px] uppercase tracking-[0.2em] text-dim">
                    No locations added for this page
                  </p>
                  <p className="max-w-xs text-[11px] leading-relaxed text-dim/70">
                    No bus stops confirmed with this key yet. Log a field check to add the first.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="border border-slate2/60 p-4">
              ...
              {stops.length ? (
                <>
                  {probable && (
                    <p className="mb-2 font-mono text-[10px] leading-relaxed text-flare/90">
                      // probable match — london shelters are typically JCDecaux / Clear Channel;
                      key unconfirmed until a field check.
                    </p>
                  )}
                    ...
                    <ul className="grid gap-1.5">
                      {stops.map((s) => (
                        <li key={s.id}>
                          <Link
                            to={`/bus-stop/${s.id}`}
                            className="group flex items-center gap-3 border border-slate2/40 px-3 py-2 transition-colors hover:border-ozone/60 hover:bg-slate2/10"
                          >
                            <span
                              className="h-2 w-2 shrink-0 rounded-full bg-flare"
                              title="probable"
                            />
                            <span className="min-w-0 flex-1">
                              <span className="block truncate font-display text-[13px] font-medium text-silver transition-colors group-hover:text-ozone">
                                {s.name}
                              </span>
                              <span className="font-mono text-[9px] uppercase tracking-[0.2em] text-dim/60">
                                {s.facing} · bus shelter
                              </span>
                            </span>
                            <span className="font-mono text-[9px] uppercase tracking-[0.2em] text-dim/50 transition-colors group-hover:text-ozone">
                              ↗
                            </span>
                          </Link>
                        </li>
          ...
          <div className="lg:sticky lg:top-24 lg:self-start">
            <div className="border border-slate2/60 bg-card p-6">
              <div className="flex justify-center">
                <AccessKeyIcon
                  slug={slug}
                  iconSvg={k.iconSvg}
                  chipClassName="h-16 w-16 border border-ozone/40"
                  className="h-11 w-11"
                />
              </div>

              <div className="mt-5">
                <span className="font-mono text-[9px] uppercase tracking-[0.3em] text-ozone/80">
                  // about
                </span>
                <h1 className="mt-1 font-display text-2xl font-bold tracking-[-0.02em] text-silver">
                  {k.label}
                </h1>
                <p className="mt-2 text-[13px] leading-relaxed text-darkgray">
                  {k.about || k.blurb}
                </p>
                {(k.countries || []).length > 0 && (
                  <div className="mt-3 flex flex-wrap items-center gap-1.5">
                    ...
                    {k.countries.map((c) => (
                      <span
                        key={c}
                        className="border border-slate2/60 px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-[0.15em] text-silver/70"
                      >
                        {c}
                      </span>
                    ))}
              ...
              <div className="mt-6 border-t border-slate2/40 pt-5">
                ...
                <div className="mt-3 relative aspect-[4/3] overflow-hidden border border-slate2/60 bg-brand-smoke">
                  ...
                  <span className="pointer-events-none absolute bottom-2 right-2 flex h-7 w-7 items-center justify-center border border-slate2 bg-brand-smoke/90">
                    {k.iconSvg ? (
                      <img src={k.iconSvg} alt="" className="h-5 w-5 object-contain" />
                    ) : (
                      <KeyGlyph slug={slug} className="h-5 w-5 text-black" />
                    )}
                  </span>
                </div>
              </div>
              ...
              <div className="mt-5 border-t border-slate2/40 pt-5">
                ...
                {k.buyUrl ? (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="mt-2 inline-flex w-full items-center justify-center gap-2 border border-slate2 bg-transparent px-4 py-2.5 font-mono text-[10px] font-bold uppercase tracking-[0.25em] text-silver transition-colors hover:border-ozone hover:bg-ozone hover:text-void"
                  >
            ...
            <div className="mt-3 flex justify-between font-mono text-[10px] uppercase tracking-[0.2em] text-dim">
              <Link to={`/access-keys/${prev}`} className="transition-colors hover:text-ozone">
                ← {ACCESS_KEYS[prev]?.label}
              </Link>
              <Link to={`/access-keys/${next}`} className="transition-colors hover:text-ozone">
                {ACCESS_KEYS[next]?.label} →
              </Link>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8">
          <Link
            to="/report"
            className="inline-flex items-center gap-2 border border-ozone bg-ozone px-4 py-2.5 font-mono text-[10px] font-bold uppercase tracking-[0.25em] text-void transition-colors hover:bg-flare hover:border-flare"
          >
