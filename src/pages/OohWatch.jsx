...
const FACES = [
  {
    icon: Radio,
    name: 'Field',
    desc: 'Live time, shake-to-bond ring, and three complications — nearby offenses, UWB distance, and $OOH balance. The default field face.',
  },
  {
    icon: Hexagon,
    name: 'Hex',
    desc: "Read-only mirror of the engine's hexagram state — the active glyph, its binary, and meaning. Glanceable on the wrist.",
  },
];
    ...
    <div className="min-h-screen bg-void grid-bg text-silver">
      ...
      <div className="mx-auto max-w-5xl page-top px-6 pb-12">
          ...
          <div className="grid grid-cols-1 items-center gap-6 md:grid-cols-[auto_1fr]">
            ...
            <div>
              ...
              <div className="mt-4 flex flex-wrap gap-2 font-mono text-[9px] uppercase tracking-widest text-silver/40">
                <span className="flex items-center gap-1 border border-slate2 px-2 py-1">
                  <Bluetooth className="h-3 w-3" /> BLE 5.3
                </span>
                <span className="flex items-center gap-1 border border-slate2 px-2 py-1">
                  <Cpu className="h-3 w-3" /> SE EAL6+
                </span>
                <span className="flex items-center gap-1 border border-slate2 px-2 py-1">
                  <Battery className="h-3 w-3" /> 7 days
                </span>
                <span className="flex items-center gap-1 border border-slate2 px-2 py-1">
                  <Droplet className="h-3 w-3" /> IP68
                </span>
              </div>
            </div>
          </div>
          ...
          {FACES.map((f) => (
            <div key={f.name} className="border border-slate2 bg-card p-5">
              <div className="flex items-center gap-2">
                <f.icon className="h-4 w-4 text-ozone" />
        ...
        <div className="mt-6 border border-slate2 bg-card p-5">
          ...
          <div className="mt-3 flex items-center gap-2 font-mono text-[10px] text-silver/50">
            <ShieldCheck className="h-4 w-4 text-brand-green" /> The crown PRESS triggers on-device
            signing — the app composes, the secure element signs.
          </div>
        </div>
        ...
        <div className="mt-6 flex flex-wrap items-center gap-4">
          <Link
            to="/lab/devices"
            className="inline-flex items-center gap-2 font-mono text-[11px] uppercase tracking-[0.2em] text-silver/50 transition-colors hover:text-ozone"
          >
          ...
          <Link
            ...
            className="inline-flex items-center gap-2 font-mono text-[11px] uppercase tracking-[0.2em] text-silver/50 transition-colors hover:text-ozone"
          >
            Phone companion <ArrowRight className="h-3 w-3" />
          </Link>
        </div>
      </div>
      <SiteFooter />
