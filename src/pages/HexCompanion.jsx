...
function Phone({ code, name, tab, children }) {
  return (
    <div className="flex flex-col items-center">
      <div className="w-[272px] border border-slate2 bg-void p-1.5">
        <div className="relative h-[540px] overflow-hidden border border-slate2/60 bg-card">
          <div className="flex items-center justify-between border-b border-slate2/60 px-3 py-2 font-mono text-[8px] uppercase tracking-[0.2em] text-silver/40">
            <span className="text-ozone">{code}</span>
            <span>OOH · UWB · SE</span>
          </div>
          ...
          <div className="absolute inset-x-0 bottom-0 grid grid-cols-5 border-t border-slate2/60 bg-void">
            {TABS.map((t) => (
              <div
                key={t.id}
                className={`flex flex-col items-center gap-0.5 py-2 ${t.id === tab ? 'text-ozone' : 'text-silver/25'}`}
              >
      ...
      <div className="mt-2 text-center font-mono text-[9px] uppercase tracking-[0.2em] text-silver/40">
        {code} · {name}
      </div>
    </div>
  );
...
const Row = ({ k, v, c = '' }) => (
  <div className="flex justify-between border-b border-slate2/40 py-1.5 font-mono text-[10px] last:border-0">
    ...
    <span style={c ? { color: c } : undefined} className={c ? '' : 'text-silver'}>
      {v}
    </span>
  </div>
);
const Label = ({ children }) => (
  <div className="mb-2 font-mono text-[9px] uppercase tracking-[0.2em] text-ozone">{children}</div>
);
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between font-mono text-[7px] uppercase tracking-wider text-silver/30">
        {steps.map((s, i) => (
          <span key={s} className={i === 1 ? 'text-ozone' : ''}>
            {i + 1} {s}
          </span>
        ))}
      </div>
      <div className="my-4 flex flex-col items-center">
        <div
          className="flex h-20 w-20 items-center justify-center rounded-full border-2 border-ozone/60"
          style={{ boxShadow: '0 0 24px rgba(237,255,0,.18)' }}
        >
          <Radio className="h-8 w-8 text-ozone" strokeWidth={1.2} />
        </div>
        <div className="mt-2 font-mono text-[10px] uppercase tracking-[0.2em] text-silver">
          Shake to bond
        </div>
        <div className="mt-0.5 font-mono text-[8px] text-silver/40">
          hold the engine · shake twice
        </div>
      </div>
      <div className="font-mono text-[8px] uppercase tracking-widest text-silver/40">
        Available devices
      </div>
      <div className="mt-1.5 space-y-1.5">
        {devices.map((d) => (
          <div
            key={d.n}
            className="flex items-center justify-between border border-slate2 bg-void px-2 py-1.5"
          >
            <div>
              <div className="font-mono text-[10px] text-silver">{d.n}</div>
              <div className="font-mono text-[7px] text-silver/40">{d.s}</div>
            </div>
            <span className="border border-ozone/50 px-2 py-0.5 font-mono text-[7px] uppercase tracking-wide text-ozone">
              pair
            </span>
          </div>
        ))}
      ...
      <div className="mt-2 flex items-center gap-2 font-mono text-[9px] text-silver/50">
        <ShieldCheck className="h-3.5 w-3.5 text-brand-green" /> On-device secure element
        attestation
      </div>
    </>
  );
...
function ScreenMap() {
  ...
  return (
    <>
      ...
      <div
        className="relative h-40 border border-slate2 bg-void"
        style={{
          backgroundImage:
            'linear-gradient(rgba(241,241,241,.04) 1px,transparent 1px),linear-gradient(90deg,rgba(241,241,241,.04) 1px,transparent 1px)',
      ...
      >
        {markers.map(([x, y], i) => (
          <div
            key={i}
            className="absolute h-2 w-2 -translate-x-1/2 -translate-y-1/2 bg-flare"
            style={{ left: `${x}%`, top: `${y}%`, boxShadow: '0 0 8px rgba(255,92,0,.6)' }}
          />
        ))}
      ...
      <div className="mt-3 flex flex-wrap gap-1.5">
        {['Billboards', 'Digital', 'Transit', 'Painted'].map((c, i) => (
          <span
            key={c}
            className={`border px-2 py-1 font-mono text-[8px] uppercase tracking-wide ${i === 0 ? 'border-ozone text-ozone' : 'border-slate2 text-silver/40'}`}
          >
            {c}
          </span>
        ))}
    ...
    <>
      ...
      <div className="flex items-center gap-4">
        <div
          ...
          style={{ textShadow: '0 0 20px rgba(237,255,0,.3)' }}
        >
          {h.char}
        </div>
        <div className="flex flex-1 flex-col gap-1">
          {lines
            ...
            .reverse()
            .map((v, i) => (
              <div key={i} className="flex h-2.5 gap-1">
                {v ? (
      ...
      <div className="mt-3 font-mono text-[9px] uppercase tracking-widest text-silver/40">
        Recent operations
      </div>
  ...
  return (
    <>
      ...
      <div className="mt-3 border border-flare/40 bg-void p-3">
        ...
        <div className="mt-2 border border-ozone bg-ozone py-2 text-center font-mono text-[9px] font-bold uppercase tracking-[0.2em] text-void">
          PRESS engine to sign
        </div>
      </div>
      <div className="mt-2 font-mono text-[8px] text-silver/40">
        App composes · secure element signs on-device
      </div>
    </>
  );
}
    ...
    <>
      ...
      <div className="border border-slate2 bg-void p-3">
        ...
        <div className="mt-2 flex gap-2">
          <div className="flex-1 border border-brand-green/50 py-2 text-center font-mono text-[9px] uppercase tracking-wide text-brand-green">
            Ring ↑ For
          </div>
          <div className="flex-1 border border-slate2 py-2 text-center font-mono text-[9px] uppercase tracking-wide text-silver/40">
            Ring ↓ Against
          </div>
        </div>
      </div>
      ...
      <div className="mt-3 border border-ozone bg-ozone py-2 text-center font-mono text-[9px] font-bold uppercase tracking-[0.2em] text-void">
        PRESS commits vote
      </div>
    </>
  );
  ...
  return (
    <div className="min-h-screen bg-void grid-bg text-silver">
      ...
      <div className="mx-auto max-w-6xl page-top px-6 pb-12">
        <Breadcrumbs
          items={[{ label: 'Lab', to: '/lab' }, { label: 'Phone Companion' }]}
          className="mb-4"
        />
        <header className="flex flex-wrap items-baseline gap-x-5 gap-y-2 border-b border-slate2 pb-4">
          <h1 className="text-2xl font-bold uppercase tracking-[0.14em]">
            Phone <span className="text-ozone">Companion</span>
          </h1>
          <p className="font-mono text-[11px] uppercase tracking-[0.18em] text-silver/50">
            Five screens · the phone half of the engine
          </p>
        ...
        <p className="my-6 max-w-3xl font-mono text-xs leading-loose text-silver/50">
          The engine composes intent physically; the app is the window into it. Every screen maps to
          a live oohearth.app surface — the map is the offense record, the wallet holds the Genesis
          twin, the DAO tab is real governance. The secure element always signs on-device after a
          physical PRESS.
        </p>

        <div className="grid grid-cols-1 justify-items-center gap-8 sm:grid-cols-2 lg:grid-cols-3">
          <Phone code="S-01" name="Pair" tab="PAIR">
            <ScreenPair />
          </Phone>
          <Phone code="S-02" name="City Map" tab="MAP">
            <ScreenMap />
          </Phone>
          <Phone code="S-03" name="Hex State" tab="HEX">
            <ScreenHex />
          </Phone>
          <Phone code="S-04" name="Wallet" tab="WALLET">
            <ScreenWallet />
          </Phone>
          <Phone code="S-05" name="DAO" tab="DAO">
            <ScreenDao />
          </Phone>
        </div>
        ...
        <div className="mt-10 border border-slate2 bg-card p-5">
          <div className="font-mono text-[11px] uppercase tracking-[0.2em] text-ozone">
            Layer routing · Ba Gua mode → app surface
          </div>
