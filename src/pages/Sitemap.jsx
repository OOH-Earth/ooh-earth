...
function VisBadge({ vis }) {
  ...
  return (
    <span
      className={`shrink-0 border px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-[0.2em] ${v.cls}`}
    >
      {v.text}
    </span>
  );
...
function StatusBadge({ status }) {
  ...
  return (
    <span
      className={`shrink-0 border px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-[0.2em] ${s.cls}`}
    >
      {s.text}
    </span>
  );
...
function RouteCard({ r }) {
  ...
  return (
    <Wrap
      {...wrapProps}
      className={`group flex flex-col border p-4 transition-colors ${r.planned ? 'border-dashed border-ozone/40 bg-card/40 hover:border-ozone/70' : 'border-slate2/50 bg-card hover:border-ozone/40'} ${to ? 'cursor-pointer' : ''}`}
    >
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex shrink-0 items-center gap-1">
          ...
          <VisBadge vis={r.vis} />
          <span
            className={`border px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-[0.2em] ${auth.cls}`}
          >
            {auth.text}
          </span>
        </div>
      </div>
      <h3 className="mt-2 flex items-center gap-1 font-display text-base font-bold tracking-[-0.01em] text-silver">
        {r.name}
        {to && (
      ...
      <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 font-mono text-[9px] uppercase tracking-[0.15em] text-dim">
        <span className="flex items-center gap-1">
          <Users className="h-3 w-3" /> {r.audience}
        </span>
      </div>
      {r.flows.length > 0 && (
        <div className="mt-2 flex flex-wrap gap-1.5">
          {r.flows.map((f) => (
            <code
              key={f}
              className="border border-slate2/60 px-1.5 py-0.5 font-mono text-[9px] text-darkgray"
            >
              → {f}
            </code>
          ))}
...
function Journey({ j }) {
  return (
    <div className="flex flex-wrap items-center gap-2">
      <span className={`font-mono text-[9px] uppercase tracking-[0.25em] ${j.color}`}>
        {j.label}
      </span>
      {j.steps.map((s, i) => (
        <span key={s} className="flex items-center gap-2">
          <span className="border border-slate2/60 bg-card px-2 py-1 font-mono text-[10px] text-silver">
            {s}
          </span>
...
export default function Sitemap() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        /* route guard handles auth */
      }
      const admin =
        !!me &&
        ((me.role ?? me.data?.role) === 'admin' || (me.access ?? me.data?.access) === 'admin');
      if (admin) {
        ...
        try {
          ...
          if (d?.ok)
            setIntel({
              access_ladder: d.access_ladder || [],
              architecture: d.architecture || [],
              loose_ends: d.loose_ends || [],
            });
        } catch {
          /* fail closed — sections stay empty */
        } finally {
  ...
  const canSee = (vis) =>
    vis === 'public' || (vis === 'internal' && canInternal) || (vis === 'stage' && canStage);

  const groups = SITEMAP_GROUPS.map((g) => ({
    ...g,
    routes: g.routes.filter((r) => canSee(r.vis)),
      ...
      <main className="px-5 pb-24 pt-28 md:px-8">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="border-b border-slate2/50 pb-6">
            <div className="flex flex-wrap items-center gap-3">
              <span className="font-mono text-[10px] uppercase tracking-[0.3em] text-ozone">
                // site map // ux + flows + review
              </span>
              <span
                className={`border px-2 py-0.5 font-mono text-[9px] font-bold uppercase tracking-[0.25em] ${IS_STAGE ? 'border-[#FF0040]/60 text-[#FF0040]' : 'border-[#39FF14]/50 text-[#39FF14]'}`}
              >
                env · {APP_ENV}
              </span>
            </div>
            ...
            <p className="mt-2 font-display text-sm leading-[1.5] text-darkgray">
              Every route in oohearth.app — {totalRoutes} pages
              {plannedCount ? ` + ${plannedCount} planned` : ''} — with UX explainers, audience,
              auth, visibility tier, and onward flows.{' '}
              {canInternal ? 'Showing all tiers (admin).' : 'Public-safe view.'}
            </p>
          </div>
          ...
          <div className="mt-6 flex flex-wrap items-center gap-x-6 gap-y-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {Object.values(AUTH_LABEL).map((a) => (
                <span
                  key={a.text}
                  className={`border px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-[0.2em] ${a.cls}`}
                >
                  {a.text}
                </span>
              ))}
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {Object.values(VISIBILITY).map((v) => (
                <span
                  key={v.text}
                  className={`border px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-[0.2em] ${v.cls}`}
                >
                  {v.text}
                </span>
              ))}
            ...
            {isAdmin && (
              ...
              >
                {previewPublic ? (
                  <>
                    <Eye className="h-3.5 w-3.5" /> Public view — click to restore
                  </>
                ) : (
                  <>
                    <EyeOff className="h-3.5 w-3.5" /> Preview public view
                  </>
                )}
              </button>
            )}
          ...
          {groups.map((g) => (
            <section key={g.group} className="mt-10">
              <div className="flex flex-col gap-1 border-l-2 border-ozone/60 pl-4">
                <span className={`font-mono text-[10px] uppercase tracking-[0.3em] ${g.accent}`}>
                  {g.group}
                </span>
              ...
              <div className="mt-5 space-y-2">
                {intelLoading && intel.access_ladder.length === 0 && (
                  <div className="font-mono text-[10px] uppercase tracking-[0.2em] text-dim">
                    // loading internal intel…
                  </div>
                )}
                {intel.access_ladder.map((t) => (
                  <div
                    key={t.tier}
                    className="flex flex-col gap-2 border border-slate2/50 bg-card p-3 md:flex-row md:items-center"
                  >
                    <span
                      className={`w-fit shrink-0 border px-2 py-0.5 font-mono text-[10px] font-bold uppercase tracking-[0.2em] ${t.cls}`}
                    >
                      {t.tier}
                    </span>
                    <code className="shrink-0 font-mono text-[10px] text-dim md:w-56">
                      {t.gate}
                    </code>
                    <p className="font-display text-[12px] leading-relaxed text-darkgray">
                      {t.powers}
                    </p>
                  </div>
                ))}
              ...
              <div className="mt-5 space-y-2">
                {archRows.map((a) => (
                  <div key={a.layer} className="border border-slate2/50 bg-card p-3">
                    ...
                    <p className="mt-1 font-display text-[12px] leading-relaxed text-darkgray">
                      {a.detail}
                    </p>
                  </div>
                ))}
              ...
              <div className="mt-5 space-y-2">
                {looseRows.map((e) => {
                  ...
                  return (
                    <div
                      key={e.item}
                      className="flex flex-col gap-2 border border-slate2/50 bg-card p-3 md:flex-row md:items-center md:justify-between"
                    >
                      <div className="min-w-0 flex-1">
                        <div className="font-display text-sm font-semibold text-silver">
                          {e.item}
                        </div>
                        <p className="mt-0.5 font-display text-[12px] leading-relaxed text-darkgray">
                          {e.note}
                        </p>
                      </div>
                      <span
                        className={`shrink-0 border px-2 py-1 font-mono text-[9px] uppercase tracking-[0.2em] ${s.cls}`}
                      >
                        {s.text}
                      </span>
                    </div>
          ...
          <div className="mt-12 flex flex-wrap gap-2">
            <Link
              to="/"
              className="border border-slate2 px-4 py-2 font-mono text-[10px] font-bold uppercase tracking-[0.25em] text-darkgray transition-colors hover:border-ozone hover:text-ozone"
            >
              ← Home
            </Link>
            {isAdmin && (
              <Link
                to="/dashboard"
                className="border-2 border-ozone bg-ozone px-4 py-2 font-mono text-[10px] font-bold uppercase tracking-[0.25em] text-void transition-colors hover:bg-flare hover:border-flare"
              >
                Open Console →
              </Link>
            )}
