...
export default function NftCreator() {
  const [config, setConfig] = useState({
    casing: "slab", finish: "clear", title: "Clean City",
    grade: "9.5", serial: "OOH-00001", labelColor: "ozone",
  });
  ...
  return (
    <div className="min-h-screen bg-void grid-bg text-silver">
      ...
      <div className="mx-auto max-w-6xl page-top px-6 pb-12">
        <Breadcrumbs items={[{ label: "Lab", to: "/lab" }, { label: "NFT Creator" }]} className="mb-4" />
        <header className="flex flex-wrap items-baseline gap-x-5 gap-y-2 border-b border-slate2 pb-4">
          <h1 className="text-2xl font-bold uppercase tracking-[0.14em]">NFT <span className="text-ozone">Creator</span></h1>
          <p className="font-mono text-[11px] uppercase tracking-[0.18em] text-silver/50">Subvertising · Adbusting · 3D slab studio</p>
          <div className="ml-auto flex items-center gap-4 font-mono text-[10px] uppercase tracking-[0.2em]">
            <Link to="/lab" className="text-silver/40 hover:text-ozone">Lab</Link>
            <Link to="/zora" className="text-silver/40 hover:text-ozone">Zora</Link>
            <span className="border border-flare/40 px-2 py-0.5 text-flare">Studio · Prototype</span>
          </div>
        </header>
        ...
        <div className="mt-6 border border-slate2 bg-card p-5">
          <div className="font-mono text-[11px] uppercase tracking-[0.2em] text-ozone">Premade designs · adbusting themes</div>
          <div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">
            {PREMADE_DESIGNS.map((p) => {
              const lc = LABEL_COLORS.find((c) => c.id === p.labelColor) || LABEL_COLORS[0];
              return (
                <button key={p.id} onClick={() => applyPreset(p)} className="group flex flex-col gap-2 border border-slate2 p-3 text-left transition-colors hover:border-ozone/50">
        ...
        <div className="mt-6 border border-slate2 bg-card p-5">
          ...
          <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3">
            <Link to="/lab/device" className="group flex items-center gap-3 border border-slate2 p-4 transition-colors hover:border-ozone/50">
              ...
              <div><div className="text-sm font-bold">3D Device</div><div className="font-mono text-[10px] text-silver/50">Coin-cube geometry reference</div></div>
            ...
            <Link to="/lab/coin" className="group flex items-center gap-3 border border-slate2 p-4 transition-colors hover:border-ozone/50">
              ...
              <div><div className="text-sm font-bold">Genesis Coin</div><div className="font-mono text-[10px] text-silver/50">On-chain coin protocol</div></div>
              <ArrowRight className="ml-auto h-4 w-4 text-silver/30 group-hover:text-ozone" />
            </Link>
            <Link to="/zora" className="group flex items-center gap-3 border border-slate2 p-4 transition-colors hover:border-ozone/50">
              ...
              <div><div className="text-sm font-bold">Zora Mint</div><div className="font-mono text-[10px] text-silver/50">On-chain NFT minting</div></div>
              <ArrowRight className="ml-auto h-4 w-4 text-silver/30 group-hover:text-ozone" />
            </Link>
          </div>
        </div>
