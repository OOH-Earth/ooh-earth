...
import { Loader2, Lock, ExternalLink, Power, Eye, EyeOff, Gauge, ArrowUpRight, Briefcase } from "lucide-react";
...
function Stat({ label, value, tone }) {
  const toneCls = tone === "ozone" ? "text-ozone" : tone === "flare" ? "text-flare" : tone === "red" ? "text-destructive" : "text-silver/40";
  return (
      ...
      <div className="min-h-screen bg-void text-silver">
        ...
        <div className="page-top mx-auto max-w-md px-5 py-20 text-center">
          ...
          <p className="mt-2 font-mono text-[11px] leading-relaxed text-silver/60">
            The careers control console is restricted to admin accounts. Log in with an admin account to manage role status and visibility.
          </p>
      ...
      <div className="page-top mx-auto max-w-5xl px-5 py-10 md:px-8">
        ...
        <div className="mt-4 border border-slate2 bg-card">
          <div className="flex flex-wrap items-center gap-3 border-b border-slate2 px-4 py-3">
            ...
            <h1 className="font-display text-xl font-bold uppercase tracking-[0.16em]">Careers Control <span className="text-ozone">Console</span></h1>
        ...
        <p className="mt-4 max-w-2xl font-mono text-[11px] leading-relaxed text-silver/55">
          Each role toggles status (live / future need / filled / draft) and public visibility. Role content — title, description, responsibilities — stays in code at <span className="text-silver/80">src/components/ooh/careers/roles.js</span>; this console only controls what's live on <Link to="/careers" className="text-ozone hover:underline">/careers</Link> right now.
        </p>
        ...
        {items === null ? (
          ...
          <p className="py-20 text-center font-mono text-[11px] text-silver/50">No roles registered.</p>
        ) : (
          <CollapsibleSection
            title="Roles"
            ...
            right={
              <span className="font-mono text-[9px] tracking-[0.1em] text-silver/35">
                {counts.live} live · {counts.future} future · {counts.filled} filled · {counts.hidden} hidden
              </span>
            }
          >
            <div className="space-y-1.5 p-3">
              {items.map((r, i) => {
                ...
                const led = hidden ? "bg-destructive" : r.status === "live" ? "bg-ozone" : r.status === "future" ? "bg-flare" : "bg-silver/50";
                return (
                  <div key={r.id} className={`border bg-card transition-colors ${hidden ? "border-destructive/30" : "border-slate2 hover:border-silver/30"}`}>
                    ...
                    <div className="flex items-center gap-3 border-b border-slate2 px-4 py-2.5">
                      ...
                      <span className={`h-2 w-2 rounded-full ${led} ${hidden ? "" : "animate-pulse"}`} title={hidden ? "hidden" : st.label} />
                      <span className="font-display text-sm font-bold">{r.title}</span>
                      {roleContent && (
                        <a href={`/careers/${r.role_id}`} target="_blank" rel="noreferrer" className="text-silver/30 transition-colors hover:text-ozone" title="Open role page">
                          <ExternalLink className="h-3 w-3" />
                        </a>
                      )}
                      {!roleContent && (
                        <span className="border border-destructive/40 px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-[0.15em] text-destructive/80">orphaned — removed from roles.js</span>
                      )}
                      <span className="ml-auto font-mono text-[9px] tracking-[0.1em] text-silver/35">{roleContent?.category || "—"}</span>
                    ...
                    <div className="flex flex-wrap items-end gap-x-7 gap-y-4 px-4 py-3.5">
                      <Ctrl label="Status" icon={<Power className="h-3 w-3" />}>
                        {["live", "future", "filled", "draft"].map((s) => (
                          <Tog key={s} active={r.status === s} tone={STATUS_TONE[s]} onClick={() => save(r, { status: s }, "Status")}>
                            {s}
                          </Tog>
                        ))}
                      </Ctrl>
        ...
        <Link to="/careers" className="mt-6 inline-flex items-center gap-2 border border-slate2 px-3 py-1.5 font-mono text-[10px] uppercase tracking-[0.2em] text-silver/70 transition-colors hover:border-ozone hover:text-ozone">
