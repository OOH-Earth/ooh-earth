...
const DEVICE_LOG = [
  ...
  [
    'E',
    'Detented rotary dials — sequencer-driven hexagram program, per-hexagram chord, cyberpunk shell',
    'current',
  ],
];
const PIPELINE = [
...
const TODAY = [
  [
    'Genesis Token · 3D chip spinner',
    'Built TokenChip3D — casino-chip geometry (colored ring, 4 edge spots, recessed field, arched text, center glyph). Full viewer parity with CoinViewer3D: auto-rotate, speed ×0.5/1/2, zoom, 5 bg swatches, corner brackets + scan line, gated PNG export. Replaces the 2D TokenIconStudio.',
  ],
  [
    'Naming disambiguation',
    "Added 'What's in a name' section to Genesis Token — separates $OOHEX from Metaplex Genesis (launchpad tool), Solana Seeker Genesis Token (NFT pass), and GXN altcoin. 'Genesis' = first supply bootstrapping the treasury.",
  ],
  [
    'Shadow grounding fix',
    'Coin + token viewers: contact-shadow + ShadowMaterial ground planes repositioned from y=-1.0/-0.9 to y=-1.55 (flush below the 1.5-radius disc). Previous offset left the shadow detached / cutting through the artifact. Grounded + contact shadow widened to 4.5×3.',
  ],
  [
    'Lighting',
    'Both viewers: 3-point + PMREM RoomEnvironment, PCFSoftShadowMap, ACESFilmic tone mapping retained. Shadow opacity tuned 0.35 → 0.4 for firmer grounding.',
  ],
  [
    'Cinematic shadows',
    'Coin + token viewers: 2K shadow map, blur radius 14, normalBias 0.04 (kills acne), softer contact-shadow falloff (0.34→0.12→0), real-shadow opacity 0.26. Cinema-grade soft penumbra.',
  ],
  [
    'Chip exploded view',
    'Built ChipExplodedView — 4-layer SVG exploded diagram (Finished Face → Web7 PVD Coating → 10% Copper Bond → 90% Steel Core) matched to reference spec. Added Web7 material spec strip + open/verifiable/circular/SDG principles. Wired into Genesis Chip page.',
  ],
];
  ...
  return (
    <div className="min-h-screen bg-void grid-bg text-silver">
      ...
      <div className="mx-auto max-w-4xl page-top px-6 pb-12">
        <Breadcrumbs
          items={[{ label: 'Lab', to: '/lab' }, { label: 'Status Report' }]}
          className="mb-4"
        />
        <header className="flex flex-wrap items-baseline gap-x-5 gap-y-2 border-b border-slate2 pb-4">
          <h1 className="text-2xl font-bold uppercase tracking-[0.14em]">
            Engineering <span className="text-ozone">Status</span>
          </h1>
          <p className="font-mono text-[11px] uppercase tracking-[0.18em] text-silver/50">
            Hex Engine Lab · report 002 · 2026-08-01
          </p>
        ...
        <Section n="00" title="Summary">
          <p className="font-mono text-xs leading-loose text-silver/60">
            The Hex Engine Lab is a ten-piece prototype suite for the tangible I Ching controller,
            Genesis Coin and Living Coin — the physical layer of the oohearth.app ecosystem. The
            full suite is built, proven, and staged on <span className="text-silver">main</span>{' '}
            behind agency auth, awaiting publish. Active development continues on{' '}
            <span className="text-silver">BACKUP</span> (the interactive 3D device and its mechanics
            roadmap). Everything builds clean. Grounded in the science of the 64-hexagram torus
            (Judge, 2021).
          </p>
        </Section>
            ...
            <table className="w-full border-collapse font-mono text-[11px]">
              <thead>
                <tr className="border-b border-slate2 text-left">
                  {['Piece', 'Route', 'State'].map((h) => (
                    <th
                      key={h}
                      className="px-3 py-2 font-normal uppercase tracking-widest text-ozone"
                    >
                      {h}
                    </th>
                  ))}
                ...
                {REGISTER.map(([name, route, state, c]) => (
                  <tr key={route} className="border-b border-slate2/50 last:border-0">
                    <td className="px-3 py-2 text-silver">
                      <Link to={route} className="hover:text-ozone">
                        {name}
                      </Link>
                    </td>
                    <td className="px-3 py-2 text-silver/50">{route}</td>
                    <td className="px-3 py-2">
                      <span className="inline-flex items-center gap-2 text-silver/70">
                        <span className={`h-2 w-2 ${dot(c)}`} />
                        {state}
                      </span>
                    </td>
                  </tr>
          ...
          <div className="space-y-2 font-mono text-[11px]">
            {DEVICE_LOG.map(([rev, desc, tag]) => (
              <div key={rev} className="flex gap-3 border-b border-slate2/40 pb-1.5">
                <span className={tag === 'current' ? 'text-ozone' : 'text-silver/40'}>
                  REV {rev}
                </span>
                <span className="flex-1 text-silver/70">{desc}</span>
                <span className={tag === 'current' ? 'text-brand-green' : 'text-silver/30'}>
                  {tag}
                </span>
              </div>
            ))}
          ...
          <div className="space-y-2 font-mono text-[11px]">
            {PIPELINE.map(([k, v]) => (
              <div key={k} className="flex gap-3">
                <span className="w-16 shrink-0 text-ozone">{k}</span>
                <span className="text-silver/60">{v}</span>
              </div>
            ))}
          </div>
          <p className="mt-3 font-mono text-[10px] text-silver/40">
            Rule: prove on BACKUP → promote to main → Founder publishes. main + BACKUP kept
        ...
        <Section n="04" title="Roadmap">
          <div className="grid grid-cols-1 gap-x-8 gap-y-2 sm:grid-cols-2">
            {ROADMAP.map(([k, v]) => (
              <div key={k} className="border-b border-slate2/40 pb-1.5 font-mono text-[11px]">
                <span className="text-ozone">{k}</span>{' '}
                <span className="text-silver/50">— {v}</span>
              </div>
            ))}
          </div>
        </Section>
          ...
          <ul className="space-y-2 font-mono text-[11px] leading-relaxed text-silver/60">
            {OPEN.map((o, i) => (
              <li key={i} className="flex gap-2">
                <span className="text-flare">▸</span> {o}
              </li>
            ))}
          </ul>
        </Section>
        ...
        <div className="mt-10 border-t border-slate2 pt-4 font-mono text-[10px] uppercase tracking-widest text-silver/40">
          OOH Earth Lab · Engineering · logged to sitemap under Lab ·{' '}
          <a
            href="https://www.laetusinpraesens.org/docs20s/chinouro.php"
            target="_blank"
            rel="noreferrer"
            className="underline hover:text-ozone"
          >
            science ref
          </a>
        </div>
