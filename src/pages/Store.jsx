...
const STORE_TOUR = [
  {
    title: 'OOH Store',
    body: 'Field research, digital products, and drops — all built on this platform and sourced from our own data. Every sale funds the Field Offensive.',
  },
  {
    title: 'How it works',
    body: 'Library docs open in a reader. Digital products check out through Stripe and unlock to your account. Drops open their own page. Free items download on the spot.',
    cta: true,
  },
];
...
function Badge({ children, tone = 'ozone' }) {
  ...
  return (
    <span
      className={`flex items-center gap-1 border ${tones[tone]} bg-void/80 px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-[0.2em] backdrop-blur`}
    >
      {children}
    </span>
  );
  ...
  return (
    <div className="group flex flex-col overflow-hidden border border-slate2/50 bg-card transition-all duration-300 hover:border-ozone/50 hover:shadow-[0_0_0_1px_rgba(237,255,0,0.15),0_18px_40px_-24px_rgba(237,255,0,0.25)]">
      <div className="relative aspect-[4/3] overflow-hidden border-b border-slate2/50 bg-void">
        {item.image_url ? (
          <Image
            src="https://media.base44.com/images/public/6a62213cff3ccbca88c04ff5/bea0a19dc_Screenshot_2026-08-03_at_232051.jpg"
            alt={item.title}
        ...
        <div className="absolute bottom-2 right-2">
          <span
            className={`border px-2 py-1 font-display text-sm font-black tabular backdrop-blur ${isFree ? 'border-[#39FF14]/50 bg-void/80 text-[#39FF14]' : 'border-ozone/50 bg-void/80 text-ozone'}`}
          >
            {priceLabel(item)}
          </span>
        </div>
      ...
      <div className="flex flex-1 flex-col p-4">
        <h3 className="font-display text-base font-bold tracking-[-0.01em] text-silver">
          {item.title}
        </h3>
        {item.subtitle && (
        ...
        {item.description && (
          <p className="mt-2 flex-1 font-display text-[12px] leading-relaxed text-darkgray line-clamp-3">
            {item.description}
          </p>
        )}
        {item.tags?.length ? (
          <div className="mt-3 flex flex-wrap gap-1">
            {item.tags.slice(0, 3).map((t) => (
              <span
                key={t}
                className="border border-slate2/50 px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-[0.15em] text-darkgray"
              >
                {t}
              </span>
            ))}
...
function Grid({ items, ownedIds, onAction, busy }) {
  ...
  if (!items.length)
    return (
      <div className="border border-slate2/60 bg-card p-12 text-center font-mono text-[10px] uppercase tracking-[0.3em] text-dim">
        // Nothing here yet
      </div>
    );
...
export default function Store() {
  ...
  const shown = useMemo(() => {
    ...
    list.sort((a, b) => {
      if (sort === 'featured')
        return (
          (b.featured ? 1 : 0) - (a.featured ? 1 : 0) || (a.sort_order || 0) - (b.sort_order || 0)
        );
      if (sort === 'newest')
  ...
  const deliver = async (item) => {
    try {
      ...
      if (d?.owned || d?.free) {
        ...
        if (d.content) {
          downloadItemPdf({ ...item, content: d.content });
          return;
        }
      }
  ...
  const onAction = async (item) => {
    ...
    const isFree = item.status === 'free' || Number(item.price_usd) === 0;
    if (isFree || ownedIds.has(item.id)) {
      await deliver(item);
      return;
    }
    if (inIframe) {
      setError('Checkout works only from the published app — open oohearth.app in its own tab.');
      return;
    }
    setBusy(item.id);
    try {
      ...
      if (e?.message === 'login_required' || /401/.test(String(e?.message))) {
        navigate('/login');
        return;
      }
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="border-b border-slate2/50 pb-8">
            <span className="font-mono text-[10px] uppercase tracking-[0.3em] text-ozone">
              // commerce · built on this platform
            </span>
            ...
            <p className="mt-3 max-w-2xl font-display text-sm leading-[1.6] text-darkgray md:text-base">
              Field research, digital products, mockups, and drops — sourced from our own data and
              built on oohearth.app. Every sale funds the Field Offensive: no ads.
            </p>
            <div className="mt-4">
              <Link
                to="/store/admin"
                className="inline-flex items-center gap-1.5 border border-slate2 px-3 py-1.5 font-mono text-[9px] font-bold uppercase tracking-[0.2em] text-silver/60 transition-colors hover:border-ozone hover:text-ozone"
              >
          ...
          {thanks && (
            <div className="mt-6 flex items-center gap-2 border border-[#39FF14]/40 bg-[#39FF14]/5 p-3">
              ...
              <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-[#39FF14]">
                // Payment received — thank you. Your purchase is unlocked to your account; open it
                below or from its page.
              </span>
            </div>
          )}
          {error && (
            <p className="mt-4 font-mono text-[10px] uppercase tracking-[0.2em] text-flare">
              {error}
            </p>
          )}
          ...
          <div className="mt-8 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div className="flex flex-wrap gap-1.5">
              {['all', ...cats, 'components'].map((c) => {
                ...
                const label =
                  c === 'all' ? 'All' : c === 'components' ? 'Components' : meta?.label || c;
                const active = filter === c;
                return (
                  <button
                    key={c}
                    onClick={() => setFilter(c)}
                    className={`border px-3 py-1.5 font-mono text-[9px] font-bold uppercase tracking-[0.2em] transition-colors ${active ? 'border-ozone bg-ozone text-void' : 'border-slate2/60 text-darkgray hover:border-ozone hover:text-ozone'}`}
                  >
                    {label}
                  </button>
                );
              ...
              <div className="flex items-center gap-1.5">
                ...
                {SORTS.map((s) => (
                  <button
                    ...
                    className={`px-2 py-1 font-mono text-[9px] uppercase tracking-[0.15em] transition-colors ${sort === s.id ? 'text-ozone' : 'text-darkgray hover:text-ozone'}`}
                  >
                    {s.label}
                  </button>
                ))}
          ...
          {filter === 'all' && featured && (
            <button
              onClick={() => onAction(featured)}
              className="group mt-6 flex w-full flex-col overflow-hidden border border-ozone/40 bg-card text-left transition-colors hover:border-ozone md:flex-row"
            >
              <div className="relative aspect-[16/9] w-full overflow-hidden border-b border-slate2/50 bg-void md:aspect-auto md:w-1/2 md:border-b-0 md:border-r">
                {featured.image_url ? (
                  <Image
                    src="https://media.base44.com/images/public/6a62213cff3ccbca88c04ff5/652891b63_ooh-nft-OOH-00001.png"
                    alt={featured.title}
                    className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-[1.03]"
                  />
                ) : (
                  <ProductPreview item={featured} />
                )}
              ...
              <div className="flex flex-1 flex-col justify-center p-6 md:p-8">
                <span className="font-mono text-[9px] uppercase tracking-[0.25em] text-ozone">
                  {(CAT_META[featured.category] || CAT_META.library).label}
                </span>
                <h2 className="mt-2 font-display text-2xl font-bold tracking-[-0.01em] text-silver md:text-3xl">
                  {featured.title}
                </h2>
                {featured.description && (
                  <p className="mt-2 max-w-lg font-display text-[13px] leading-relaxed text-darkgray line-clamp-3">
                    {featured.description}
                  </p>
                )}
                <div className="mt-4 flex items-center gap-3">
                  <span className="font-display text-xl font-black text-ozone">
                    {priceLabel(featured)}
                  </span>
                  <span className="flex items-center gap-1 font-mono text-[9px] font-bold uppercase tracking-[0.25em] text-silver group-hover:text-ozone">
          ...
          <div className="mt-6">
            {filter === 'components' ? (
              <>
                <div className="mb-4 font-mono text-[9px] uppercase tracking-[0.25em] text-dim">
                  // Live components built on this site — Framer / React / SVG
                </div>
