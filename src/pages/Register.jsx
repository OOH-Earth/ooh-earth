...
export default function Register() {
  ...
  if (showOtp) {
    return (
      <AuthShell>
        ...
        <p className="mt-1 font-mono text-[11px] tracking-[0.05em] text-dim">We sent a 6-digit code to <span className="text-silver">{email}</span></p>

        {error && <div className="mt-5 border border-flare/40 bg-flare/[0.06] px-3 py-2 font-mono text-[11px] text-flare">{error}</div>}

        <input
          value={otpCode}
          onChange={(e) => setOtpCode(e.target.value.replace(/\D/g, "").slice(0, 6))}
          inputMode="numeric" autoComplete="one-time-code" autoFocus maxLength={6} placeholder="––––––"
        ...
        <button onClick={handleVerify} disabled={loading || otpCode.length < 6} className="mt-5 flex w-full items-center justify-center gap-2 border-2 border-ozone bg-ozone py-3 font-mono text-xs font-bold uppercase tracking-[0.2em] text-void transition-colors hover:border-flare hover:bg-flare disabled:opacity-50">
        ...
        <div className="mt-4 text-center font-mono text-[10px] uppercase tracking-widest text-dim">
          {resendIn > 0 ? <>Resend in {resendIn}s</> : (
            <button onClick={handleResend} className="text-silver transition-colors hover:text-ozone">Resend code</button>
          )}
          {resent && resendIn > 0 && <span className="ml-2 text-brand-green">· sent</span>}
        </div>
        <button onClick={handleBack} className="mt-3 flex w-full items-center justify-center gap-1.5 font-mono text-[10px] uppercase tracking-widest text-darkgray transition-colors hover:text-silver">
  ...
  return (
    <AuthShell>
      ...
      <p className="mt-1 font-mono text-[11px] uppercase tracking-[0.15em] text-dim">Join the network</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <div className="relative">
            ...
            <input id="password" type={showPw ? "text" : "password"} autoComplete="new-password" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} className={`${INPUT} pr-10`} required />
        ...
        <div className="space-y-1.5">
          ...
          <div className="relative">
            <Lock className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-darkgray" aria-hidden="true" />
            <input id="confirm" type={showPw ? "text" : "password"} autoComplete="new-password" placeholder="••••••••" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className={INPUT} required />
          </div>
        </div>
      ...
      <div className="mt-6 border-t border-slate2/60 pt-5 text-center font-mono text-[11px] text-dim">
        Already enlisted? <Link to="/login" className="text-ozone hover:underline">Log in →</Link>
      </div>
    </AuthShell>
  );
}
