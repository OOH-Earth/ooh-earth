...
export default function RadioOps() {
  ...
  useEffect(() => {
    if (!RADIO_OPS_ENABLED) { setLoaded(true); return; }
        ...
        {!RADIO_OPS_ENABLED && (
          <div className="mt-6 border border-flare/40 bg-flare/5 p-6">
            ...
            <p className="mt-3 font-mono text-[11px] leading-relaxed tracking-[0.03em] text-silver/70">
              The broadcast engine isn&apos;t wired up yet. Stand up AzuraCast, then paste your base URL and
              station listen URL into <span className="text-ozone">src/lib/radioOps.js</span> — this page and the
              in-app OOH Radio channel light up automatically.
            </p>
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-3">
              ...
              <div className="border border-slate2 bg-void/60 p-6 md:col-span-2">
                <div className="flex items-center justify-between">
                  ...
                  {np?.isLive ? (
                    <span className="font-mono text-[8px] font-bold uppercase tracking-[0.25em] text-flare">● Live DJ</span>
                  ) : (
                    <span className="font-mono text-[8px] uppercase tracking-[0.25em] text-dim">AutoDJ</span>
                  )}
                </div>
                ...
                <div className="mt-4 flex gap-4">
                  <div className="h-20 w-20 shrink-0 overflow-hidden border border-slate2 bg-slate2/30">
                    {np?.art ? (
                      <img src={np.art} alt="" className="h-full w-full object-cover" onError={(e) => { e.currentTarget.style.display = "none"; }} />
                    ) : (
                      <div className="flex h-full w-full items-center justify-center"><Radio className="h-6 w-6 text-dim" /></div>
                    )}
                  </div>
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-3 h-1 w-full overflow-hidden rounded-full bg-slate2">
                      <div className="h-full bg-ozone transition-all duration-1000" style={{ width: `${progress}%` }} />
                    </div>
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="border border-slate2 bg-void/60 p-5">
                  <div className="font-mono text-[8px] uppercase tracking-[0.25em] text-dim">Up next</div>
                  <div className="mt-2 truncate font-mono text-[11px] tracking-[0.02em] text-silver/80" title={np?.playingNext || ""}>
                    {np?.playingNext || "—"}
                  </div>
                </div>
              </div>
                ...
                <div className="mt-4 divide-y divide-slate2/40">
                  {np.recentHistory.map((h, i) => (
                    <div key={i} className="flex items-center gap-3 py-2.5">
                      <span className="font-mono text-[9px] tabular-nums text-dim">{String(i + 1).padStart(2, "0")}</span>
                      <span className="truncate font-mono text-[11px] tracking-[0.02em] text-silver/75">{h.text}</span>
                    </div>
                  ))}
                </div>
              </div>
