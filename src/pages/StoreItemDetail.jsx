...
const MD_COMPONENTS = {
  h2: (p) => (
    <h2
      className="mt-6 mb-2 font-display text-lg font-bold tracking-[-0.01em] text-silver"
      {...p}
    />
  ),
  ...
  blockquote: (p) => (
    <blockquote className="border-l-2 border-ozone/60 pl-4 text-darkgray italic" {...p} />
  ),
};
...
export default function StoreItemDetail() {
  ...
  const loadContent = async () => {
    ...
    try {
      ...
      setGate({ ...GATE_DEFAULT, locked: true });
    }
  };
  ...
  const buy = async () => {
    ...
    if (isFree || gate.owned) {
      if (gate.file_url) {
        window.open(gate.file_url, '_blank');
        return;
      }
      if (gate.content) {
        downloadItemPdf({ ...item, content: gate.content });
        return;
      }
      await loadContent();
      return;
      ...
      <div className="min-h-screen bg-void page-top">
        ...
        <div className="px-8 pt-32">
          ...
          <button
            ...
            className="mt-4 flex items-center gap-2 border border-slate2 px-3 py-2 font-mono text-[9px] uppercase tracking-[0.25em] text-silver hover:border-ozone hover:text-ozone"
          >
    ...
    <div className="relative min-h-screen bg-void page-top">
      ...
      <main className="px-5 pb-24 pt-28 md:px-8">
        <div className="mx-auto max-w-3xl">
          <Breadcrumbs
            items={[{ label: 'Store', to: '/store' }, { label: item.title }]}
            className="mb-4"
          />
          <button
            onClick={() => navigate('/store')}
            className="flex items-center gap-2 font-mono text-[9px] uppercase tracking-[0.25em] text-dim transition-colors hover:text-ozone"
          >
          ...
          <div className="mt-4 flex flex-col gap-5 md:flex-row">
            ...
            <div className="flex-1">
              ...
              <h1 className="mt-2 font-display text-3xl font-bold leading-[1.05] tracking-[-0.02em] text-silver">
                {item.title}
              </h1>
              {item.subtitle && (
              ...
              {item.description && (
                <p className="mt-3 font-display text-[13px] leading-[1.6] text-darkgray">
                  {item.description}
                </p>
              )}
              {item.tags?.length ? (
              ...
              <div className="mt-5 flex flex-wrap items-center gap-3">
                <span className="font-display text-2xl font-black tabular text-silver">
                  {priceLabel(item)}
                </span>
                <button
                  onClick={buy}
          ...
          {gate.loading ? (
            ...
            gate.content ? (
              <article className="mt-8 border-t border-slate2/50 pt-6">
                <div className="mb-4 flex items-center justify-between">
                  <span className="flex items-center gap-1.5 font-mono text-[9px] uppercase tracking-[0.25em] text-[#39FF14]">
                    <CheckCircle2 className="h-3 w-3" />{' '}
                    {gate.free ? 'Free access' : 'Owned — full access'}
                  </span>
                  <button
                    onClick={() => downloadItemPdf({ ...item, content: gate.content })}
                    className="flex items-center gap-1.5 border border-slate2/60 px-3 py-1.5 font-mono text-[9px] font-bold uppercase tracking-[0.2em] text-darkgray transition-colors hover:border-ozone hover:text-ozone"
                  >
            ...
            <div className="mt-8 border-t border-slate2/50 pt-6">
              <div className="flex flex-col items-start gap-3 border border-ozone/40 bg-card p-6">
                <span className="flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.25em] text-ozone">
                  <Lock className="h-3.5 w-3.5" /> Locked — purchase to unlock
                </span>
                <p className="font-display text-[13px] leading-[1.6] text-darkgray">
                  The full {item.category === 'library' ? 'document' : 'deliverable'} unlocks to
                  your account after purchase — read it here anytime and re-download whenever you
                  need.
                ...
                <button
                  onClick={buy}
                  disabled={busy}
                  className="flex items-center gap-1.5 border border-ozone bg-ozone px-4 py-2.5 font-mono text-[9px] font-bold uppercase tracking-[0.25em] text-void transition-colors hover:bg-flare hover:border-flare disabled:opacity-50"
                >
                  {busy ? (
                    <Loader2 className="h-3.5 w-3.5 animate-spin" />
                  ) : (
