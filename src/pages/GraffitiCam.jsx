...
const inp =
  'w-full bg-void border border-slate2 px-3 py-2.5 font-mono text-[11px] text-silver outline-none transition-colors placeholder:text-dim focus:border-flare';
...
export default function GraffitiCam() {
  ...
  const submit = async () => {
    ...
    if (!isFinite(latN) || !isFinite(lngN)) {
      setError('GPS coordinates required — use Locate me, search, or enter manually.');
      return;
    }
    setSubmitting(true);
  ...
  return (
    <div className="min-h-screen bg-void grid-bg text-silver">
      ...
      <div className="mx-auto max-w-4xl page-top px-6 pb-12">
        <Breadcrumbs
          items={[{ label: 'Lab', to: '/lab' }, { label: 'Graffiti Camera' }]}
          className="mb-4"
        />
        ...
        <p className="my-6 max-w-2xl font-mono text-xs leading-loose text-silver/50">
          Point-and-shoot graffiti and street art documentation. Capture the piece, classify the
          medium and style, log the coordinates — feeds the graffiti portal and the live field map.
        </p>

        {done ? (
          <div className="border border-ozone/40 bg-ozone/5 p-8">
            {done.queued ? (
              <MapPin className="h-7 w-7 text-flare" />
            ) : (
              <CheckCircle2 className="h-7 w-7 text-ozone" />
            )}
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <Link
                to="/graffiti"
                className="inline-flex items-center gap-2 border border-ozone bg-ozone px-5 py-2.5 font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-void transition-colors hover:bg-flare hover:border-flare"
              >
                View on graffiti portal
              </Link>
              <Link
                to="/map"
                className="inline-flex items-center gap-2 border border-slate2 px-5 py-2.5 font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-darkgray transition-colors hover:border-ozone hover:text-ozone"
              >
                Open field map
              </Link>
              <button
                onClick={reset}
                className="inline-flex items-center gap-2 border border-slate2 px-5 py-2.5 font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-darkgray transition-colors hover:border-flare hover:text-flare"
              >
          ...
          <div className="space-y-6">
              ...
              <div className="flex items-center justify-between">
                ...
                {capturedUrl && (
                  <button
                    ...
                    className="flex items-center gap-1.5 font-mono text-[9px] uppercase tracking-[0.2em] text-silver/40 transition-colors hover:text-flare"
                  >
            ...
            {capturedUrl && (
              <div className="space-y-4">
                <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-silver/40">
                  Step 02 — Classify the piece
                </span>

                {/* Surface type */}
                <div>
                  <span className="mb-2 block font-mono text-[9px] uppercase tracking-[0.3em] text-dim">
                    Surface type
                  </span>
                  ...
                  {photoSource === 'exif' && photoCoords ? (
                    ...
                    <div className="flex items-center gap-2 border border-ozone/40 bg-ozone/5 px-3 py-2">
                      ...
                      <span className="font-mono text-[11px] text-silver">
                        {photoSource === 'search' ? 'Search result' : 'GPS lock'}
                      </span>
                      <span className="ml-auto font-mono text-[10px] tabular text-ozone">
                        {photoCoords.lat.toFixed(5)}, {photoCoords.lng.toFixed(5)}
                      </span>
                    ...
                    <div className="flex items-center justify-between border border-slate2 px-3 py-2">
                      ...
                      <button
                        ...
                        onClick={locate}
                        disabled={locating}
                        className="font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-ozone transition-opacity hover:opacity-70 disabled:opacity-40"
                      >
                        {locating ? '…' : 'Locate me'}
                      </button>
                    </div>
                  ...
                  {photoSource !== 'exif' && !photoCoords && (
                    <PlaceSearch
                      onSelect={(r) => {
                        ...
                        setLat(r.lat.toFixed(5));
                        setLng(r.lng.toFixed(5));
                      }}
                    />
                  )}
                ...
                <button
                  ...
                  className="flex w-full items-center justify-center gap-2 border-2 border-flare bg-flare py-4 font-mono text-[11px] font-bold uppercase tracking-[0.3em] text-void transition-colors hover:bg-ozone hover:border-ozone disabled:opacity-40"
                >
                  {submitting ? (
