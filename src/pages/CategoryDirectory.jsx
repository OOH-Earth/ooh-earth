    ...
    <div className="min-h-screen bg-void text-silver">
      ...
      <main className="page-top mx-auto max-w-6xl px-5 pb-24 md:px-8">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          <span className="mr-1 font-mono text-[9px] uppercase tracking-[0.25em] text-dim">
            Region
          </span>
          ...
          {regionFacets.map((f) => (
            <button
              key={f.slug}
              onClick={() => setRegion(f.slug)}
              className={chip(region === f.slug)}
            >
          ...
          <Link
            to="/regions"
            className="ml-1 inline-flex items-center gap-1 font-mono text-[9px] uppercase tracking-[0.2em] text-dim/70 hover:text-ozone"
          >
            + roadmap
          </Link>
        </div>
        ...
        {access && activeRegion.access === 'field' && (
          <div className="mt-4 flex items-start gap-2 border border-flare/30 bg-card/40 px-3 py-2">
            ...
            <p className="font-display text-[11px] leading-[1.5] text-darkgray">
              ...
              {activeRegion.city} has no open public register of outdoor units — records are
              member-gathered and unit-type classification here is provisional. Counts are a floor,
              not a census.
            </p>
          </div>
        )}
        ...
        {items.length === 0 ? (
          <div className="border border-dashed border-slate2/60 bg-card/40 p-10 text-center">
            ...
            <p className="mt-3 font-display text-sm text-darkgray">
              ...
              <Link to="/report" className="text-ozone hover:underline">
                File the first one →
              </Link>
            </p>
          </div>
              ...
              return (
                ...
                >
                  <div className="relative aspect-[16/10] overflow-hidden">
                    ...
                    <span className="absolute left-2 top-2 border border-ozone/50 bg-void/70 px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-[0.2em] text-ozone">
                      {cat.label}
                    </span>
                    {rg && (
                      <span className="absolute right-2 top-2 border border-slate2/70 bg-void/70 px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-[0.2em] text-darkgray">
                        {rg.city}
                      </span>
                    )}
                  </div>
                  <div className="flex flex-1 flex-col p-4">
                    ...
                    {it.address && (
                      <p className="mt-1 font-display text-[11px] leading-[1.4] text-darkgray">
                        {it.address}
                      </p>
                    )}
                    {it.notes && (
                      <p className="mt-2 font-mono text-[10px] leading-[1.4] text-dim/80">
                        {it.notes}
                      </p>
                    )}
                  </div>
