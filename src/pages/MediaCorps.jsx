  ...
  const filtered = useMemo(
    () =>
      (corps || []).filter((c) => {
        ...
        const searchMatch =
          !q ||
          `${c.name} ${c.hq} ${c.parent || ''} ${(c.regions || []).join(' ')}`
            .toLowerCase()
            .includes(q);
  ...
  const visibleCorps = useMemo(() => {
    ...
    return filtered.filter(
      (c) =>
        isFinite(c.lat) &&
        isFinite(c.lng) &&
        c.lat >= mapBounds.south &&
        c.lat <= mapBounds.north &&
        c.lng >= mapBounds.west &&
        c.lng <= mapBounds.east,
    );
  ...
  return (
    <div className="fixed inset-0 flex flex-col overflow-hidden bg-void pt-[calc(7rem_+_env(safe-area-inset-top))] md:pt-[calc(8rem_+_env(safe-area-inset-top))] pb-[calc(76px_+_env(safe-area-inset-bottom))] lg:pb-0">
      ...
      <div className="flex items-center justify-between gap-2 border-b border-slate2/60 bg-void/90 px-3 py-2 backdrop-blur-md md:px-5">
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-[9px] uppercase tracking-[0.3em] text-ozone">
            // Media Corps Registry
          </span>
          <span className="hidden font-mono text-[9px] uppercase tracking-[0.2em] text-dim lg:inline">
            · {stats.total} corps · {stats.countries}+ countries ·{' '}
            {(stats.panels / 1000000).toFixed(1)}M+ panels
          </span>
        </div>
        <div className="flex items-center gap-2">
          <MapStyleSwitcher />
      ...
      <div className="flex flex-wrap items-center gap-1.5 border-b border-slate2/60 bg-void px-3 py-2 md:px-5">
        ...
        {/* Global South filter */}
        <button
          onClick={() => setGlobalSouthOnly((v) => !v)}
          title="Filter to Global South HQs (below 35°N)"
        ...
        {viewMode !== 'list' && (
          <label className="hidden items-center gap-1.5 cursor-pointer select-none md:flex">
            ...
            />
            <span className="font-mono text-[8px] uppercase tracking-[0.15em] text-darkgray">
              Search as I move
            </span>
          </label>
        )}
      ...
      <div className="hidden items-center gap-3 border-b border-slate2/60 bg-void px-5 py-1.5 md:flex">
        ...
        {Object.entries(stats.regions)
          ...
          .map(([k, v]) => (
            <span
              key={k}
              className="flex items-center gap-1 font-mono text-[8px] uppercase tracking-[0.15em] text-darkgray"
            >
              <span className="h-1.5 w-1.5 rounded-full bg-slate2" /> {k.replace(/ /g, '\u00A0')}{' '}
              <span className="text-ozone">{v}</span>
            </span>
          ))}
      </div>
        ...
        >
          <div className="border-b border-slate2/60 px-4 py-2.5 flex items-center justify-between">
            ...
            {searchAsMove && viewMode !== 'list' && (
              <span className="font-mono text-[7px] uppercase tracking-[0.15em] text-ozone/60">
                live
              </span>
            )}
          </div>
          <div
            className={`min-h-0 flex-1 overflow-y-auto divide-y divide-slate2/30 ${viewMode === 'list' ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-slate2/30 p-px' : ''}`}
          >
            {corps === null ? (
                  ...
                  {viewMode === 'list' ? (
                    <>
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <span className="font-display text-sm font-bold text-silver truncate">
                          {corp.name}
                        </span>
                        {isGlobalSouth(corp) && (
                          <span className="font-mono text-[6px] uppercase tracking-[0.1em] text-flare/70 border border-flare/30 px-1">
                            GS
                          </span>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-3 font-mono text-[8px] text-darkgray">
                        ...
                        {corp.panels > 0 && (
                          <span>
                            {corp.panels >= 1000000
                              ? (corp.panels / 1000000).toFixed(1) + 'M'
                              : Math.round(corp.panels / 1000) + 'K'}{' '}
                            panels
                          </span>
                        )}
                      </div>
                      {corp.parent && (
                        <div className="font-mono text-[7px] text-dim/60 truncate">
                          {corp.parent}
                        </div>
                      )}
                    </>
                    ...
                    <>
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5">
                          <span className="truncate font-display text-[13px] font-bold text-silver">
                            {corp.name}
                          </span>
                          {isGlobalSouth(corp) && (
                            <span className="font-mono text-[6px] uppercase tracking-[0.1em] text-flare/70 border border-flare/30 px-0.5">
                              GS
                            </span>
                          )}
                        </div>
                        <div className="truncate font-mono text-[8px] uppercase tracking-[0.1em] text-dim">
                          {corp.hq}
                        </div>
                      </div>
                      {corp.panels > 0 && (
            ...
            {corps !== null && sidebarCorps.length === 0 && (
              <div className="px-4 py-8 text-center font-mono text-[10px] uppercase tracking-[0.2em] text-dim">
                {searchAsMove ? 'No corps in view — pan the map' : 'No match'}
              </div>
            )}
          </div>
          {viewMode !== 'list' && (
            <div className="border-t border-slate2/60 p-3">
              <Link
                to="/report"
                className="flex items-center justify-center gap-1.5 border border-ozone bg-ozone px-3 py-2 font-mono text-[9px] font-bold uppercase tracking-[0.2em] text-void transition-colors hover:bg-flare hover:border-flare"
              >
              ...
              <>
                {viewMode === 'globe' ? (
                  <MediaCorpGlobe
                    corps={filtered}
                    selected={selected}
                    onSelect={(id) => setSelected(corps.find((c) => c.id === id))}
                ...
                <div className="absolute right-3 top-3 z-[1000] hidden flex-col gap-1.5 border border-slate2 bg-void/90 p-2.5 backdrop-blur-md sm:flex">
                  ...
                  <span className="flex items-center gap-1.5 font-mono text-[8px] uppercase tracking-[0.12em] text-darkgray">
                    <span className="h-2 w-2 rounded-full bg-ozone" /> Global
                  </span>
                  <span className="flex items-center gap-1.5 font-mono text-[8px] uppercase tracking-[0.12em] text-darkgray">
                    <span className="h-2 w-2 rounded-full bg-flare" /> Regional
                  </span>
                  <span className="flex items-center gap-1.5 font-mono text-[8px] uppercase tracking-[0.12em] text-darkgray">
                    <span className="h-2 w-2 rounded-full bg-darkgray" /> Local
                  </span>
                  <span className="mt-1 flex items-center gap-1.5 font-mono text-[7px] uppercase tracking-[0.15em] text-dim/60">
                    <Building2 className="h-2.5 w-2.5" /> Pin size = panels
                  </span>
                  {showCoverage && (
                    <span className="flex items-center gap-1.5 font-mono text-[7px] uppercase tracking-[0.15em] text-ozone/60">
                      <Radar className="h-2.5 w-2.5" /> Circle = reach
                    </span>
                  )}
                </div>
