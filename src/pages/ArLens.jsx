            ...
            <>
              ...
              <div>
                ...
                <p className="mt-1 max-w-xs font-mono text-[10px] uppercase tracking-[0.2em] text-dim">
                  {camErr}. If you're in a preview iframe, open the published app on your phone —
                  camera permissions require a secure, top-level origin.
                </p>
              </div>
              <Link
                to="/map"
                className="border border-slate2 px-4 py-2 font-mono text-[10px] uppercase tracking-[0.25em] text-silver hover:border-ozone hover:text-ozone"
              >
                Back to map
              </Link>
            </>
      ...
      {camReady && (
        <>
          ...
          {locked && (
            <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
              <div className="relative" style={{ width: `${box}%`, aspectRatio: '3 / 2' }}>
                ...
                {layers.takeover && (
                  <div
                    className="absolute inset-0 flex flex-col items-center justify-center overflow-hidden border-2 border-ozone"
                ...
                {layers.intel && (
                  <>
                    <div className="absolute -left-2 -top-2 -translate-x-full -translate-y-1/2 border border-slate2 bg-void/85 px-2.5 py-1.5 backdrop-blur-md">
                      ...
                      <div className="font-display text-base font-black tabular text-flare">
                        {pm25 != null ? `${Math.round(pm25)}` : '—'}
                      </div>
                      <div className="font-mono text-[7px] uppercase tracking-[0.15em] text-dim">
                        {pmMult ? `${pmMult}× WHO` : 'µg/m³'}
                      </div>
                    </div>
                    ...
                    <div className="absolute -bottom-2 left-1/2 translate-y-full border border-slate2 bg-void/85 px-2.5 py-1.5 backdrop-blur-md">
                      ...
                      <div className="font-mono text-[9px] tabular text-silver">
                        {pos ? `${pos.lat.toFixed(4)}, ${pos.lng.toFixed(4)}` : 'acquiring…'}
                      </div>
                    </div>
                  </>
                )}
          ...
          <div className="absolute inset-x-0 top-0 flex items-center justify-between p-4">
            <Link
              to="/map"
              className="flex h-9 w-9 items-center justify-center border border-slate2 bg-void/70 text-silver backdrop-blur-md hover:border-ozone hover:text-ozone"
            >
              <X className="h-4 w-4" />
            </Link>
            <div className="flex items-center gap-2 border border-slate2 bg-void/70 px-3 py-1.5 backdrop-blur-md">
              <span
                className={`h-1.5 w-1.5 rounded-full ${geoErr ? 'bg-flare' : pos ? 'bg-ozone' : 'bg-dim animate-blink'}`}
              />
            ...
            <button
              onClick={stopCamera}
              className="flex h-9 w-9 items-center justify-center border border-slate2 bg-void/70 text-silver backdrop-blur-md hover:border-flare hover:text-flare"
            >
          ...
          {!locked && (
            <div className="absolute right-4 top-1/2 flex -translate-y-1/2 flex-col gap-2">
              <button
                onClick={() => setBox((b) => Math.min(95, b + 10))}
                className="flex h-9 w-9 items-center justify-center border border-slate2 bg-void/70 text-silver backdrop-blur-md hover:border-ozone hover:text-ozone"
              >
                <Plus className="h-4 w-4" />
              </button>
              <button
                onClick={() => setBox((b) => Math.max(25, b - 10))}
                className="flex h-9 w-9 items-center justify-center border border-slate2 bg-void/70 text-silver backdrop-blur-md hover:border-ozone hover:text-ozone"
              >
          ...
          <div className="absolute inset-x-0 bottom-0 p-5">
            {!locked ? (
              ...
              <div className="flex flex-col items-center gap-3">
                {layers.log && (
                  <div className="w-full max-w-sm">
                    ...
                    {logState === 'error' && (
                      <button
                        onClick={logReport}
                        className="flex w-full items-center justify-center gap-2 border-2 border-flare px-6 py-3 font-mono text-[11px] uppercase tracking-[0.25em] text-flare"
                      >
                ...
                <button
                  onClick={resetLock}
                  className="flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.25em] text-silver/60 hover:text-ozone"
                >
    ...
    <button
      ...
      className={`flex flex-col items-center gap-1 border px-2 py-2 backdrop-blur-md transition-colors ${
        on
          ...
          : 'border-slate2 bg-void/70 text-darkgray hover:text-silver'
      }`}
    >
      {icon}
