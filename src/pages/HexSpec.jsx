      ...
      <table className="w-full border-collapse font-mono text-[11px]">
        <thead>
          <tr className="border-b border-slate2 text-left">
            {head.map((h) => (
              <th key={h} className="px-3 py-2 font-normal uppercase tracking-widest text-ozone">
                {h}
              </th>
            ))}
          </tr>
          ...
          {rows.map((r, i) => (
            <tr key={i} className="border-b border-slate2/50 last:border-0">
              {r.map((c, j) => (
                <td
                  key={j}
                  className={`px-3 py-2 align-top ${j === 0 ? 'text-silver' : 'text-silver/60'}`}
                >
                  {c}
                </td>
              ))}
...
export default function HexSpec() {
  const layerRows = DIAL.map((k) => {
    const t = TRI[k];
    return [`${t.sym} ${t.el}`, t.layer, t.verb, t.verbDesc];
  });
  ...
  return (
    <div className="min-h-screen bg-void grid-bg text-silver">
      ...
      <div className="mx-auto max-w-4xl page-top px-6 pb-12">
        <Breadcrumbs
          items={[{ label: 'Lab', to: '/lab' }, { label: 'Engineering Spec' }]}
          className="mb-4"
        />
        <header className="flex flex-wrap items-baseline gap-x-5 gap-y-2 border-b border-slate2 pb-4">
          <h1 className="text-2xl font-bold uppercase tracking-[0.14em]">
            Engineering <span className="text-ozone">Spec</span>
          </h1>
          <p className="font-mono text-[11px] uppercase tracking-[0.18em] text-silver/50">
            Hex Engine · digital half · v1.0
          </p>
        ...
        <Section n="00" title="System overview">
          <p className="font-mono text-xs leading-loose text-silver/60">
            The Hex Engine is a tangible I Ching controller: six rotating rings compose a hexagram
            (0–63), a Ba Gua dial selects one of eight OOH Earth layers, and PRESS executes the
            resulting operation. The physical device composes intent; a secure element signs
            on-device after physical confirmation. This document specifies the digital half —
            protocol, state machine, and BLE service.
          </p>
        </Section>
        ...
        <Section n="04" title="Frame format · 8 bytes">
          ...
          <p className="mt-3 font-mono text-[11px] text-silver/50">
            CRC8 = (0xA5 + hex + mode) mod 256 · little-endian sequence.
          </p>
        </Section>
          ...
          <ul className="space-y-2 font-mono text-[11px] leading-relaxed text-silver/60">
            {OPEN_Q.map((q, i) => (
              <li key={i} className="flex gap-2">
                <span className="text-ozone">·</span> {q}
              </li>
            ))}
          </ul>
        </Section>
