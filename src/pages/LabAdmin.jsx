...
function Stat({ label, value, tone }) {
  const toneCls =
    tone === 'ozone' ? 'text-ozone' : tone === 'flare' ? 'text-flare' : 'text-silver/40';
  return (
      ...
      <div className="min-h-screen bg-void text-silver">
        ...
        <div className="page-top mx-auto max-w-md px-5 py-20 text-center">
          ...
          <p className="mt-2 font-mono text-[11px] leading-relaxed text-silver/60">
            The lab control console is restricted to admin accounts. Log in with an admin account to
            manage prototype access and status.
          </p>
          <Link
            to="/login"
    ...
    <div className="min-h-screen bg-void text-silver grid-bg">
      ...
      <div className="page-top mx-auto max-w-5xl px-5 py-10 md:px-8">
        ...
        <div className="mt-4 border border-slate2 bg-card">
          <div className="flex flex-wrap items-center gap-3 border-b border-slate2 px-4 py-3">
            ...
            <h1 className="font-display text-xl font-bold uppercase tracking-[0.16em]">
              Lab Control <span className="text-ozone">Console</span>
            </h1>
        ...
        <p className="mt-4 max-w-2xl font-mono text-[11px] leading-relaxed text-silver/55">
          Each channel toggles access (public ↔ agency/investor), build status, and hub visibility.
          Changes propagate to the live stack and route gating on save.
        </p>
        ...
        {items === null ? (
          ...
          <CollapsibleSection
            ...
            right={
              <span className="font-mono text-[9px] tracking-[0.1em] text-silver/35">
                {counts.public} pub · {counts.restricted} rest · {counts.live} live ·{' '}
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 border-b border-slate2 px-4 py-2.5">
                      ...
                      <span
                        ...
                        title={hidden ? 'hidden' : pub ? 'public' : 'restricted'}
                      />
                    ...
                    <div className="flex flex-wrap items-end gap-x-7 gap-y-4 px-4 py-3.5">
                      <Ctrl label="Access" icon={<Radio className="h-3 w-3" />}>
                        <Tog
                          active={pub}
                          tone="ozone"
                          onClick={() => save(r, { access: 'public' }, 'Access')}
                        >
                          Public
                        </Tog>
                        <Tog
                          active={!pub}
                          tone="flare"
                          onClick={() => save(r, { access: 'restricted' }, 'Access')}
                        >
                          Agency / Inv
                        </Tog>
                      </Ctrl>
                      ...
                      <Ctrl label="Status" icon={<Power className="h-3 w-3" />}>
                        <Tog
                          active={live}
                          tone="ozone"
                          onClick={() => save(r, { status: 'live' }, 'Status')}
                        >
                          Live
                        </Tog>
                        <Tog
                          active={!live}
                          tone="flare"
                          onClick={() => save(r, { status: 'in_build' }, 'Status')}
                        >
                          In build
                        </Tog>
                      </Ctrl>
