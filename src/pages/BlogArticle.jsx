...
export default function BlogArticle({ scope = 'public' }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setStatus(
          /403|forbidden|agency/i.test(msg)
            ? 'forbidden'
  ...
  const crumbs = isAgency
    ? [
        { label: 'Agency', to: '/agency/blog' },
        { label: 'Newsroom', to: '/agency/blog' },
        ...
        <div className="mx-auto max-w-3xl">
          ...
          {status === 'forbidden' && (
            <div className="flex flex-col items-center gap-3 border border-flare/40 bg-flare/[0.04] p-10 text-center">
              ...
              <p className="max-w-md font-display text-[13px] leading-relaxed text-darkgray">
                This post is part of the internal newsroom. Ask an admin to enable your agency
                status.
              </p>
              <Link
                to="/blog"
          ...
          {(status === 'notfound' || status === 'error') && (
            <div className="flex flex-col items-center gap-3 border border-slate2/50 bg-card p-10 text-center">
              ...
              <Link
                to={base}
                className="mt-1 flex items-center gap-1.5 border border-ozone px-4 py-2 font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-ozone transition-colors hover:bg-ozone hover:text-void"
              >
                <ArrowLeft className="h-3.5 w-3.5" /> Back to {isAgency ? 'newsroom' : 'blog'}
              </Link>
            </div>
          )}
          ...
          {status === 'ok' && post && (
            <article>
              <div className="flex flex-wrap items-center gap-2">
                ...
                {post.audience === 'agency' && (
                  <span className="border border-flare/50 px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-[0.2em] text-flare">
                    agency
                  </span>
                )}
              ...
              <h1 className="mt-4 font-display text-3xl font-bold leading-[1.05] tracking-[-0.02em] text-silver md:text-4xl">
                {post.title}
              </h1>
              {post.author && (
              ...
              {post.excerpt && (
                <p className="mt-4 border-l-2 border-ozone/50 pl-4 font-display text-[15px] leading-[1.6] text-silver/80">
                  {post.excerpt}
                </p>
              )}
              ...
              {post.cta && (
                <div className="mt-8 border border-ozone/40 bg-ozone/[0.04] p-4 font-display text-[14px] leading-relaxed text-silver">
                  {post.cta}
                </div>
              )}
              ...
              <div className="mt-10 flex flex-wrap items-center gap-2 border-t border-slate2/50 pt-6">
                <Link
                  to={base}
                  className="flex items-center gap-1.5 border border-slate2 px-4 py-2 font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-darkgray transition-colors hover:border-ozone hover:text-ozone"
                >
                  <ArrowLeft className="h-3.5 w-3.5" /> {isAgency ? 'Newsroom' : 'Blog'}
                </Link>
                {isAgency && (
                  <button
                    onClick={copyBody}
                    className="flex items-center gap-1.5 border border-flare/50 px-4 py-2 font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-flare transition-colors hover:bg-flare hover:text-void"
                  >
                    {copied ? (
                      <>
                      ...
                      <>
                        <Copy className="h-3.5 w-3.5" /> Copy to post
                      </>
                    )}
                  </button>
                )}
