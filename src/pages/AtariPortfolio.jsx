...
const PORTFOLIO_TOUR = [
  {
    ...
    body: 'Your live treasury console. Every number is read directly from on-chain RPCs and your database — no simulated balances. This tour walks you through it. Press the compass icon in the nav to restart it anytime.',
  },
  {
    target: '[data-tour="total"]',
    title: 'Total portfolio value',
    body: 'On-chain USD (Polygon POL + USDC + USDC.e at live rates) plus recorded fiat pledges. This is the real, spendable plus pledged total.',
  },
  {
    target: '[data-tour="holdings"]',
    title: 'On-chain holdings',
    body: 'Read live from the Polygon RPC against your treasury wallet. POL is priced via CoinGecko; USDC and USDC.e are stablecoin balances.',
  },
  {
    target: '[data-tour="receive"]',
    title: 'Receive addresses',
    body: 'Tap any address to copy it. Share these publicly so adbusters and donors send funds straight to your wallet. Never paste private keys or seed phrases anywhere in this app.',
  },
  {
    target: '[data-tour="inbound"]',
    title: 'Inbound transactions',
    body: 'Recent deposits into your SOL and ETH treasury addresses, pulled from public explorers. Click a row to open it on Solscan or Etherscan.',
  },
  {
    target: '[data-tour="mints"]',
    title: 'Minted coins',
    body: 'Coins minted through the Zora pipeline. Each links to its contract or transaction on Basescan or Solscan. Creator trading fees accrue to your minting wallet.',
  },
  {
    target: '[data-tour="pledges"]',
    title: 'Fiat pledges',
    body: 'The sum of recorded FundingLead amounts — admin-only pledge records. Actually collected fiat lives in Stripe and your bank, not here.',
  },
  {
    title: 'Revenue flows in',
    body: 'Crypto arrives the moment someone sends to your receive addresses. To grow it: share the addresses, run the Zora mint pipeline, and publish the app so Stripe checkout can collect fiat.',
    cta: true,
  },
];
...
function Holding({ label, value, sub = '', accent = '' }) {
  return (
    <div className="border border-slate2/60 bg-card p-4">
      ...
      <div className={`mt-2 font-display text-2xl font-bold tabular ${accent || 'text-silver'}`}>
        {value}
      </div>
      {sub && (
...
function WalletRow({ label, addr }) {
  ...
  return (
    <button
      onClick={copy}
      className="flex items-center justify-between gap-3 border border-slate2/50 bg-card p-3 text-left transition-colors hover:border-ozone/50"
    >
      ...
      {copied ? (
        ...
        <Copy className="h-3.5 w-3.5 shrink-0 text-dim" />
      )}
    </button>
  );
}
        ...
        <div className="mx-auto max-w-5xl">
          ...
          <div className="flex flex-wrap items-end justify-between gap-4 border-b border-slate2/50 pb-6">
            <div>
              <span className="font-mono text-[10px] uppercase tracking-[0.3em] text-ozone">
                // 0101001 // treasury — atari portfolio
              </span>
          ...
          <section data-tour="total" className="mt-8">
            <div className="border border-ozone/40 bg-card p-6">
              <div className="flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.3em] text-ozone">
                ...
                <InfoTip label="Total portfolio value">
                  On-chain USD (Polygon POL + USDC + USDC.e at live rates) plus recorded fiat
                  pledges. Real, spendable + pledged.
                </InfoTip>
              </div>
              <div className="mt-3 font-display text-5xl font-bold tabular text-silver">
                {fmtUsd(totalUsd)}
              </div>
          ...
          <section data-tour="holdings" className="mt-8">
            ...
            <span className="font-mono text-[10px] uppercase tracking-[0.25em] text-dim">
              // polygon treasury — live RPC{' '}
              <InfoTip label="On-chain holdings">
                Live balances read from the Polygon RPC. POL priced via CoinGecko; USDC / USDC.e are
                stablecoins.
              </InfoTip>
            </span>
            <div className="mt-4 grid grid-cols-2 gap-2.5 md:grid-cols-4">
              <Holding
                label="POL"
                value={poly ? poly.matic.toFixed(4) : '—'}
                sub={poly ? `@ ${fmtUsd(poly.maticUsd)}` : ''}
              />
            ...
            {!poly && (
              <p className="mt-3 font-mono text-[9px] uppercase tracking-[0.2em] text-flare">
                // RPC unreachable — retry
              </p>
            )}
          </section>
          ...
          <section data-tour="receive" className="mt-10">
            ...
            <span className="font-mono text-[10px] uppercase tracking-[0.25em] text-dim">
              // tap to copy — share to receive{' '}
              <InfoTip label="Receive addresses">
                Tap to copy. Share publicly so funds land in your wallet. Never paste private keys
                or seed phrases in this app.
              </InfoTip>
            </span>
          ...
          <section data-tour="inbound" className="mt-10">
            <div className="flex items-center justify-between">
              <h2 className="flex items-center gap-1.5 font-display text-lg font-bold uppercase tracking-[-0.01em] text-silver">
                Inbound transactions{' '}
                <InfoTip label="Inbound transactions">
                  Recent deposits into your SOL and ETH treasury wallets, from public explorers.
                  Click to open on Solscan or Etherscan.
                </InfoTip>
              </h2>
              <span className="font-mono text-[10px] uppercase tracking-[0.25em] text-dim">
              ...
              {inbound.length ? (
                inbound.map((t) => (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="group flex items-center gap-3 border border-slate2/50 bg-card p-3 transition-colors hover:border-ozone/50"
                  >
          ...
          <section data-tour="mints" className="mt-10">
            <div className="flex items-center justify-between">
              <h2 className="flex items-center gap-1.5 font-display text-lg font-bold uppercase tracking-[-0.01em] text-silver">
                Minted coins{' '}
                <InfoTip label="Minted coins">
                  Coins minted via the Zora pipeline. Each links to Basescan or Solscan. Creator
                  trading fees accrue to your minting wallet.
                </InfoTip>
              </h2>
              <Link
                to="/zora"
                className="font-mono text-[10px] uppercase tracking-[0.25em] text-dim transition-colors hover:text-ozone"
              >
                // mint pipeline →
              </Link>
            </div>
            <div className="mt-4 grid gap-2 md:grid-cols-2">
              {mints.length ? (
                mints.map((m) => {
                  ...
                  const link = m.token_address
                    ? meta.token(m.token_address)
                    : m.tx_hash
                      ? meta.tx(m.tx_hash)
                      : null;
                  ...
                  return (
                    <Tag
                      key={m.id}
                      {...tagProps}
                      className={`group flex items-center gap-3 border border-slate2/50 bg-card p-3 ${link ? 'transition-colors hover:border-ozone/50' : ''}`}
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <span className="truncate font-display text-sm font-bold text-silver">
                            {m.name || m.symbol || 'Unnamed coin'}
                          </span>
                          {m.symbol && (
                            <span className="font-mono text-[9px] uppercase text-ozone">
                              ${m.symbol}
                            </span>
                          )}
                        </div>
                        <div className="truncate font-mono text-[9px] uppercase tracking-[0.15em] text-dim">
                          {meta.label} · {(m.token_address || m.tx_hash || '').slice(0, 14)}…
                        </div>
                      </div>
                      {link && (
                        <Link2 className="h-3.5 w-3.5 shrink-0 text-dim group-hover:text-ozone" />
                      )}
                    </Tag>
                  );
                })
          ...
          <section data-tour="pledges" className="mt-10">
            <div className="flex items-center justify-between">
              <h2 className="flex items-center gap-1.5 font-display text-lg font-bold uppercase tracking-[-0.01em] text-silver">
                Fiat pledges{' '}
                <InfoTip label="Fiat pledges">
                  Sum of recorded FundingLead amounts — admin-only pledge records. Actually
                  collected fiat is in Stripe and your bank.
                </InfoTip>
              </h2>
              <span className="font-mono text-[10px] uppercase tracking-[0.25em] text-dim">
                // admin ledger
            ...
            <div className="mt-4 border border-slate2/60 bg-card p-6">
              <div className="font-display text-3xl font-bold tabular text-flare">
                {fmtUsd(pledges)}
              </div>
              <div className="mt-1 font-mono text-[9px] uppercase tracking-[0.2em] text-dim">
                // sum of recorded FundingLead amounts
              </div>
            </div>
          </section>
        </div>
