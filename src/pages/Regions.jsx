...
function Badge({ map, k }) {
  ...
  return (
    <span
      className={`border px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-[0.2em] ${b.cls}`}
    >
      {b.text}
    </span>
  );
...
function RegionCard({ r }) {
  ...
  return (
    <div
      className={`flex flex-col border p-4 ${active ? 'border-slate2/60 bg-card' : 'border-dashed border-slate2/40 bg-card/40'}`}
    >
      ...
      <div className="mt-3 flex items-center justify-between font-mono text-[9px] uppercase tracking-[0.2em] text-dim/70">
        ...
        {r.slug === 'london' ? (
          <Link to="/bus-stops" className="hover:text-ozone">
            Directory →
          </Link>
        ) : active ? (
          <Link to="/categories" className="hover:text-ozone">
            Directories →
          </Link>
        ) : (
    ...
    <div className="min-h-screen bg-void text-silver">
      ...
      <main className="page-top mx-auto max-w-6xl px-5 pb-24 md:px-8">
        ...
        <p className="mt-2 max-w-3xl text-[13px] leading-relaxed text-darkgray">
          The record grows city by city. Two things are true of every region at once — how much
          we've
          <span className="text-silver"> mapped</span> it, and whether the{' '}
          <span className="text-silver">open data</span> to map it even exists.
        </p>
        ...
        <div className="mt-6 grid gap-3 md:grid-cols-2">
          <div className="border border-slate2/60 bg-card p-4">
            ...
            <p className="mt-3 font-display text-[12px] leading-[1.5] text-darkgray">
              How far the mapping has got — Live, Partial, WIP, or queued (Planned). We push each as
              far as it goes, then open the next front.
            </p>
          </div>
          <div className="border border-slate2/60 bg-card p-4">
            <div className="font-mono text-[9px] uppercase tracking-[0.3em] text-flare">
              // access — data regime
            </div>
            <div className="mt-3 flex flex-wrap gap-2">
        ...
        <div className="mt-3 flex items-start gap-2 border border-flare/30 bg-card/40 px-4 py-3">
          ...
          <p className="max-w-4xl font-display text-[12px] leading-[1.6] text-darkgray">
            ...
            The right to see public space is not shared equally. In the UK and much of Europe,
            accessible-asset registers, OpenStreetMap coverage, planning portals and
            freedom-of-information let us enumerate unit
            <span className="text-silver"> types</span> authoritatively — open access. In much of
            the Global South, no such register exists: <span className="text-silver">Bangkok</span>{' '}
            is completely uncovered from a unit-type perspective, so the record there is gathered on
            foot by members and every classification is provisional. We surface that difference
            rather than flatten it — an open commons in one place and an opaque one in another is
            exactly the inequity the movement exists to name (SDG&nbsp;11.7).
          </p>
        </div>
