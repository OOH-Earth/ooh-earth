...
function emitReadAloud(v) {
  readListeners.forEach((fn) => fn(v));
}
...
function ensureDeepVoice() {
  ...
  deepVoice =
    ...
    voices.find((v) =>
      /daniel|alex|david|mark|fred|arthur|oliver|thomas|rishi|google uk english male|google us english|microsoft david|microsoft mark/i.test(
        v.name,
      ),
    ) ||
...
export default function useSoundscape() {
  ...
  const toggleReadAloud = useCallback(() => {
    ...
    try {
      localStorage.setItem(KEY2, next ? '1' : '0');
    } catch {}
    emitReadAloud(next);
    if (!next) {
      try {
