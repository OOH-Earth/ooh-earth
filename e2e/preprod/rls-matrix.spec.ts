...
test.describe('REAL_BACKEND — LocationPhoto RLS matrix against BACKUP', () => {
  ...
  test('creator=allowed, other user=denied, anonymous=denied, admin=allowed for a pending row', async ({
    ...
    // A direct, minimal REST call with the creator's own token (rather than
    // driving AdScanLab's real UI, which e2e/preprod/ad-scanner-
    // multiphoto.spec.ts already covers) -- this test is specifically about
    // read-side RLS, and this is the shortest reliable path to a real
    // pending LocationPhoto row owned by a known identity to read back.
    await gotoAsIdentity(page, '/', 'creator'); // establishes a real authenticated context for parity with the other preprod specs, even though this test uses fetch() directly below
    const headers = {
    ...
    const asOther = await readAsToken(preprodToken('otherUser')!);
    ...
    const asAdmin = await readAsToken(preprodToken('admin')!);
