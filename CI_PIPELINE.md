...
| 9 | Add a constrained, manual-only executor so the Lynxie agent orchestrator (external system) can publish an already-reviewed, single-file patch to an existing PR's own head branch without holding a personal access token | **Done, 2026-09-15** — `.github/workflows/lynxie-publish-pr-head.yml`, `workflow_dispatch` only (no schedule/push/pull_request trigger), `permissions: contents: write, pull-requests: read, actions: read` only (no admin, no secrets scope). Uses exclusively this run's own auto-issued `GITHUB_TOKEN` — no new secret, no PAT, reuses the same `github-actions[bot]` authority `release-please.yml` already proves works in this repo (Decision Register #8). Server-side refuses unless: target isn't `main`/`master`, the PR is OPEN, its head branch/SHA exactly match the request, and a committed, reviewed patch file (never `workflow_dispatch` input text) under `.github/lynxie-patches/` — looked up via a strict `[a-zA-Z0-9_-]+` `request_id` — has a sha256 matching a same-commit, reviewed `manifest.json` entry, whose own fields the caller-supplied inputs are cross-checked against before anything is touched. The final `git push` has no `--force`/`-f` anywhere in the file — a non-fast-forward push (remote moved) fails naturally. `dry_run` input (default `true`) runs every check and reports the result with nothing committed or pushed. |
